...
class Split_Enumeration:
    ...
    def split_enumeration(self) -> None:
        ...
        self.pop_add_del_if_empty("cpuid", "(0,0)")
        for _ in range(self.cpucount):
            self.pop_add_del_if_empty("cpuid", "(11,1)")
            self.pop_add_del_if_empty("cpuid", "(11,0)")

        for i in range(self.cpucount):
            self.pop_add_del_if_empty("set_affinity", f"({i})")

        self.pop_add_del_if_empty("get_threads_count", "()")
