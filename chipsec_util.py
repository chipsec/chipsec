...
class ChipsecUtil:
    def __init__(self, argv):
        ...
        self.CHIPSEC_LOADED_AS_EXE = (
            True if (hasattr(sys, "frozen") or hasattr(sys, "importers")) else False
        )
    ...
    def parse_args(self):
        parser = argparse.ArgumentParser(
            usage="%(prog)s [options] <command>", add_help=False
        )
    ...
    def import_cmds(self):
        if self.CHIPSEC_LOADED_AS_EXE:
            ...
            cmds = [
                ...
                if "chipsec/utilcmd/" in i
                and i[-4:] == ".pyc"
                and not os.path.basename(i)[:2] == "__"
            ]
        else:
        ...
        for cmd in cmds:
            try:
                ...
                logger().error(
                    "Exception occurred during import of {}: '{}'".format(cmd, str(msg))
                )
                continue
    ...
    def main(self):
        ...
        if self._load_config:
            try:
                self._cs.init(
                    self._platform,
                    self._pch,
                    comm.requires_driver() and not self._no_driver,
                    self._driver_exists,
                )
        ...
        logger().log(
            "[CHIPSEC] Executing command '{}' with args {}\n".format(
                self._cmd, self.argv[2:]
            )
        )
        comm.run()
