...
class IOBARRuntimeError (RuntimeError):
    pass
class IOBARAccessError (RuntimeError):
    pass
...
class iobar:
    ...
    def is_IO_BAR_defined( self, bar_name ):
        try:
            ...
            if logger().VERBOSE: logger().warn( "'%s' I/O BAR definition not found in XML config" % bar_name)
            return False
    ...
    def get_IO_BAR_base_address( self, bar_name ):
        ...
        base = self.cs.pci.read_dword( int(bar['bus'],16), int(bar['dev'],16), int(bar['fun'],16), int(bar['reg'],16) )
        ...
        if logger().VERBOSE: logger().log( '[io] %s: 0x%016X (size = 0x%X)' % (bar_name,base,size) )
        return base, size
