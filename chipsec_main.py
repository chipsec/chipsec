...
class ChipsecMain:
    ...
    def run_module(self, modx, module_argv):
        ...
        try:
            ...
            if logger().DEBUG and not self._list_tags:
                logger().log("[*] Module path: {}".format(modx.get_location()))
            ...
            if logger().DEBUG:
                logger().log_bad(traceback.format_exc())
            logger().log_error_check(
                "Exception occurred during {}.run(): '{}'".format(
                    modx.get_name(), str(msg)
                )
            )
            raise msg
        return result
    ...
    def load_modules_from_path(self, from_path, recursive=True):
        ...
        for dirname, subdirs, mod_fnames in os.walk(os.path.abspath(from_path)):
            ...
            for modx in mod_fnames:
                if fnmatch.fnmatch(modx, "*.py") and not fnmatch.fnmatch(
                    modx, "__init__.py"
                ):
    ...
    def load_my_modules(self):
        ...
        logger().log(
            '[*] loading common modules from "{}" ..'.format(
                common_path.replace(os.getcwd(), ".")
            )
        )
        ...
        if (chipset.CHIPSET_CODE_UNKNOWN != self._cs.code) and os.path.exists(
            chipset_path
        ):
            logger().log(
                '[*] loading platform specific modules from "{}" ..'.format(
        ...
        logger().log(
            '[*] loading modules from "{}" ..'.format(
                self.Modules_Path.replace(os.getcwd(), ".")
            )
        )
    ...
    def run_loaded_modules(self):
        ...
        for (modx, modx_argv) in self.Loaded_Modules:
            ...
            if result == module_common.ModuleResult.DEPRECATED:
                logger().error(
                    "Module {} does not inherit BaseModule class".format(str(modx))
                )
        ...
        if self._markdown_out:
            chipsec.file.write_file(
                self._markdown_out, results.markdown_full(self._markdown_out)
            )
        ...
        if test_deltas is not None:
            ...
            if not self.no_time:
                logger().log(
                    "[CHIPSEC] Time elapsed            {:.3f}".format(time.time() - t)
                )
            for k in summary.keys():
                if k == "total":
                    ...
                    logger().log(
                        "[CHIPSEC] Modules with {:11}{:d}:".format(k, len(summary[k]))
                    )
                    ...
                    logger().log(
                        "[CHIPSEC] Modules {:16}{:d}:".format(k, len(summary[k]))
                    )
                    for mod in summary[k]:
                        if k == "failed to run":
                            logger().error(mod)
    ...
    def parse_args(self):
        ...
        options.add_argument(
            "-h", "--help", help="show this message and exit", action="store_true"
        )
        options.add_argument(
            "-m",
            "--module",
        ...
        options.add_argument(
            "-vv", "--vverbose", help="very verbose HAL debug mode", action="store_true"
        )
        ...
        adv_options.add_argument(
            "-j", "--json", dest="_json_out", help="specify filename for JSON output"
        )
        adv_options.add_argument(
            "-x",
            "--xml",
        ...
        adv_options.add_argument(
            "--no_time", help="don't log timestamps", action="store_true"
        )
        adv_options.add_argument(
            "--deltas",
            dest="_deltas_file",
        ...
        adv_options.add_argument(
            "--replay", dest="_from_file", help="replay a chipsec run with JSON file"
        )
        adv_options.add_argument(
            "--helper",
            dest="_driver_exists",
        ...
        if (
            self._module_argv
            and len(self._module_argv) == 1
            and self._module_argv[0].count(",")
        ):
            logger().log(
    ...
    def properties(self):
        ret = OrderedDict()
        ret["OS"] = "{} {} {} {}".format(
            self._cs.helper.os_system,
            self._cs.helper.os_release,
            self._cs.helper.os_version,
            self._cs.helper.os_machine,
        )
        ret["Python"] = "Python {}".format(platform.python_version())
        ret["Platform"] = "{}, VID: {:04X}, DID: {:04X}, RID: {:02X}".format(
            self._cs.longname, self._cs.vid, self._cs.did, self._cs.rid
        )
        if not self._cs.is_atom():
            ret["PCH"] = "{}, VID: {:04X}, DID: {:04X} RID: {:02X}".format(
                self._cs.pch_longname,
                self._cs.pch_vid,
                self._cs.pch_did,
                self._cs.pch_rid,
            )
    ...
    def log_properties(self):
        ...
        logger().log(
            "[CHIPSEC] Python  : {} ({})".format(
                platform.python_version(), "64-bit" if self.PYTHON_64_BITS else "32-bit"
            )
        )
        logger().log(
            "[CHIPSEC] Helper  : {} ({})".format(*self._cs.helper.helper.get_info())
        )
        chip_info = (
            "[CHIPSEC] {:8}: {}\n[CHIPSEC]      VID: {:04X}\n"
            "[CHIPSEC]      DID: {:04X}\n[CHIPSEC]      RID: {:02X}"
        ...
        logger().log(
            chip_info.format(
                "Platform", self._cs.longname, self._cs.vid, self._cs.did, self._cs.rid
            )
        )
        if not self._cs.is_atom():
            logger().log(
        ...
        if not self.PYTHON_64_BITS and platform.machine().endswith("64"):
            logger().warn(
                "Python architecture (32-bit) is different from OS architecture (64-bit)"
            )
        ...
        if self._load_config:
            try:
                ...
                if self._unknownPlatform:
                    ...
                    logger().error(
                        "To specify a pch please use --pch command-line option\n"
                    )
                    logger().error(
                        "To load legacy configuration and run anyways please use -i command-line option"
                    )
                    if logger().DEBUG:
                        logger().log_bad(traceback.format_exc())
                    if self.failfast:
