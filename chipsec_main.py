...
def parse_args(argv: Sequence[str]) -> Optional[Dict[str, Any]]:
    ...
    adv_options.add_argument('-I', '--include', dest='IMPORT_PATHS', nargs='+', default=[], help='Specify additional path to load modules from')
...
class ChipsecMain:
    ...
    def __init__(self, switches: Dict[str, Any], argv: Sequence[str]) -> None:
        ...
        if isinstance(self.IMPORT_PATHS, str):
            self.IMPORT_PATHS = [self.IMPORT_PATHS]
        elif self.IMPORT_PATHS is None:
            self.IMPORT_PATHS = []
        self.argv = argv
        self._cs = chipset.cs()
        self.parse_switches()
