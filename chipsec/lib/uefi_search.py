...
def check_rules(efi, rules, entry_name, _log, bLog=True):
    ...
    for rule_name in rules.keys():
        ...
        elif ('guid' in rule) and (rule['guid'] != ''):
            match_mask |= MATCH_GUID
        elif ('regexp' in rule) and (rule['regexp'] != ''):
            match_mask |= MATCH_REGEXP
        elif ('md5' in rule) and (rule['md5'] != ''):
            match_mask |= MATCH_HASH_MD5
        elif ('sha1' in rule) and (rule['sha1'] != ''):
            match_mask |= MATCH_HASH_SHA1
        elif ('sha256' in rule) and (rule['sha256'] != ''):
        ...
        if 'match' in entry:
            ...
            if bmatch:
                ...
                if 'description' in entry:
                    _log.log("    {}".format(entry['description']))
                _log.log(efi)
