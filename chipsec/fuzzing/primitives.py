...
class base_primitive(object):
    ...
    def __init__(self):
        self.fuzz_complete = (
            False  # this flag is raised when the mutations are exhausted.
        )
        self.fuzz_library = []  # library of static fuzz heuristics to cycle through.
        self.fuzzable = (
            True  # flag controlling whether or not the given primitive is to be fuzzed.
        )
    ...
    def __init__(self, name, values):
        ...
        if self.values != []:
            for val in self.values:
                assert isinstance(
                    val, str
                ), "Value list may only contain strings or raw data"
...
class bit_field(base_primitive):
    def __init__(
        ...
        if isinteger(value) or isinstance(value, (list, tuple)):
            ...
            raise ValueError(
                "The supplied value must be either an Int, Long, List or Tuple."
            )
    ...
    def to_binary(self, number=None, bit_count=None):
        ...
        return "".join(
            map(lambda x: str((number >> x) & 1), range(bit_count - 1, -1, -1))
        )
...
class byte(bit_field):
    def __init__(
        ...
        bit_field.__init__(
            self, value, 8, None, endian, format, signed, full_range, fuzzable, name
        )
...
class word(bit_field):
    def __init__(
        ...
        bit_field.__init__(
            self, value, 16, None, endian, format, signed, full_range, fuzzable, name
        )
...
class dword(bit_field):
    def __init__(
        ...
        bit_field.__init__(
            self, value, 32, None, endian, format, signed, full_range, fuzzable, name
        )
...
class qword(bit_field):
    def __init__(
        ...
        bit_field.__init__(
            self, value, 64, None, endian, format, signed, full_range, fuzzable, name
        )
