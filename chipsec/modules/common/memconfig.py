...
class memconfig(BaseModule):
    ...
    def check_memmap_locks(self):
        ...
        ia_untrusted = None
        if self.cs.is_register_defined("MSR_BIOS_DONE") and self.cs.register_has_field(
        ...
        for r in regs:
            if not self.cs.is_register_defined(r) or not self.cs.register_has_field(
                r, memmap_registers[r]
            ):
                self.logger.log_unknown(
                    "Skipping Validation: Register {} or field {} was not defined for this platform.".format(
                        r, memmap_registers[r]
                    )
                )
                continue
            ...
            if locked == 1:
                self.logger.log_good(
                    "{:20} = 0x{:16X} - LOCKED   - {}".format(r, v, d["desc"])
                )
            else:
                all_locked = False
                self.logger.log_bad(
                    "{:20} = 0x{:16X} - UNLOCKED - {}".format(r, v, d["desc"])
                )
