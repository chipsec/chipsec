...
class rtclock(BaseModule):
    ...
    def check_rtclock(self):
        ...
        if check_config_regs:
            ...
            self.logger.log_log_warning("Unable to test lock bits without attempting to modify CMOS.")
        ...
        if ll == 1 and ul == 1:
            ...
            self.logger.log_log_warning( "Protected locations in RTC memory are accessible (BIOS may not be using them)" )
