...
class variables(BaseModule):
    ...
    def can_modify(self, name, guid, data, attrs):
        ...
        if StatusCode.EFI_SUCCESS != status:
            self.logger.log(
                "    < modification of {} returned error 0x{:X}".format(name, status)
            )
        else:
            self.logger.log("    < modification of {} returned succees".format(name))

        self.logger.log(
            "    > checking variable {} contents after modification..".format(name)
        )
        ...
        if _changed:
            self.logger.log_bad(
                "EFI variable {} has been modified. Restoring original contents..".format(
                    name
                )
            )
            ...
            if restoreddata != data:
                self.logger.error(
                    "Failed to restore contents of variable {} failed!".format(name)
                )
            else:
                self.logger.log(
                    "    contents of variable {} have been restored".format(name)
                )
        else:
            self.logger.log_good(
                "Could not modify UEFI variable {}:{}".format(guid, name)
            )
        return _changed
    ...
    def check_secureboot_variable_attributes(self, do_modify):
            ...
            if name in sbvars.keys() and sbvars[name] is not None:
                if len(sbvars[name]) > 1:
                    self.logger.log_failed_check(
                        "There should only be one instance of variable {}".format(name)
                    )
                    return ModuleResult.FAILED
                for (off, buf, hdr, data, guid, attrs) in sbvars[name]:
                    self.logger.log(
                        "[*] Checking protections of UEFI variable {}:{}".format(
                            guid, name
                        )
                    )
                    ...
                    if EFI_VAR_NAME_SecureBoot == name:
                        is_secureboot_enabled = (
                            data is not None and len(data) == 1 and ord(data) == 0x1
                        )
                    ...
                    if name in SECURE_BOOT_KEY_VARIABLES:
                        if IS_VARIABLE_ATTRIBUTE(
                            ...
                            self.logger.log_good(
                                "Variable {}:{} is authenticated (AUTHENTICATED_WRITE_ACCESS)".format(
                                    guid, name
                                )
                            )
                            ...
                            self.logger.log_good(
                                "Variable {}:{} is authenticated (TIME_BASED_AUTHENTICATED_WRITE_ACCESS)".format(
                                    guid, name
                                )
                            )
                        else:
                            not_auth += 1
                            self.logger.log_bad(
                                "Variable {}:{} is not authenticated".format(guid, name)
                            )
        ...
        self.logger.log("")
        self.logger.log(
            "[*] Secure Boot appears to be {}abled".format(
