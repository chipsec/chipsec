...
class debugenabled(BaseModule):
    ...
    def is_supported(self):
        ...
        if not supported:
            ...
            self.logger.log_skipped_check(
                "CPU Debug features are not supported on this platform"
            )
        return supported
    ...
    def check_cpu_debug_enable(self):
        ...
        for tid in range(self.cs.msr.get_cpu_thread_count()):
            ...
            IA32_DEBUG_INTERFACE_DEBUGENABLE = (
                self.cs.get_register_field("IA32_DEBUG_INTERFACE", dbgiface, "ENABLE")
                == 1
            )
            IA32_DEBUG_INTERFACE_DEBUGELOCK = (
                self.cs.get_register_field("IA32_DEBUG_INTERFACE", dbgiface, "LOCK")
                == 1
            )
            IA32_DEBUG_INTERFACE_DEBUGEOCCURED = (
                self.cs.get_register_field(
                    "IA32_DEBUG_INTERFACE", dbgiface, "DEBUG_OCCURRED"
                )
                == 1
            )
            if self.logger.VERBOSE:
            ...
            if IA32_DEBUG_INTERFACE_DEBUGEOCCURED:
                self.logger.log_warning(
                    "Debug Occurred bit set in IA32_DEBUG_INTERFACE MSR"
                )
    ...
    def run(self, module_argv):
        ...
        self.logger.log("\n[*] Module Result")
        if (
            dci_test_fail == ModuleResult.FAILED
            or cpu_debug_test_fail == ModuleResult.FAILED
        ):
            self.logger.log_failed_check(
                "One or more of the debug checks have failed and a debug feature is enabled"
            )
            self.res = ModuleResult.FAILED
        elif (
            dci_test_fail == ModuleResult.WARNING
            or cpu_debug_test_fail == ModuleResult.WARNING
        ):
            self.logger.log_warn_check(
                "An unexpected debug state was discovered on this platform"
            )
