...
class remap(BaseModule):
    ...
    def check_remap_config(self):
        is_warning = False
        if (
            not self.cs.is_register_defined("PCI0.0.0_REMAPBASE")
            or not self.cs.is_register_defined("PCI0.0.0_REMAPLIMIT")
            or not self.cs.is_register_defined("PCI0.0.0_TOUUD")
            or not self.cs.is_register_defined("PCI0.0.0_TOLUD")
            or not self.cs.is_register_defined("PCI0.0.0_TSEGMB")
        ):
            self.logger.error(
                "Couldn't find definition of required registers (REMAP*, TOLUD, TOUUD, TSEGMB)"
            )
        ...
        self.logger.log(
            "[*]   Remap Limit Address: 0x{:016X}".format(remaplimit | 0xFFFFF)
        )
        ...
        ok = (
            (0 == tolud & ALIGNED_1MB)
            and (0 == touud & ALIGNED_1MB)
            and (0 == remapbase & ALIGNED_1MB)
            and (0 == remaplimit & ALIGNED_1MB)
        )
        remap_ok = remap_ok and ok
        if ok:
        ...
        if remap_ok:
            if is_warning:
                self.logger.log_warning(
                    "Most Memory Remap registers are configured correctly and locked"
                )
                self.logger.log(
                    "[!] Manual verification of REMAP BASE and LIMIT register values may be needed."
                )
