...
class access_uefispec(BaseModule):
    ...
    def can_modify(self, name, guid, data):
        ...
        if self.diff_var(newdata, origdata):
            self.logger.log_bad(
                "Corruption of EFI variable of concern {}. Trying to recover.".format(
                    name
                )
            )
            ret = True
            ...
            if self.diff_var(self._uefi.get_EFI_variable(name, guid), origdata):
                nameguid = name + " (" + guid + ")"
                self.logger.log_bad(
                    "RECOVERY FAILED. Variable {} remains corrupted. Original data value: {}".format(
                        nameguid, origdata
                    )
                )
        return ret
    ...
    def check_vars(self, do_modify):
        ...
        for name in vars.keys():
            ...
            for (off, buf, hdr, data, guid, attrs) in vars[name]:
                self.logger.log(
                    "[*] Variable {} ({}) Guid {} Size {} ".format(
                        name, get_attr_string(attrs), guid, hex(len(data))
                    )
                )
                ...
                if perms is not None:
                    if perms != attrs:
                        ...
                        if extra_attr != 0:
                            self.logger.log_important(
                                "  Extra attributes:" + get_attr_string(extra_attr)
                            )
                            if (
                                extra_attr
                                & ~(
                        ...
                        if missing_attr != 0:
                            self.logger.log_important(
                                "  Missing attributes:" + get_attr_string(missing_attr)
                            )
                ...
                if do_modify:
                    ...
                    if name in self.uefispec_ro_vars:
                        if self.can_modify(name, guid, data):
                            ...
                            self.logger.log_bad(
                                "Variable {} should be read only.".format(name)
                            )
                            res = ModuleResult.FAILED
                    else:
                        if self.can_modify(name, guid, data):
