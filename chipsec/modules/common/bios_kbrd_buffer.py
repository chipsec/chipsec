...
class bios_kbrd_buffer(BaseModule):
    ...
    def check_BIOS_keyboard_buffer(self):
        ...
        self.logger.log(
            "[*] Keyboard buffer head pointer = 0x{:X} (at 0x41A), tail pointer = 0x{:X} (at 0x41C)".format(
                kbrd_buf_head, kbrd_buf_tail
            )
        )
        ...
        if COMMON_FILL_PTRN == bios_kbrd_buf:
            self.logger.log_passed_check(
                "Keyboard buffer is filled with common fill pattern"
            )
        ...
        if (0x1E < kbrd_buf_tail) and (kbrd_buf_tail <= 0x1E + 32):
            # has_contents = True
            self.logger.log_bad(
                "Keyboard buffer tail points inside the buffer (= 0x{:X})".format(
                    kbrd_buf_tail
                )
            )
            self.logger.log(
                "    It may potentially expose lengths of pre-boot passwords. Was your password {:d} characters long?".format(
                    (kbrd_buf_tail + 2 - 0x1E) // 2
                )
            )
        ...
        if has_contents:
            ...
            self.logger.log_passed_check(
                "Keyboard buffer looks empty. Pre-boot passwords don't seem to be exposed"
            )
