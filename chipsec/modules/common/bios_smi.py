...
class bios_smi(BaseModule):
    ...
    def is_supported(self):
        if (
            not self.cs.is_control_defined("SmmBiosWriteProtection")
            or not self.cs.is_control_defined("TCOSMILock")
            or not self.cs.is_control_defined("SMILock")
            or not self.cs.is_control_defined("BiosWriteEnable")
        ):
    ...
    def check_SMI_locks(self):
        ...
        #
        if self.cs.is_control_defined("TCOSMIEnable") and self.cs.is_control_defined(
            "GlobalSMIEnable"
        ):
            ...
            if gbl_smi_en != 1:
                ...
                ok = False
                self.logger.log_bad(
                    "TCO SMI should be enabled if using SMM BIOS region protection"
                )
            else:
        ...
        if ok and not warn:
            ...
            self.logger.log_warning(
                "One or more warnings detected when checking SMI enable state"
            )
        else:
