...
from chipsec.module_common import BaseModule, CPU, HWCONFIG, SMM
...
TAGS = [CPU, HWCONFIG, SMM]
METADATA_TAGS = ['OPENSOURCE', 'IA', 'COMMON', 'CPU', 'SPECTRE_V2']
...
class spectre_v2(BaseModule):
    ...
    def is_supported(self) -> bool:
        if self.cs.is_intel():
            ...
            if self.cs.register.is_defined('SPEC_CTRL'):
                return True
        else:
            self.logger.log_important('SPEC_CTRL register not defined for platform.  Skipping module.')
        return False
