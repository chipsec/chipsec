...
class memlock(BaseModule):
    ...
    def check_MSR_LT_LOCK_MEMORY(self):
        ...
        for tid in range(self.cs.msr.get_cpu_thread_count()):
            ...
            lt_lock = self.cs.get_register_field(
                "MSR_LT_LOCK_MEMORY", lt_lock_msr, "LT_LOCK"
            )
            self.logger.log(
                "[*]   cpu{:d}: MSR_LT_LOCK_MEMORY[LT_LOCK] = {:x}".format(tid, lt_lock)
            )
            if 0 == lt_lock:
                status = True
        return status
    ...
    def run(self, module_argv):
        ...
        if check_MSR_LT_LOCK_MEMORY_test_fail == True:
            self.logger.log_failed(
                "Check failed. MSR_LT_LOCK_MEMORY isn't configured correctly"
            )
