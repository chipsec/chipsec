...
class sgx_check(BaseModule):
            ...
            for tid in range(self.cs.msr.get_cpu_thread_count()):
                ...
                if status == -1:
                    self.logger.verbose_log(
                        "[*] Failed to set affinity to CPU{:d}".format(tid)
                    )
                ...
                if r_ebx & BIT2:
                    ...
                    self.logger.verbose_log(
                        "[*]CPU{:d}: does not support SGX".format(tid)
                    )
    ...
    def check_sgx_config(self):
        ...
        for tid in range(self.cs.msr.get_cpu_thread_count()):
            ...
            self.logger.verbose_log(
                "[*] cpu{:d}: IA32_Feature_Control Lock = {:d}".format(
                    tid, feature_cntl_lock
                )
            )
        ...
        for tid in range(self.cs.msr.get_cpu_thread_count()):
            ...
            if 0 == mtrrcap:
                self.logger.verbose_log(
                    "[*] CPU{:d} Protected Memory Range configuration is not supported".format(
                        tid
                    )
                )
            else:
                prmrr_enable = True
                self.logger.verbose_log(
                    "[*] CPU{:d} Protected Memory Range configuration is supported".format(
                        tid
                    )
                )
        if prmrr_enable:
        ...
        prmrr_uniform = True
        prmrr_locked = True
        check_uncore_vals = self.cs.is_register_defined(
            "PRMRR_UNCORE_PHYBASE"
        ) and self.cs.is_register_defined("PRMRR_UNCORE_MASK")
        for tid in range(self.cs.msr.get_cpu_thread_count()):
            prmrr_valid_config_new = self.cs.read_register("PRMRR_VALID_CONFIG", tid)
            prmrr_base_new = self.cs.read_register_field(
                "PRMRR_PHYBASE", "PRMRR_base_address_fields", False, tid
            )
            prmrr_base_memtype_new = self.cs.read_register_field(
                "PRMRR_PHYBASE", "PRMRR_MEMTYPE", False, tid
            )
            prmrr_mask_new = self.cs.read_register_field(
                "PRMRR_MASK", "PRMRR_mask_bits", False, tid
            )
            prmrr_mask_vld_new = self.cs.read_register_field(
                "PRMRR_MASK", "PRMRR_VLD", False, tid
            )
            prmrr_mask_lock_new = self.cs.read_register_field(
                "PRMRR_MASK", "PRMRR_LOCK", False, tid
            )
            if check_uncore_vals:
                prmrr_uncore_base_new = self.cs.read_register_field(
                    "PRMRR_UNCORE_PHYBASE", "PRMRR_base_address_fields", False, tid
                )
                prmrr_uncore_mask_new = self.cs.read_register_field(
                    "PRMRR_UNCORE_MASK", "PRMRR_mask_bits", False, tid
                )
                prmrr_uncore_mask_vld_new = self.cs.read_register_field(
                    "PRMRR_UNCORE_MASK", "PRMRR_VLD", False, tid
                )
                prmrr_uncore_mask_lock_new = self.cs.read_register_field(
                    "PRMRR_UNCORE_MASK", "PRMRR_LOCK", False, tid
                )
            if self.logger.VERBOSE:
                self.logger.log(
                    "[*]      CPU{:d} PRMRR_VALID_CONFIG: 0x{:010X}".format(
                        tid, prmrr_valid_config_new
                    )
                )
                self.logger.log(
                    "[*]      CPU{:d} PRMRR base address: 0x{:012X}".format(
                        tid, prmrr_base_new
                    )
                )
                self.logger.log(
                    "[*]      CPU{:d} PRMRR memory type: 0x{:d}".format(
                        tid, prmrr_base_memtype_new
                    )
                )
                self.logger.log(
                    "[*]      CPU{:d} PRMRR mask address: 0x{:012X}".format(
                        tid, prmrr_mask_new
                    )
                )
                self.logger.log(
                    "[*]      CPU{:d} PRMRR mask valid: 0x{:d}".format(
                        tid, prmrr_mask_vld_new
                    )
                )
                self.logger.log(
                    "[*]      CPU{:d} PRMRR mask lock: 0x{:d}".format(
                        tid, prmrr_mask_lock_new
                    )
                )
                if check_uncore_vals:
                    self.logger.log(
                        "[*]      CPU{:d} PRMRR uncore base address: 0x{:012X}".format(
                            tid, prmrr_uncore_base_new
                        )
                    )
                    self.logger.log(
                        "[*]      CPU{:d} PRMRR uncore mask address: 0x{:012X}".format(
                            tid, prmrr_uncore_mask_new
                        )
                    )
                    self.logger.log(
                        "[*]      CPU{:d} PRMRR uncore mask valid: 0x{:d}".format(
                            tid, prmrr_uncore_mask_vld_new
                        )
                    )
                    self.logger.log(
                        "[*]      CPU{:d} PRMRR uncore mask lock: 0x{:d}".format(
                            tid, prmrr_uncore_mask_lock_new
                        )
                    )
            if first_iter:
            ...
            if (
                (prmrr_valid_config != prmrr_valid_config_new)
                or (prmrr_base != prmrr_base_new)
                or (prmrr_mask != prmrr_mask_new)
                or (prmrr_uncore_base != prmrr_uncore_base_new)
                or (prmrr_uncore_mask != prmrr_uncore_mask_new)
                or (prmrr_mask_vld != prmrr_mask_vld_new)
                or (prmrr_mask_lock != prmrr_mask_lock_new)
                or (prmrr_uncore_mask_vld != prmrr_uncore_mask_vld_new)
                or (prmrr_uncore_mask_lock != prmrr_uncore_mask_lock_new)
                or (prmrr_base_memtype != prmrr_base_memtype_new)
            ):
                prmrr_uniform = False
        if not prmrr_uniform:
            ...
            if config_support:
                self.logger.log(
                    "[*]  PRMRR config supports: {}".format(", ".join(prmrr_configs))
                )
            else:
            ...
            if prmrr_base == 0 and prmrr_mask == 0:
                ...
                self.logger.log(
                    "[*]  PRMRR memory type : 0x{:X}".format(prmrr_base_memtype)
                )
                ...
                self.logger.log(
                    "[*]      PRMRR uncore mask valid: 0x{:d}".format(
                        prmrr_uncore_mask_vld
                    )
                )
                ...
                self.logger.log(
                    "[*]      PRMRR mask lock: 0x{:X}".format(prmrr_mask_lock)
                )
                if prmrr_locked:
                ...
                if check_uncore_vals:
                    self.logger.log(
                        "[*]  PRMRR uncore base address: 0x{:012X}".format(
                            prmrr_uncore_base
                        )
                    )
                    self.logger.log(
                        "[*]  PRMRR uncore mask address: 0x{:012X}".format(
                            prmrr_uncore_mask
                        )
                    )
                    self.logger.log("[*]  Verifying PRMR uncore address are valid")
                    self.logger.log(
                        "[*]      PRMRR uncore mask valid: 0x{:X}".format(
                            prmrr_uncore_mask_vld
                        )
                    )
                    if prmrr_uncore_mask_vld == 0x1:
                        self.logger.log_good(
                            "Mcheck marked uncore PRMRR address as valid"
                        )
                    else:
                        self.logger.log_bad(
                            "Mcheck marked uncore PRMRR address as invalid"
                        )
                        self.res = ModuleResult.FAILED
                    self.logger.log(
                        "[*]  Verifying if PRMR uncore mask register is locked"
                    )
                    self.logger.log(
                        "[*]      PRMRR uncore mask lock: 0x{:X}".format(
                            prmrr_uncore_mask_lock
                        )
                    )
        ...
        if bios_feature_control_enable and locked:
            ...
            for tid in range(self.cs.msr.get_cpu_thread_count()):
                ...
                if r_eax & BIT0:
                    self.logger.verbose_log(
                        "[*] CPU{:d} SGX-1 instructions are supported".format(tid)
                    )
                    sgx1_instr_support = True
                else:
                    self.logger.verbose_log(
                        "[*] CPU{:d} SGX-1 instructions are not supported".format(tid)
                    )
                if r_eax & BIT1:
                    self.logger.verbose_log(
                        "[*] CPU{:d} SGX-2 instructions are supported".format(tid)
                    )
                    sgx2_instr_support = True
                else:
                    self.logger.verbose_log(
                        "[*] CPU{:d} SGX-2 instructions are not supported".format(tid)
                    )
            if sgx1_instr_support:
                self.logger.log_good(
                    "Intel SGX instructions are supported and available to use"
                )
                sgx_ok = True
            else:
                self.logger.log_bad(
                    "Intel SGX instructions are not supported on system"
                )
                sgx_ok = False
        ...
        if sgx_ok and prmrr_enable and prmrr_uniform:
            ...
            (not sgx_ok)
            and (not bios_feature_control_enable)
            and prmrr_enable
            and prmrr_uniform
        ):
        ...
        if self.cs.is_register_defined("BIOS_SE_SVN") and self.cs.is_register_defined(
            "BIOS_SE_SVN_STATUS"
        ):
            self.logger.log(
                "\n[*] BIOS_SE_SVN : 0x{:016X}".format(
                    self.cs.read_register("BIOS_SE_SVN")
                )
            )
            self.logger.log(
                "[*]     PFAT_SE_SVN : 0x{:02X}".format(
                    self.cs.read_register_field("BIOS_SE_SVN", "PFAT_SE_SVN")
                )
            )
            self.logger.log(
                "[*]     ANC_SE_SVN : 0x{:02X}".format(
                    self.cs.read_register_field("BIOS_SE_SVN", "ANC_SE_SVN")
                )
            )
            self.logger.log(
                "[*]     SCLEAN_SE_SVN : 0x{:02X}".format(
                    self.cs.read_register_field("BIOS_SE_SVN", "SCLEAN_SE_SVN")
                )
            )
            self.logger.log(
                "[*]     SINIT_SE_SVN : 0x{:02X}".format(
                    self.cs.read_register_field("BIOS_SE_SVN", "SINIT_SE_SVN")
                )
            )
            self.logger.log(
                "[*] BIOS_SE_SVN_STATUS : 0x{:016X}".format(
                    self.cs.read_register("BIOS_SE_SVN_STATUS")
                )
            )
            self.logger.log(
                "[*]     BIOS_SE_SVN ACM threshold lock : 0x{:d}".format(
                    self.cs.read_register_field("BIOS_SE_SVN_STATUS", "LOCK")
                )
            )

        self.logger.log("\n[*] Check SGX debug feature settings")
        sgx_debug_status = self.cs.read_register_field(
            "SGX_DEBUG_MODE", "SGX_DEBUG_MODE_STATUS_BIT"
        )
        self.logger.log(
            "[*] SGX Debug Enable             : {:d}".format(sgx_debug_status)
        )
        ...
        debug_enable = self.cs.get_register_field(
            "IA32_DEBUG_INTERFACE", debug_interface, "ENABLE"
        )
        debug_lock = self.cs.get_register_field(
            "IA32_DEBUG_INTERFACE", debug_interface, "LOCK"
        )
        ...
        if (0 == debug_enable) and (1 == sgx_debug_status):
            self.logger.log_bad(
                "Enabling sgx_debug without enabling debug mode in msr IA32_DEBUG_INTERFACE is not a valid configuration"
            )
            self.res = ModuleResult.FAILED
        if 1 == debug_lock:
