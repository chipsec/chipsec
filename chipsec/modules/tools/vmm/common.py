...
class BaseModuleSupport(BaseModuleDebug):
    ...
    def get_initial_data(self, statuses, vector, size, padding="\x00"):
        connectionid_message = [
            (" ".join(["{:02x}".format(ord(x)) for x in DD(k)]))
            for k, v in self.hv_connectionid.items()
            if v == 1
        ]
        connectionid_event = [
            (" ".join(["{:02x}".format(ord(x)) for x in DD(k)]))
        ...
        for status in statuses:
            for item in self.initial_data:
                if (int(item["vector"], 16) == vector) and (item["status"] == status):
                    data = item["data"]
                    data = data.replace(
                        "CONNECTION_ID_MESSAGE_TYPE",
                        random.choice(connectionid_message),
                    )
                    data = data.replace(
                        "CONNECTION_ID_EVENT_TYPE", random.choice(connectionid_event)
                    )
    ...
    def add_initial_data(self, vector, buffer, status):
        ...
        if not found:
            self.initial_data.append(
                {"vector": "{:02X}".format(vector), "status": status, "data": buffer}
            )
        return
...
class BaseModuleHwAccess(BaseModuleSupport):
    ...
    def cpuid_info(self, eax, ecx, desc):
        ...
        self.msg(
            "EAX: 0x{:08X} EBX: 0x{:08X} ECX: 0x{:08X} EDX: 0x{:08X}".format(
