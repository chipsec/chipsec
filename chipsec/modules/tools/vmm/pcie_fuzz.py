...
class pcie_fuzz(BaseModule):
    ...
    def fuzz_mmio_bar(self, bar, is64bit, size=0x1000):
        self.logger.log(
            "[*] Fuzzing MMIO BAR 0x{:016X}, size = 0x{:X}..".format(bar, size)
        )
        reg_off = 0
    ...
    def fuzz_mmio_bar_random(self, bar, is64bit, size=0x1000):
        self.logger.log(
            "[*] Fuzzing MMIO BAR in random mode 0x{:016X}, size = 0x{:X}..".format(
                bar, size
            )
        )
        reg_off = 0
    ...
    def fuzz_mmio_bar_in_active_range(self, bar, is64bit, list):
        self.logger.log(
            "[*] Fuzzing MMIO BAR in Active range 0x{:016X}, size of range = 0x{:X}..".format(
                bar, len(list)
            )
        )
        for reg_off in list:
    ...
    def fuzz_mmio_bar_in_active_range_random(self, bar, is64bit, list):
        self.logger.log(
            "[*] Fuzzing MMIO BAR in Active range 0x{:016X} in random mode, size of range = 0x{:X}..".format(
                bar, len(list)
            )
        )
        reg_off = 0
    ...
    def fuzz_mmio_bar_in_active_range_bit_flip(self, bar, is64bit, list):
        self.logger.log(
            "[*] Fuzzing (bit flipping) MMIO BAR in Active range 0x{:016X}, size of range = 0x{:X}..".format(
                bar, len(list)
            )
        )
        reg_off = 0
    ...
    def find_active_range(self, bar, size):
        self.logger.log(
            "[*] Determine MMIO BAR Active range 0x{:016X}, size  0x{:X}..".format(
                bar, size
            )
        )
        ...
        for i in range(len(one) // 4 - 1):
            if (
                one[4 * i] != two[4 * i]
                or one[4 * i + 1] != two[4 * i + 1]
                or one[4 * i + 2] != two[4 * i + 2]
                or one[4 * i + 3] != two[4 * i + 3]
            ):
    ...
    def fuzz_pcie_device(self, b, d, f):
        ...
        for (bar, isMMIO, is64bit, bar_off, bar_reg, size) in device_bars:
            if bar not in _EXCLUDE_BAR:
                ...
                if isMMIO:
                    self.logger.log(
                        "[*] + 0x{:02X} ({:X}): MMIO BAR at 0x{:016X} (64-bit? {:d}) with size: 0x{:08X}. Fuzzing..".format(
                            bar_off, bar_reg, bar, is64bit, size
                        )
                    )
                    if ACTIVE_RANGE and size > 0x1000:
                        ...
                        if len(list) > 0:
                            if BIT_FLIP:
                                self.fuzz_mmio_bar_in_active_range_bit_flip(
                                    bar, is64bit, list
                                )
                            else:
                ...
                # Fuzzing I/O registers of the PCIe device
                    ...
                    if IO_FUZZ:
                        self.logger.log(
                            "[*] + 0x{:02X}: I/O BAR at 0x{:08X}. Fuzzing..".format(
                                bar_off, bar
                            )
                        )
