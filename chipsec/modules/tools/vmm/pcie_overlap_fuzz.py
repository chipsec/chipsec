...
class pcie_overlap_fuzz(BaseModule):
                ...
                if bar1 not in _EXCLUDE_MMIO_BAR1:
                    if isMMIO1:
                        for (b2, d2, f2, vid2, did2) in pcie_devices:
                            device_bars2 = self.cs.pci.get_device_bars(
                                b2, d2, f2, bCalcSize=True
                            )
                            for (
                                bar2,
                                isMMIO2,
                                ...
                                if bar2 not in _EXCLUDE_MMIO_BAR2:
                                    if isMMIO2:
                                        if bar1 != bar2:
                                            self.logger.log(
                                                "[*] overlap device {:02X}:{:02X}.{:X} offset {:X} bar: {:08X} and {:02X}:{:02X}.{:X} offset {:X} bar: {:08X}".format(
                                                    b1,
                                                    d1,
                                                    f1,
                                                    bar_off1,
                                            ...
                                            if FUZZ_OVERLAP:
                                                ...
                                                self.logger.log(
                                                    "[*] fuzzing MMIO BAR 0x{:X}...".format(
                                                        _bar
                                                    )
                                                )
                                                if FUZZ_RANDOM:
                                                    self.fuzz_mmio_bar_random(
                                                        _bar, _is64bit, _size
                                                    )
                                                else:
                                                    self.fuzz_mmio_bar(
    ...
    def run(self, module_argv):
        self.logger.start_test(
            "Tool to overlap and fuzz MMIO spaces of available PCIe devices"
        )
