...
class HyperVHypercall(BaseModuleHwAccess):
    ...
    def print_hypervisor_info(self):
        ...
        self.msg(
            "ECX(31) - Hypervisor Present                  :  {:x}".format(
        ...
        if self.hypervisor_present:
            ...
            gprs = self.cpuid_info(
                0x40000000, 0x0, "Hypervisor CPUID leaf range and vendor ID signature"
            )
            (
                max_input_value,
                id_signature_ebx,
            ...
            id_signature = pack(
                "<3L", id_signature_ebx, id_signature_ecx, id_signature_edx
            )
            self.msg(
                "The maximum input value for hypervisor CPUID  :  {:08X}".format(
                    max_input_value
                )
            )
            self.msg(
                "Hypervisor Vendor ID Signature                :  {}".format(
                    id_signature
                )
            )

            gprs = self.cpuid_info(
                0x40000001, 0x0, "Hypervisor vendor-neutral interface identification"
            )
            (interface_signature, rsvd_ebx, rsvd_ecx, rsvd_edx) = gprs
            interface_signature = pack("<1L", interface_signature)
            self.msg(
                "Hypervisor Interface Signature                :  {}".format(
                    interface_signature
                )
            )
            ...
            if interface_signature == "Hv#1":
                ...
                if (max_input_value < 0x40000005) or (max_input_value > 0x400000FF):
                    self.msg("")
                    self.msg(
                        "*** WARNING ***: Invalid CPUID.0x40000000.0x0.EAX value\n\n"
                    )
    ...
    def print_hypervisor_cpuid(self, cpuid_eax, cpuid_ecx=0x0):
        if cpuid_eax == 0x40000002:
            (eax, ebx, ecx, edx) = self.cpuid_info(
                cpuid_eax, cpuid_ecx, "Hypervisor system identity"
            )
            ...
            (eax, ebx, ecx, edx) = self.cpuid_info(
                cpuid_eax, cpuid_ecx, "Feature identification"
            )
            self.msg(
                "   EAX - features available to the partition                :  {:08X}".format(
                    eax
                )
            )
            self.info_bitwise(eax, cpuid_desc[cpuid_eax]["EAX"])
            self.msg(
                "   EBX - flags specified at partition creation              :  {:08X}".format(
                    ebx
                )
            )
            self.info_bitwise(ebx, cpuid_desc[cpuid_eax]["EBX"])
            self.msg(
                "   ECX - power management related information               :  {:08X}".format(
                    ecx
                )
            )
            self.info_bitwise(ecx, cpuid_desc[cpuid_eax]["ECX"])
            self.msg(
                "   EDX - misc. features available to the partition          :  {:08X}".format(
                    edx
                )
            )
            ...
            (eax, ebx, ecx, edx) = self.cpuid_info(
                cpuid_eax, cpuid_ecx, "Implementation recommendations"
            )
            self.msg(
                "   EAX(9-0) - recommendations for optimal performance       :  {:08X}".format(
                    eax
                )
            )
            self.msg(
                "   EBX      - recommended number of attempts                :  {:08X}".format(
                    ebx
                )
            )
        elif cpuid_eax == 0x40000005:
            (eax, ebx, ecx, edx) = self.cpuid_info(
                cpuid_eax, cpuid_ecx, "Implementation limits"
            )
            self.msg(
                "   EAX - The maximum number of virtual processors supported :  {:08X}".format(
                    eax
                )
            )
            self.msg(
                "   EBX - The maximum number of logical processors supported :  {:08X}".format(
                    ebx
                )
            )
            self.msg(
                "   ECX - The maximum number of physical interrupt vectors   :  {:08X}".format(
                    ecx
                )
            )
        elif cpuid_eax == 0x40000006:
            (eax, ebx, ecx, edx) = self.cpuid_info(
                cpuid_eax, cpuid_ecx, "Implementation hardware features"
            )
                ...
                try:
                    ...
                    if result == HV_STATUS_INVALID_HYPERCALL_INPUT:
                        ...
                        if result == HV_STATUS_INVALID_HYPERCALL_INPUT:
                            ...
                            result = (
                                self.hv.hypercall64_extended_fast(hciv, buffer) & 0xFFFF
                            )
                            hv_rep = 1
                            if result == HV_STATUS_INVALID_HYPERCALL_INPUT:
                                ...
                                result = (
                                    self.hv.hypercall64_memory_based(hciv, buffer)
                                    & 0xFFFF
                                )
                                hv_fast = 0
                            else:
                    ...
                    self.msg(
                        "Exception on hypercall (0x{:08X}): {}".format(
                            call_code, str(e)
                        )
                    )
        return
        ...
        for i in id_list:
            ...
            if result != HV_STATUS_INVALID_CONNECTION_ID:
                if result == HV_STATUS_SUCCESS:
                    ...
                    result = self.hv.hypercall64_memory_based(hciv, buffer) & 0xFFFF
                    self.hv_connectionid[i] = (
                        0x2 if result == HV_STATUS_SUCCESS else 0x3
                    )
        return
    ...
    def scan_for_success_status(self, i, total_tests):
        ...
        pattern = ""
        hc = (
            self.hv_hypercalls[i]
            if i in self.hv_hypercalls
            else [0, 0, HV_STATUS_INVALID_HYPERCALL_CODE]
        )
        if hc[2] != HV_STATUS_SUCCESS:
            for x in range(total_tests):
                ...
                statistics[result] = (
                    1 if result not in statistics else statistics[result] + 1
                )
                if result == HV_STATUS_SUCCESS:
                    pattern = buffer
                    break
    ...
    def print_input_parameters(self, i, maxlen, status_list):
        ...
        for l in range(maxlen):
            ...
            ranges = []
            for v in range(0x100):
                if (matrix[l][v] not in status_list) and (
                    matrix[l][v + 1] in status_list
                ):
                    x = v + 1
                if (matrix[l][v] in status_list) and (
                    matrix[l][v + 1] not in status_list
                ):
                    if x == v:
    ...
    def print_hypercall_status(self):
        ...
        for i in sorted(self.hv_hypercalls.keys()):
            ...
            if status not in status_list:
                self.msg(
                    "HYPERV_HYPERCALL REP:{:d} FAST:{:d} {:04X}  {:02X}  {:40} '{}'".format(
                        hc[0], hc[1], i, hc[2], status, hcname
                    )
                )
        return
        ...
        for partid in sorted(self.hv_partitionid.keys()):
            for n in range(0x10):
                for m in range(0x10):
                    ...
                    if result == HV_STATUS_SUCCESS:
                        self.msg(
                            "  Partition: {:08X}  Property: {:08X}  Value: {:016x}".format(
                                partid, prop, unpack("<Q", self.hv.output)[0]
                            )
                        )
        return
    ...
    def set_partition_property(self, part, prop, value):
        ...
        self.msg(
            ">>> Setting partition property:  Partition: {:08X}  Property: {:08X}  Value: {:016x}  Status: {}".format(
                part, prop, value, status
            )
        )
        return
            ...
            for connid in sorted(self.hv_connectionid.keys()):
                if self.hv_connectionid[connid] == HV_PORT_TYPE_MESSAGE:
                    ...
                    for i in range(0x100, 0x1000):
                        ...
                        buffer = pack(
                            "<4LQ", connid, 0, messagetype, payloadsize, message0
                        )
                        ...
                        self.dbg(
                            "HvPostMessage: {} {}".format(
                                get_hypercall_status(result), binascii.hexlify(buffer)
                            )
                        )
