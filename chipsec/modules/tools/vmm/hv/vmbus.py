...
class HyperV(BaseModuleDebug):
    ...
    def __del__(self):
        ...
        if len(self.old_sint2) == 2:
            self.cs.msr.write_msr(
                0, HV_X64_MSR_SINT2, self.old_sint2[0], self.old_sint2[1]
            )
        if len(self.old_simp) == 2:
            self.cs.msr.write_msr(
                0, HV_X64_MSR_SIMP, self.old_simp[0], self.old_simp[1]
            )
        if len(self.old_siefp) == 2:
            self.cs.msr.write_msr(
                0, HV_X64_MSR_SIEFP, self.old_siefp[0], self.old_siefp[1]
            )
    ...
    def hv_post_msg(self, message):
        ...
        while retries > 0:
            result = (
                self.hypercall.hypercall64_memory_based(hciv, message[:0x100]) & 0xFFFF
            )
    ...
    def hv_recv_msg(self, sint):
        ...
        if message_type == HVMSG_NONE:
            ...
            self.cs.mem.write_physical_mem(
                self.simp + 0x100 * sint, 0x4, DD(HVMSG_NONE)
            )
...
class VMBus(HyperV):
    ...
    def vmbus_post_msg(self, message):
        if len(message) > 240:
            self.err(
                "vmbus_post_msg: message it too long {:d} bytes".format(len(message))
            )
            message = message[:240]
        ...
        if result != HV_STATUS_SUCCESS:
            status = (
                hypercall_status_codes[result]
                if result in hypercall_status_codes
                else ""
            )
    ...
    def vmbus_recv_msg(self, timeout=0):
        ...
        if message:
            msg_type, payload_size, msg_flags, rsvd, port_id = unpack(
                "<LBBHQ", message[0:16]
            )
            if msg_type not in [HVMSG_NONE, 0x0001]:
                status = (
                    hv_message_type[msg_type] if msg_type in hv_message_type else ""
                )
                self.fatal(
                    "vmbus_recv_msg: unhandeled message type 0x{:08X} {}".format(
                        msg_type, status
                    )
                )
            if (payload_size < 8) or (payload_size > 240):
                self.msg(
                    "vmbus_recv_msg: invalid payload size {:d}".format(payload_size)
                )
                payload_size = 240
            if rsvd != 0x0000:
    ...
    def vmbus_setevent(self, child_relid):
        ...
        if child_relid not in self.open_channels:
            self.err(
                "vmbus_setevent could not find channel with child relid: {:d}".format(
                    child_relid
                )
            )
            return False
        ...
        if channel["monitor_allocated"] == 1:
            ...
            self.cs.mem.set_mem_bit(
                self.monitor_page2 + trigger_group_offset, monitor_bit
            )
        else:
    ...
    def vmbus_get_next_version(self, current_version):
        versions = {
            VERSION_WIN8_1: VERSION_WIN8,
            VERSION_WIN8: VERSION_WIN7,
            VERSION_WS2008: VERSION_INVAL,
        }
        return (
            versions[current_version] if current_version in versions else VERSION_INVAL
        )
    ...
    def vmbus_establish_gpadl(self, child_relid, gpadl, pfn):
        ...
        channel_gpadl_header = pack(
            "<LLHH", child_relid, gpadl, range_buflen, rangecount
        )
        result = self.vmbus_post_msg(
            channel_message_header + channel_gpadl_header + gpa_range[: 27 * 8]
        )
        ...
        while result and gpa_range != "":
            ...
            result = self.vmbus_post_msg(
                channel_message_header + channel_gpadl_body + gpa_range[: 28 * 8]
            )
    ...
    def vmbus_teardown_gpadl(self, child_relid, gpadl):
        ...
        if result:
            ...
            if msgtype == CHANNELMSG_RESCIND_CHANNELOFFER:
                ...
                result = result and (
                    self.vmbus_onmessage() == CHANNELMSG_GPADL_TORNDOWN
                )
    ...
    def vmbus_open(self, child_relid, gpadl, pageoffset=2, userdata="\x00" * 120):
        ...
        channel_open_channel = pack(
            "<5L", child_relid, openid, gpadl, target_vp, pageoffset
        )
        result = self.vmbus_post_msg(
            channel_message_header + channel_open_channel + userdata
        )
        if result:
    ...
    def vmbus_onmessage(self):
        ...
        if len(message) == 0:
            ...
            self.dbg(
                "vmbus_onmessage: message {:d} {}".format(
                    msgtype, vmbus_channel_message_type[msgtype]
                )
            )
...
class VMBusDiscovery(VMBus):
    ...
    def print_supported_versions(self):
        ...
        for version in sorted(self.supported_versions.keys()):
            status = (
                "Unknown" if version not in vmbus_versions else vmbus_versions[version]
            )
            self.msg(
                "  {:d} . {:2d} - {}".format(version >> 16, version & 0xFFFF, status)
            )
        return
    ...
    def print_offer_channels(self):
        ...
        uuid_sorted_by_connid = dict(
            (value["connection_id"], key)
            for (key, value) in self.offer_channels.items()
        ).values()
        for i in uuid_sorted_by_connid:
            ...
            monid = "Monitor: {:d} ID=0x{:02X}".format(
                channel["monitor_allocated"], channel["monitor_id"]
            )
            ...
            self.msg(
                "  Hardware IDs:  {}   {}".format(
                    uuid(i[0x00:0x10]), uuid(i[0x10:0x20])
                )
            )
        ...
        for gpadl in sorted(self.created_gpadl.keys()):
            ...
            self.msg(
                "  0x{:08X}   |  0x{:08X}   |  0x{:08X}".format(
                    gpadl, channel["child_relid"], channel["status"]
                )
            )
        self.msg("---------------------------------------------------")
        return
    ...
    def print_open_channels(self):
        ...
        for child_relid in sorted(self.open_channels):
            ...
            self.msg(
                "  0x{:08X}   |  0x{:08X}   |  0x{:08X}".format(
                    child_relid, channel["openid"], channel["status"]
                )
            )
        self.msg("---------------------------------------------------")
        return
