...
class HypercallFuzz(BaseModule):
    ...
    def run(self, module_argv):
        ...
        testnum = (
            get_int_arg(module_argv[2])
            if len(module_argv) > 2 and module_argv[2] != ""
            else 10000000
        )
        ...
        if hv.hypervisor_present:
            ...
            hypercalls_for_scanning = list(
                set(range(0x100)) - set(excluded_hypercalls_from_scan)
            )
            hv.scan_hypercalls(hypercalls_for_scanning)

        if callnum == "all":
            hypercalls = list(
                set(hv.hv_hypercalls.keys()) - set(excluded_hypercalls_from_fuzzing)
            )
        else:
            hypercalls = [callnum]
