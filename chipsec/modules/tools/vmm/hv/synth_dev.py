    ...
    def send_1(self, relid, messages, info, order):
        if len(messages) > 0:
            ...
            if msg_recv != "":
                ...
                info[msg1]["next"] = self.send_1(
                    relid, messages, info[msg1]["next"], order
                )
        return info
    ...
    def device_fuzzing(self, relid):
        for x in range(1, 0x100):
            for a in range(0, 0x100):
                ...
                self.vmbus_open(
                    relid,
                    self.ringbuffers[relid].gpadl,
                    self.ringbuffers[relid].send_size,
                )
...
class synth_dev(BaseModule):
    ...
    def run(self, module_argv):
        ...
        command = (
            module_argv[0] if len(module_argv) > 0 and module_argv[0] != "" else "none"
        )
        relid = (
            get_int_arg(module_argv[1])
            if len(module_argv) > 1 and module_argv[1] != ""
            else 0x5
        )
        ...
        try:
            ...
            if relid not in [
                value["child_relid"] for value in vb.offer_channels.values()
            ]:
            ...
            if command == "info":
                vb.vmbus_establish_gpadl(
                    relid, vb.ringbuffers[relid].gpadl, vb.ringbuffers[relid].pfn
                )
                vb.vmbus_open(
                    relid, vb.ringbuffers[relid].gpadl, vb.ringbuffers[relid].send_size
                )
