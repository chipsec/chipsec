...
class RingBufferFuzzer(RingBuffer):
    ...
    def ringbuffer_read(self):
        if self.fuzzing:
            ...
            write_index, read_index, interrupt_mask, pending_send_sz = unpack(
                "<4L", buffer
            )
            overwrite(buffer, DD(randint(0, 0xFFFFFFFF)), 4 * randint(0, 3))
            self.cs.mem.write_physical_mem(
                self.pfn[self.send_size], len(buffer), buffer
            )
            result = ""
            self.count += 1
...
class synth_kbd(BaseModule):
    ...
    def run(self, module_argv):
        ...
        if relid == 0:
            vb.fatal(
                "Could not found keyboard device with GUID: {}".format(HV_KBD_GUID)
            )
        ...
        try:
            ...
            synth_kbd_protocol_request = pack(
                "<LL", SYNTH_KBD_PROTOCOL_REQUEST, SYNTH_KBD_VERSION
            )
            vb.vmbus_sendpacket(
                relid,
            ...
            if (proto_status & 0x1) == 0x1:
                ...
                while True:
                    ...
                    if msg_type == SYNTH_KBD_EVENT:
                        ...
                        vb.hex(
                            "unhandled message type: {:d}".format(msg_type),
                            synth_kbd_msg,
                        )
            else:
