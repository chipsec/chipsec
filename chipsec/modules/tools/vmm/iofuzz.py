...
class iofuzz(BaseModule):
    ...
    def fuzz_ports(self, iterations, write_count, random_order=False):
        ...
        for it in range(iterations):
            ...
            port_value = self.cs.hals.PortIO.read_port_byte(io_addr)

            if _FUZZ_SPECIAL_VALUES:
                self.logger.log('    Writing special 1-2-4 byte values')
                try:
                    self.cs.hals.PortIO.write_port_byte(io_addr, port_value)
                    self.cs.hals.PortIO.write_port_byte(io_addr, (~port_value) & 0xFF)
                    self.cs.hals.PortIO.write_port_byte(io_addr, 0xFF)
                    self.cs.hals.PortIO.write_port_byte(io_addr, 0x00)
                    self.cs.hals.PortIO.write_port_byte(io_addr, 0x5A)
                    self.cs.hals.PortIO.write_port_word(io_addr, 0xFFFF)
                    self.cs.hals.PortIO.write_port_word(io_addr, 0x0000)
                    self.cs.hals.PortIO.write_port_word(io_addr, 0x5AA5)
                    self.cs.hals.PortIO.write_port_dword(io_addr, 0xFFFFFFFF)
                    self.cs.hals.PortIO.write_port_dword(io_addr, 0x00000000)
                    self.cs.hals.PortIO.write_port_word(io_addr, 0x5AA55AA5)
                except:
                    pass
            ...
            for v in range(MAX_PORT_VALUE + 1):
                for _ in range(write_count):
                    try:
                        self.cs.hals.PortIO.write_port_byte(io_addr, v)
                    except:
                        pass
