...
class iofuzz(BaseModule):
    def fuzz_ports(self, iterations, write_count, random_order=False):
        ...
        if random_order:
            self.logger.log(
                "[*] Fuzzing randomly chosen {:d} I/O ports..\n".format(iterations)
            )
        else:
            self.logger.log(
                "[*] Fuzzing I/O ports in a range 0:0x{:X}..\n".format(iterations - 1)
            )
        ...
        for it in range(iterations):
            ...
            self.logger.log(
                "    writing values 0..{:X} ({:d} times each)".format(
                    MAX_PORT_VALUE, write_count
                )
            )
    ...
    def run(self, module_argv):
        ...
        self.logger.log(
            "  count           how many times to write to each port (default = {:d})".format(
                DEFAULT_PORT_WRITE_COUNT
            )
        )
        self.logger.log(
            "  iterations      number of I/O ports to fuzz (default = {:d} in random mode)".format(
        ...
        write_count = (
            int(module_argv[1]) if len(module_argv) > 1 else DEFAULT_PORT_WRITE_COUNT
        )
        ...
        self.logger.log(
            "    Mode            : {}".format(
                "random" if _random_order else "exhaustive"
            )
        )
