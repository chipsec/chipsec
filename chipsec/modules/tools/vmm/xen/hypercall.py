...
from chipsec.modules.tools.vmm.common import BaseModuleHwAccess
...
class XenHypercall(BaseModuleHwAccess):
    ...
    def get_value(self, arg):
        if type(arg) in [dict]:
            value = choice(arg.get('masks')) & getrandbits(64)
    ...
    def scan_hypercalls(self, vector_list):
        for vector in vector_list:
            ...
            if (result['exception'] is False) and (result['status'] != get_invalid_hypercall_code()):
