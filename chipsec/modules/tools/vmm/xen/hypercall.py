...
class XenHypercall(BaseModuleHwAccess):
    def __init__(self):
        ...
        self.buff_va = 0
        self.buff_pa = 0
        (self.buff_va, self.buff_pa) = self.cs.mem.alloc_physical_mem(
    ...
    def hypercall(self, args, size=0, data=""):
        ...
        self.dbg(
            "ARGS: {}  DATA: {}".format(
                " ".join(["{:016X}".format(x) for x in args]),
                binascii.hexlify(data[:32]),
            )
        )
        try:
    ...
    def get_hypervisor_info(self):
        ...
        if not self.xen_version(XENVER_VERSION)["exception"]:
            ...
            info["xen_version"] = "{:d}.{:d}{}".format(
                info["xen_major"], info["xen_minor"], extra_version["buffer"]
            )
            ...
            info["platform_parameters"] = struct.unpack(
                "<Q", platform_parameters["buffer"]
            )[0]
            ...
            for i in range(0x100):
                feature = self.xen_version(
                    XENVER_GET_FEATURES, 8, struct.pack("<LL", i, 0)
                )
    ...
    def scan_hypercalls(self, vector_list):
        for vector in vector_list:
            ...
            if (result["exception"] == False) and (
                result["status"] != get_invalid_hypercall_code()
            ):
                self.hypercalls[vector] = result
                self.add_initial_data(
                    vector,
                    result["buffer"],
                    get_hypercall_status(result["status"], True),
                )
        return
    ...
    def print_hypercall_status(self):
        ...
        for vector in sorted(self.hypercalls.keys()):
            ...
            self.msg(
                "HYPERCALL {:04X}  {:016X}  {:45} '{}'".format(
                    vector, data["status"], status, name
                )
            )
        return
    ...
    def fuzz_hypercall(self, code, iterations):
        ...
        args = rule.get("args", [])
        self.msg(
            "Fuzzing {} (0x{:02X}) hypercall".format(
                get_hypercall_name(code, "Unknown"), code
            )
        )
        self.stats_reset()
