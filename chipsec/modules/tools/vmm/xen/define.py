...
def get_hypercall_status(code, brief=False):
    ...
    if (code >> 32) == 0xFFFFFFFF:
        code = get_iverr(code)
        defstatus = [
            "XEN_ERRNO_{:04X}".format(code),
            "Unknown error 0x{:04X}".format(code),
        ]
...
def get_hypercall_status_extended(code):
    return "{} - {}".format(
        get_hypercall_status(code, False), get_hypercall_status(code, True)
    )
...
console_io_commands = {0: "CONSOLEIO_WRITE", 1: "CONSOLEIO_READ"}
...
set_variables(
    {get_iverr(i): hypercall_status_codes[i][0] for i in hypercall_status_codes.keys()}
)
