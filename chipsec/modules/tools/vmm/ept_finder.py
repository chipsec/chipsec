...
class ept_finder(BaseModule):
        ...
        if self.read_from_file:
            for (pa, end_pa, source) in self.par:
                if (pa <= addr) and (addr + size <= end_pa):
                    ...
                    return source.read(size)
            self.logger.error(
                "Invalid memory address: %016x-%016x" % (addr, addr + size)
            )
            return "\xFF" * size
        return self.cs.mem.read_physical_mem(addr, size)
    ...
    def dump_dram(self, filename, pa, end_pa, buffer_size=0x100000):
        with open(filename, "wb") as dram:
            ...
            while pa < end_pa:
                dram.write(
                    self.cs.mem.read_physical_mem(pa, min(end_pa - pa, buffer_size))
                )
                pa += buffer_size
        return
    ...
    def usage(self):
        self.logger.log(
            "Usage: chipsec_main -m tools.vmm.ept_finder [ -a dump,<file_name>|file,<file_name>,<revision_id> ]"
        )
        return
    ...
    def run(self, module_argv):
        ...
        if self.read_from_file:
            if len(module_argv) == 3:
                ...
                for name in filenames:
                    ...
                    self.logger.log(
                        "  Mapping file to address: 0x%012x  size: 0x%012x  name: %s"
                        % (addr, size, name)
                    )
        ...
        if len(module_argv) == 2 and module_argv[0] == "dump":
            for (pa, end_pa, source) in self.par:
                postfix = (
                    "lo" if pa == 0x0 else "hi" if pa == 0x100000000 else "0x%08x" % pa
                )
        ...
        self.logger.log(
            "[*] -> EPTP: "
            + " ".join(["%08X" % x for x in sorted(ept_pml4_list.keys())])
        )
        ept_vmcs_list = self.find_vmcs_by_ept(
            [x for x in ept_pml4_list.keys()], revision_id
        )
        self.logger.log("[*] Found VMCSs: %d" % len(ept_vmcs_list))
        self.logger.log(
            "[*] -> VMCS: " + " ".join(["%08X" % x for x in sorted(ept_vmcs_list)])
        )
        ...
        for eptp in sorted(ept_pml4_list.keys()):
            ept = c_extended_page_tables_from_file(
                self.cs, self.read_from_file, self.par
            )
