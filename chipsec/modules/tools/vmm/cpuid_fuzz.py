...
class cpuid_fuzz(BaseModule):
    def fuzz_CPUID(self, eax_start, random_order=False):
        ...
        self.logger.log(
            "[*] Fuzzing CPUID with EAX in range 0x{:08X}:0x{:08X}..".format(
                eax_start, eax_end
            )
        )
        it = 0
            ...
            if eax not in _EXCLUDE_CPUID:
                ...
                if _FUZZ_ECX_RANDOM:
                    ...
                    for ecx in range(_MAX_ECX):
                        ...
                        if _LOG_OUT_RESULTS:
                            self.logger.log(
                                "    Out: EAX=0x{:08X}, EBX=0x{:08X}, ECX=0x{:08X}, EDX=0x{:08X}".format(
    ...
    def run(self, module_argv):
        ...
        self.logger.log(
            "    Mode: {}".format("random" if _random_order else "sequential")
        )
        self.logger.log(
            "    Step to fuzz range of EAX values (_EAX_FUZZ_STEP): 0x{:X}".format(
                _EAX_FUZZ_STEP
            )
        )
        self.logger.log(
            "    No of EAX values to fuzz within each step (_NO_EAX_TO_FUZZ): 0x{:X}".format(
                _NO_EAX_TO_FUZZ
            )
        )
        self.logger.log(
            "    Fuzz ECX with random values? (_FUZZ_ECX_RANDOM): {:d}".format(
                _FUZZ_ECX_RANDOM
            )
        )
        self.logger.log("    Max ECX value (_MAX_ECX): 0x{:08X}".format(_MAX_ECX))
        self.logger.log(
            "    Exclude the following EAX values from fuzzing (_EXCLUDE_CPUID): {}".format(
                str(_EXCLUDE_CPUID)
            )
        )
        self.logger.log(
            "    Flush log file after each iteration (_FLUSH_LOG_EACH_ITER): {:d}".format(
                _FLUSH_LOG_EACH_ITER
            )
        )
        self.logger.log(
            "    Log output results (_LOG_OUT_RESULTS): {:d}".format(_LOG_OUT_RESULTS)
        )
