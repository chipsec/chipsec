...
class hypercallfuzz(BaseModule):
    ...
    def fuzz_generic_hypercalls(self):
        ...
        _fmt = (
            ...
            if self.maxval <= 0xFF
                ...
                "{:04X}"
                if self.maxval <= 0xFFFF
                else ("{:08X}" if self.maxval <= 0xFFFFFFFF else "{:016X}")
            )
        )
        ...
        if self.random_order:
            ...
            self.logger.log(
                "[*] Fuzzing {:d} random hypercalls with random arguments...".format(
                    self.iterations
                )
            )
            for it in range(self.iterations):
                ...
                if _LOG_ALL_GPRS:
                    self.logger.log(
                        _str.format(it, rax, rbx, rcx, rdx, rdi, rsi, r8, r9, r10, r11)
                    )
                else:
        ...
        self.logger.log(
            "[*] finished fuzzing: time elapsed {:.3f}".format(time.time() - t)
        )
    ...
    def usage(self):
        ...
        self.logger.log(
            "  iterations      number of iterations in random mode (default = {:d})".format(
    ...
    def run(self, module_argv):
        ...
        self.maxval = (
            DEFAULT_MAXVAL_RANDOM if self.random_order else DEFAULT_MAXVAL_EXHAUSTIVE
        )
