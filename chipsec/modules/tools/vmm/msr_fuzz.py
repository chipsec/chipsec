...
class msr_fuzz(BaseModule):
    def fuzz_MSRs(self, msr_addr_start, random_order=False):
        ...
        self.logger.log(
            "[*] Fuzzing MSRs in range 0x{:08X}:0x{:08X}..".format(
                msr_addr_start, msr_addr_end
            )
        )
        it = 0
        if random_order:
        ...
        while it < it_max:
            ...
            if msr_addr not in _EXCLUDE_MSR:
                if _READ_MSR:
                    ...
                    try:
                        (eax, edx) = self.cs.msr.read_msr(0, msr_addr)
                    except:
                        pass
    ...
    def run(self, module_argv):
        ...
        if _random_order:
            self.logger.log(
                "    Number of iterations: {:d}".format(_NO_ITERATIONS_TO_FUZZ)
            )
