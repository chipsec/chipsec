...
class sinkhole(BaseModule):
    ...
    def is_supported(self):
        ...
        if not (self.cs.helper.is_windows() or self.cs.helper.is_linux()):
            ...
            not self.cs.is_register_defined("IA32_APIC_BASE")
    ...
    def check_LAPIC_SMRR_overlap(self):
        ...
        smrrbase = self.cs.get_register_field(
            "IA32_SMRR_PHYSBASE", smrr_physbase_msr, "PhysBase"
        )
        smrr_base = self.cs.get_register_field(
            "IA32_SMRR_PHYSBASE", smrr_physbase_msr, "PhysBase", True
        )
        apicbase = self.cs.get_register_field(
            "IA32_APIC_BASE", apic_base_msr, "APICBase"
        )
        apic_base = self.cs.get_register_field(
            "IA32_APIC_BASE", apic_base_msr, "APICBase", True
        )
        ...
        self.logger.log(
            "   Writing 0x{:X} to IA32_APIC_BASE[APICBase]..".format(smrrbase)
        )
        ...
        res = self.cs.write_register_field(
            "IA32_APIC_BASE",
            "APICBase",
            smrrbase,
        ...
        if res is None:
            self.logger.log_important(
                "Error encountered when attempting to modify IA32_APIC_BASE"
            )
        ...
        if apic_base_msr_new == apic_base_msr:
            ...
            self.logger.log_passed(
                "CPU does not seem to have SMM memory sinkhole vulnerability"
            )
        else:
            self.logger.log_bad("Could modify IA32_APIC_BASE to overlap SMRR")
            self.cs.write_register("IA32_APIC_BASE", apic_base_msr, 0)
            self.logger.log(
                "[*] Restored original value 0x{:016X}".format(apic_base_msr)
            )
            res = ModuleResult.FAILED
            self.logger.log_failed(
                "CPU is susceptible to SMM memory sinkhole vulnerability.  Verify that SMRR is programmed correctly."
            )
