...
_DEFAULT_GPRS = {
    "rax": _FILL_VALUE_QWORD,
    "rbx": _FILL_VALUE_QWORD,
    "rcx": _FILL_VALUE_QWORD,
    "rdx": _FILL_VALUE_QWORD,
    "rsi": _FILL_VALUE_QWORD,
    "rdi": _FILL_VALUE_QWORD,
}
...
def FILL_BUFFER(
    _fill_byte, _fill_size, _ptr_in_buffer, _ptr, _ptr_offset, _sig, _sig_offset
):
    fill_buf = _fill_byte * _fill_size
    if _ptr_in_buffer and _ptr is not None:
        fill_buf = (
            fill_buf[:_ptr_offset]
            + struct.pack("=I", _ptr & 0xFFFFFFFF)
            + fill_buf[_ptr_offset + 4 :]
        )
    if _sig is not None:
...
class smm_ptr(BaseModule):
    ...
    def fill_memory(
        ...
        s = "[*] writing 0x{:X} bytes at 0x{:016X}".format(self.fill_size, _addr)
        if is_ptr_in_buffer:
            s += " -> PTR at +0x{:X}".format(_ptr_offset)
        if _sig is not None:
            s += " -> SIG at +0x{:X}".format(_sig_offset)
        self.logger.log(s)
        ...
        if is_ptr_in_buffer and _ptr is not None:
            self.logger.log(
                "[*] writing buffer at PA 0x{:016X} with 0x{:X} bytes '{}'".format(
                    _ptr, self.fill_size, self.fill_byte
                )
            )
            self.cs.mem.write_physical_mem(
                _ptr, self.fill_size, self.fill_byte * self.fill_size
            )

        return True

    def send_smi(
        self, thread_id, smi_code, smi_data, name, desc, rax, rbx, rcx, rdx, rsi, rdi
    ):
        ...
        self.interrupts.send_SW_SMI(
            thread_id, smi_code, smi_data, rax, rbx, rcx, rdx, rsi, rdi
        )
        return True
    ...
    def check_memory(self, _addr, _smi_desc, fn, restore_contents=False):
        ...
        expected_buf = FILL_BUFFER(
            self.fill_byte,
            self.fill_size,
            _smi_desc.ptr_in_buffer,
            _smi_desc.ptr,
            _smi_desc.ptr_offset,
            _smi_desc.sig,
        ...
        if _changed:
            self.logger.log(
                "    contents changed at 0x{:016X} +{}".format(_addr, differences)
            )
            if restore_contents:
                self.logger.log(
        ...
        if _smi_desc.ptr_in_buffer and _ptr is not None:
            ...
            if _changed1:
                self.logger.log(
                    "    contents changed at 0x{:016X} +{}".format(_ptr, differences1)
                )
                if restore_contents:
                    self.logger.log(
                ...
                if DUMP_MEMORY_ON_DETECT:
                    ...
                    if not os.path.exists(_pth_smi):
                        os.makedirs(_pth_smi)
                    _f = os.path.join(
                        _pth_smi, fn + ("_ptr{:X}.dmp".format(_smi_desc.ptr_offset))
                    )
                    self.logger.log("    dumping buffer to '{}'".format(_f))
                    write_file(_f, buf1)
    ...
    def smi_fuzz_iter(
        self, thread_id, _addr, _smi_desc, fill_contents=True, restore_contents=False
    ):
        #
        ...
        if self.is_check_memory:
            fn = "{:X}-a{:X}_b{:X}_c{:X}_d{:X}_si{:X}_di{:X}".format(
                _smi_desc.smi_data, _rax, _rbx, _rcx, _rdx, _rsi, _rdi
            )
            contents_changed = self.check_memory(_addr, _smi_desc, fn, restore_contents)
            if contents_changed:
                msg = "DETECTED: SMI# {:X} data {:X} (rax={:X} rbx={:X} rcx={:X} rdx={:X} rsi={:X} rdi={:X})".format(
                    _smi_desc.smi_code,
                    _smi_desc.smi_data,
                    _rax,
                    _rbx,
    ...
    def test_config(self, thread_id, _smi_config_fname, _addr, _addr1):
        ...
        self.logger.log(
            "\n[*] >>> Testing SMI handlers defined in '{}'..".format(_smi_config_fname)
        )
        ...
        for line in fcfg:
            if "" == line.strip():
                self.logger.log(
                    "\n[*] testing SMI# 0x{:02X} (data: 0x{:02X}) {} ({})".format(
                        _smi_desc.smi_code,
                        _smi_desc.smi_data,
                        _smi_desc.name,
                        _smi_desc.desc,
                ...
                if "name" == _n:
                    ...
                    _smi_desc.gprs[_n] = (
                        (
                            _addr
                            if "PTR" == var
                            else (_FILL_VALUE_BYTE if "VAL" == var else int(var, 16))
                        )
                        if "*" != var
                        else _FILL_VALUE_QWORD
                    )
    ...
    def test_fuzz(self, thread_id, smic_start, smic_end, _addr, _addr1):
        ...
        self.logger.log(
            "[*] AX in RAX will be overridden with values of SW SMI ports 0xB2/0xB3"
        )
        ...
        self.logger.log(
            "\n[*] Setting values of general purpose registers to 0x{:016X}".format(
                _smi_desc.gprs["rax"]
            )
        )
        max_ptr_off = 1
        ...
        for off in range(max_ptr_off):
            ...
            self.logger.log(
                "\n[*] reloading buffer with PTR at offset 0x{:X}..".format(off)
            )
            if self.is_check_memory:
                self.fill_memory(
                    _addr,
            ...
            for smi_code in range(smic_start, smic_end + 1, 1):
                ...
                for smi_data in range(MAX_SMI_DATA):
                    ...
                    self.logger.log(
                        "\n[*] fuzzing SMI# 0x{:02X} (data: 0x{:02X})".format(
                            smi_code, smi_data
                        )
                    )
                    if FUZZ_SMI_FUNCTIONS_IN_ECX:
                        for _rcx in range(MAX_SMI_FUNCTIONS):
                            self.logger.log(
                                " >> function (RCX): 0x{:016X}".format(_rcx)
                            )
                            _smi_desc.gprs["rcx"] = _rcx
                            if PTR_IN_ALL_GPRS:
                                if self.smi_fuzz_iter(
                                    thread_id, _addr, _smi_desc, False, True
                                ):
                                    bad_ptr_cnt += 1
                            else:
                                self.logger.log("    RBX: 0x{:016X}".format(_addr))
                                ...
                                self.logger.log("    RSI: 0x{:016X}".format(_addr))
                                ...
                                self.logger.log("    RDI: 0x{:016X}".format(_addr))
                        ...
                        if PTR_IN_ALL_GPRS:
                            ...
                            self.logger.log("    RCX: 0x{:016X}".format(_addr))
    ...
    def run(self, module_argv):
        ...
        if len(module_argv) > 3:
            if "smram" == module_argv[3]:
                ...
                self.logger.log(
                    "[*] Using SMRAM base address (0x{:016X}) to pass to SMI handlers".format(
                        _addr
                    )
                )
            else:
                ...
                self.logger.log(
                    "[*] Using address from command-line (0x{:016X}) to pass to SMI handlers".format(
                        _addr
                    )
                )
        else:
            ...
            self.logger.log(
                "[*] Allocated memory buffer (to pass to SMI handlers)       : 0x{:016X}".format(
                    _addr
                )
            )

        if self.is_check_memory:
            (va1, _addr1) = self.cs.mem.alloc_physical_mem(
                self.fill_size, _MAX_ALLOC_PA
            )
            self.logger.log(
                "[*] Allocated 2nd buffer (address will be in the 1st buffer): 0x{:016X}".format(
                    _addr1
                )
            )
        ...
        self.logger.log("\n[*] Configuration")
        self.logger.log("    SMI testing mode          : {}".format(test_mode))
        if "config" == test_mode:
            self.logger.log("    Config file           : {}".format(_smi_config_fname))
        else:
            self.logger.log(
                "    Range of SMI codes (B2)   : 0x{:02X}:0x{:02X}".format(
                    smic_start, smic_end
                )
            )
        self.logger.log(
            "    Memory buffer pointer     : 0x{:016X} (address passed in GP regs to SMI)".format(
                _addr
            )
        )
        self.logger.log(
            "    Filling/checking memory?  : {}".format(
                "YES" if self.is_check_memory else "NO"
            )
        )
        if self.is_check_memory:
            self.logger.log(
                "      Second buffer pointer   : 0x{:016X} (address written to memory buffer)".format(
                    _addr1
                )
            )
            self.logger.log(
                "      Number of bytes to fill : 0x{:X}".format(self.fill_size)
            )
            self.logger.log(
                "      Byte to fill with       : 0x{:X}".format(ord(self.fill_byte))
            )
        self.logger.log("    Additional options (can be changed in the source code):")
        self.logger.log(
            "      Fuzzing SMI functions in ECX?          : {:d}".format(
                FUZZ_SMI_FUNCTIONS_IN_ECX
            )
        )
        self.logger.log(
            "      Max value of SMI function in ECX       : 0x{:X}".format(
                MAX_SMI_FUNCTIONS
            )
        )
        self.logger.log(
            "      Max value of SMI data (B3)             : 0x{:X}".format(MAX_SMI_DATA)
        )
        self.logger.log(
            "      Max offset of the pointer in the buffer: 0x{:X}".format(
                MAX_PTR_OFFSET_IN_BUFFER
            )
        )
        self.logger.log(
            "      Passing pointer in all GP registers?   : {:d}".format(
                PTR_IN_ALL_GPRS
            )
        )
        self.logger.log(
            "      Default values of the registers        : 0x{:016X}".format(
                _FILL_VALUE_QWORD
            )
        )
        self.logger.log(
            "      Dump all register values every SMI     : {:d}".format(
                DUMP_GPRS_EVERY_SMI
            )
        )
        self.logger.log(
            "      Bail on first detection                : {:d}".format(
                FUZZ_BAIL_ON_1ST_DETECT
            )
        )
        ...
        try:
            if "config" == test_mode:
                bad_ptr_cnt = self.test_config(
                    thread_id, _smi_config_fname, _addr, _addr1
                )
            elif "fuzz" == test_mode or "fuzzmore" == test_mode:
                bad_ptr_cnt = self.test_fuzz(
                    thread_id, smic_start, smic_end, _addr, _addr1
                )
        ...
        if bad_ptr_cnt > 0:
            self.logger.log_bad(
                "<<< Done: found {:d} potential occurrences of unchecked input pointers".format(
                    bad_ptr_cnt
                )
            )
        else:
            self.logger.log_good(
                "<<< Done: didn't find unchecked input pointers in tested SMI handlers"
            )
