...
Usage:
  ``chipsec_main -m tools.uefi.scan_image [-a generate|check,<json>,<fw_image>[,norm]]``
    ...
    - ``norm``		Optional, ``generate`` with ``fw_image`` only. Adds
                        ``sha256_norm`` to each PE32 and TE entry: the SHA-256 of
                        the module with its placement undone (relocations reversed;
                        ImageBase, TimeDateStamp, CheckSum and section relocation
                        pointers zeroed), prefixed with the normalization used
...
class scan_image(BaseModule):
    ...
    def parse_norm_arg(self, op: str, module_argv) -> bool:
        if op not in ('generate', 'check') or len(module_argv) <= 3:
            return False
        if len(module_argv) > 4:
            self.logger.log_warning(f'Ignoring {len(module_argv) - 4} extra argument(s)')
    ...
    def run(self, module_argv):
        ...
        self.include_norm = self.parse_norm_arg(op, module_argv)
