...
class s3script_modify(BaseModule):
    ...
    def get_bootscript(self):
        if self.bootscript_PAs is None or self.parsed_scripts is None:
            (self.bootscript_PAs, self.parsed_scripts) = self._uefi.get_s3_bootscript(
                False
            )
    ...
    def modify_s3_reg(self, opcode, address, new_value):
        ...
        for bootscript_pa in bootscript_PAs:
            if bootscript_pa == 0:
                continue
            self.logger.log(
                "[*] Looking for 0x{:X} opcode in the script at 0x{:016X}..".format(
                    opcode, bootscript_pa
                )
            )
            for e in parsed_scripts[bootscript_pa]:
                if (
                    e.decoded_opcode is not None
                    and opcode == e.decoded_opcode.opcode
                    and address == e.decoded_opcode.address
                ):

                    self.logger.log_good(
                        "Found opcode at offset 0x{:04X}".format(e.offset_in_script)
                    )
                    self.logger.log(e)
                    pa = bootscript_pa + e.offset_in_script
                    self.logger.log(
                        "[*] Modifying S3 boot script entry at address 0x{:016X}..".format(
                            pa
                        )
                    )
                    ...
                    if (
                        ...
                        == opcode
                        or S3BootScriptOpcode.EFI_BOOT_SCRIPT_MEM_WRITE_OPCODE == opcode
                        or S3BootScriptOpcode.EFI_BOOT_SCRIPT_IO_WRITE_OPCODE == opcode
                    ):
    ...
    def modify_s3_dispatch(self):
        ...
        self.logger.log_good(
            "Allocated new DISPATCH entry-point at 0x{:016X} (size = 0x{:X}):".format(
                new_entrypoint, ep_size
            )
        )
        print_buffer(new_ep)
        ...
        for bootscript_pa in bootscript_PAs:
            if bootscript_pa == 0:
                continue
            self.logger.log(
                "[*] Searching the script at 0x{:016X} for DISPATCH opcodes..".format(
                    bootscript_pa
                )
            )
            for e in parsed_scripts[bootscript_pa]:
                if (
                    e.decoded_opcode is not None
                    and S3BootScriptOpcode.EFI_BOOT_SCRIPT_DISPATCH_OPCODE
                    == e.decoded_opcode.opcode
                ):
    ...
    def modify_s3_dispatch_ep(self):
        ...
        for script_pa in bootscript_PAs:
            if script_pa == 0:
                continue
            self.logger.log(
                "[*] Looking for DISPATCH opcode in the script at 0x{:016X}..".format(
                    script_pa
                )
            )
            for e in parsed_scripts[script_pa]:
                if (
                    ...
                    self.logger.log_good(
                        "Found DISPATCH opcode at offset 0x{:04X} with entry-point 0x{:016X}".format(
                            e.offset_in_script, ep_pa
                        )
                    )
                    self.logger.log(e)
                    break
        ...
        self.logger.log(
            "[*] New DISPATCH entry-point at 0x{:016X} (size = 0x{:X}):".format(
                ep_pa, ep_size
            )
        )
        print_buffer(new_ep)
    ...
    def modify_s3_mem(self, address, new_value):
        if address is None:
            ...
            self.logger.log(
                "[*] Allocated memory at 0x{:016X} as a target of MEM_WRITE opcode".format(
                    address
                )
            )
        ...
        self.logger.log(
            "[*] Original value at 0x{:016X}: 0x{:08X}".format(address, val)
        )
        ...
        for bootscript_pa in bootscript_PAs:
            if bootscript_pa == 0:
                continue
            self.logger.log(
                "[*] Looking for MEM_WRITE opcode in the script at 0x{:016X}..".format(
                    bootscript_pa
                )
            )
            for e in parsed_scripts[bootscript_pa]:
                if (
                    e.decoded_opcode is not None
                    and S3BootScriptOpcode.EFI_BOOT_SCRIPT_MEM_WRITE_OPCODE
                    == e.decoded_opcode.opcode
                ):

                    self.logger.log_good(
                        "Found opcode at offset 0x{:X}".format(e.offset_in_script)
                    )
                    self.logger.log(e)
                    pa = bootscript_pa + e.offset_in_script
                    self.logger.log(
                    ...
                    print_buffer(new_entry_buf)
                    self.logger.log(
                        "After sleep/resume, read address 0x{:08X} and look for value 0x{:08X}".format(
                            address, new_value
                        )
                    )
                    return True
    ...
    def modify_s3_add(self, new_opcode):
        ...
        for bootscript_pa in bootscript_PAs:
            ...
            self.logger.log(
                "[*] Looking for TERMINATE opcode in the script at 0x{:016X}..".format(
                    bootscript_pa
                )
            )
            for e in parsed_scripts[bootscript_pa]:
                ...
                if (
                    e.decoded_opcode is not None
                    and S3BootScriptOpcode.EFI_BOOT_SCRIPT_TERMINATE_OPCODE
                    == e.decoded_opcode.opcode
                ):
                    self.logger.log_good(
                    ...
                    self.logger.log(
                        "[*] Adding new opcode entry at address 0x{:016X}..".format(pa)
                    )
                    ...
                    self.logger.log(
                        "[*] Moving TERMINATE opcode to the last entry at 0x{:016X}..".format(
                            last_entry_pa
                        )
                    )
                    self.cs.mem.write_physical_mem(
                        last_entry_pa, len(orig_entry_buf), orig_entry_buf
                    )
                    return True
        ...
        if op == "replace_op":
            ...
            if scmd in cmd2opcode:
                if len(module_argv) < 4:
                    self.logger.error(
                        "Expected module options: -a replace_op,{},<reg_address>,<value>".format(
                            scmd
                        )
                    )
                ...
                if sts:
                    self.logger.log(
                        "[*] After sleep/resume, check the value of register 0x{:X} is 0x{:X}".format(
                            reg_address, value
                        )
                    )
            elif "dispatch" == scmd:
                ...
                new_value = (
                    int(module_argv[2], 16) if len(module_argv) >= 3 else 0xB007B007
                )
                address = int(module_argv[3], 16) if len(module_argv) == 4 else None
                sts = self.modify_s3_mem(address, new_value)
            else:
                self.logger.error(
                    "Unrecognized module command-line argument: {}".format(scmd)
                )
            ...
            if scmd in cmd2opcode:
                if len(module_argv) < 5:
                    self.logger.error(
                        "Expected module options: -a add_op,{},<reg_address>,<value>,<width>".format(
                            scmd
                        )
                    )
                    return ModuleResult.ERROR
                ...
                if (
                    S3BootScriptOpcode.EFI_BOOT_SCRIPT_MEM_WRITE_OPCODE
                    == cmd2opcode[scmd]
                    or S3BootScriptOpcode.EFI_BOOT_SCRIPT_PCI_CONFIG_WRITE_OPCODE
                    == cmd2opcode[scmd]
                    or S3BootScriptOpcode.EFI_BOOT_SCRIPT_IO_WRITE_OPCODE
                    == cmd2opcode[scmd]
                ):
                    new_opcode = op_io_pci_mem(
                        cmd2opcode[scmd],
                        None,
                        width_val,
                        address,
                ...
                if len(module_argv) < 3:
                    ...
                    self.cs.mem.write_physical_mem(
                        entrypoint,
                        len(self.DISPATCH_ENTRYPOINT_INSTR),
                        self.DISPATCH_ENTRYPOINT_INSTR,
                    )
                else:
                    entrypoint = int(module_argv[2], 16)
                new_opcode = op_dispatch(
                    S3BootScriptOpcode.EFI_BOOT_SCRIPT_DISPATCH_OPCODE, None, entrypoint
                )
            else:
