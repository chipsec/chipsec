...
class uefivar_fuzz(BaseModule):
    ...
    def run(self, module_argv):
        ...
        attrib_prim = prim.dword(
            value=_ATTRIB
        )  # i think the attrib field is 4 bytes large?
            ...
            for count in range(1, ITERATIONS + CASE):
                ...
                if FUZZ_DATA:
                    if data_prim.mutate():
                        ...
                        data_prim = prim.random_data(
                            value=_DATA, min_length=0, max_length=BOUND_INT
                        )
                        data_prim.mutate()
                        _DATA = data_prim.render()
                ...
                self.logger.flush()
                status = self._uefi.set_EFI_variable(
                    bytestostring(_NAME), str(_GUID), _DATA, _SIZE, _ATTRIB
                )
