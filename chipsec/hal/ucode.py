...
class UcodeUpdateHeader(
    namedtuple(
        "UcodeUpdateHeader",
        "header_version update_revision date processor_signature checksum loader_revision processor_flags data_size total_size reserved1 reserved2 reserved3",
    )
):
    __slots__ = ()
...
def read_ucode_file(ucode_filename):
    ...
    if ucode_filename.endswith(".pdb"):
        if logger().HAL:
            logger().log(
                "[ucode] PDB file '{:256}' has ucode update header (size = 0x{:X})".format(
                    ucode_filename, UCODE_HEADER_SIZE
                )
            )
...
class Ucode:
    ...
    def ucode_update_id(self, cpu_thread_id):
        ...
        (bios_sign_id_lo, bios_sign_id_hi) = self.helper.read_msr(
            cpu_thread_id, IA32_MSR_BIOS_SIGN_ID
        )
        ...
        if bios_sign_id_lo & IA32_MSR_BIOS_SIGN_ID_STATUS:
            if logger().HAL:
                logger().log(
                    "[ucode] CPU{:d}: last Microcode update failed (current microcode id = 0x{:08X})".format(
            ...
            if logger().HAL:
                logger().log(
                    "[ucode] CPU{:d}: Microcode update ID = 0x{:08X}".format(
                        cpu_thread_id, ucode_update_id
                    )
                )
    ...
    def load_ucode_update(self, cpu_thread_id, ucode_buf):
        if logger().HAL:
            logger().log(
                "[ucode] loading microcode update on CPU{:d}".format(cpu_thread_id)
            )
