...
class Ucode(hal_base.HALBase):
    ...
    def ucode_update_id(self, cpu_thread_id):
        ...
        if (bios_sign_id_lo & IA32_MSR_BIOS_SIGN_ID_STATUS):
            self.logger.log_hal("[ucode] CPU{:d}: last Microcode update failed (current microcode id = 0x{:08X})".format(cpu_thread_id, ucode_update_id))
        else:
            self.logger.log_hal("[ucode] CPU{:d}: Microcode update ID = 0x{:08X}".format(cpu_thread_id, ucode_update_id))
    ...
    def update_ucode_all_cpus(self, ucode_file):
        if not (os.path.exists(ucode_file) and os.path.isfile(ucode_file)):
            self.logger.error("Ucode file not found: '{:.256}'".format(ucode_file))
            return False
    ...
    def load_ucode_update(self, cpu_thread_id, ucode_buf):
        self.logger.log_hal("[ucode] loading microcode update on CPU{:d}".format(cpu_thread_id))
