...
class Interrupts(hal_base.HALBase):
    ...
    def send_SW_SMI(
        ...
        if logger().HAL:
            logger().log(
                "[intr] Sending SW SMI: code port 0x{:02X} <- 0x{:02X}, data port 0x{:02X} <- 0x{:02X} (0x{:04X})".format(
                    SMI_APMC_PORT,
                    SMI_code_port_value,
                    SMI_APMC_PORT + 1,
                    SMI_data_port_value,
        ...
        return self.cs.helper.send_sw_smi(
            thread_id, SMI_code_data, _rax, _rbx, _rcx, _rdx, _rsi, _rdi
        )

    def send_SMI_APMC(self, SMI_code_port_value, SMI_data_port_value):
        if logger().HAL:
            logger().log(
                "[intr] sending SMI via APMC ports: code 0xB2 <- 0x{:02X}, data 0xB3 <- 0x{:02X}".format(
                    SMI_code_port_value, SMI_data_port_value
                )
            )
    ...
    def send_ACPI_SMI(self, thread_id, smi_num, buf_addr, invoc_reg, guid, data):
        ...
        if not invoc_reg is None:
            ...
            tmp_buf = self.cs.helper.write_physical_mem(
                buf_addr, len(data_hdr), data_hdr
            )
            # USING GAS need to write buf_addr into invoc_reg
            if invoc_reg.addrSpaceID == 0:
                self.cs.helper.write_physical_mem(
                    invoc_reg.addr, invoc_reg.access_size, buf_addr
                )
                # check for return status
                ret_buf = self.cs.helper.read_physical_mem(buf_addr, 8)
            elif invoc_reg.addrSpaceID == 1:
                self.cs.helper.write_io_port(
                    invoc_reg.addr, invoc_reg.access_size, buf_addr
                )
                # check for return status
            ...
            # clear CommBuffer
            self.cs.helper.write_physical_mem(
                buf_addr, len(data_hdr), b"\x00" * len(data_hdr)
            )
            return None
...
#define SMM_CORE_PRIVATE_DATA_SIGNATURE  SIGNATURE_32 ('s', 'm', 'm', 'c')
 struct {
  ...
  EFI_STATUS                      ReturnStatus;
    ...
    def send_smmc_SMI(
        self, smmc, guid, payload, payload_loc, CommandPort=0x0, DataPort=0x0
    ):
        ...
        self.cs.mem.write_physical_mem(
            smmc + CommBuffer_offset, 8, struct.pack("Q", payload_loc)
        )
        self.cs.mem.write_physical_mem(
            smmc + BufferSize_offset, 8, struct.pack("Q", len(data_hdr))
        )
