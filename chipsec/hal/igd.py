...
class IGD(hal_base.HALBase):
    ...
    def is_enabled(self):
        if self.cs.register_has_field(
            "PCI0.0.0_DEVEN", "D2EN"
        ) and self.cs.register_has_field("PCI0.0.0_CAPID0_A", "IGD"):
            if (
                self.cs.read_register_field("PCI0.0.0_DEVEN", "D2EN") == 1
                and self.cs.read_register_field("PCI0.0.0_CAPID0_A", "IGD") == 0
            ):
                return True
    ...
    def get_GTTMMADR(self):
        ...
        if self.logger.HAL:
            self.logger.log(
                "[igd] Graphics MMIO and GTT (GTTMMADR): 0x{:016X}".format(base)
            )
        return base
    ...
    def write_GGTT_PTE(self, pte_num, pte):
        gtt_base = self.get_GGTT_base()
        self.cs.mmio.write_MMIO_reg(
            gtt_base, self.get_PTE_size() * pte_num, pte & 0xFFFFFFFF
        )
        if self.get_PTE_size() == 8:
            self.cs.mmio.write_MMIO_reg(
                gtt_base, self.get_PTE_size() * pte_num + 4, pte >> 32
            )
        return pte
    ...
    def gfx_aperture_dma_read_write(self, address, size=0x4, value=None, pte_num=0):
        ...
        if self.logger.HAL:
            self.logger.log(
                "[igd] reading 0x{:X} bytes at PA 0x{:016X} through IGD aperture (DMA) using PTE{:d}".format(
                    size, address, pte_num
                )
            )
            self.logger.log("[igd] GFx aperture (GMADR): 0x{:016X}".format(gmadr))
            self.logger.log(
                "[igd] GFx GTT base        : 0x{:016X}".format(self.get_GGTT_base())
            )
            self.logger.log(
                "[igd] original GTT PTE{:03d}: 0x{:08X}".format(pte_num, pte_orig)
            )
        ...
        if self.logger.HAL:
            self.logger.log(
                "[igd] pages = 0x{:X}, r = 0x{:X}, N = {:d}".format(pages, r, N)
            )
        ...
        for p in range(N):
            ...
            if self.logger.HAL:
                self.logger.log(
                    "[igd] GFx PTE for address 0x{:016X}: 0x{:08X}".format(address, pte)
                )
            ...
            if value is None:
                if self.logger.HAL:
                    self.logger.log(
                        "[igd] reading 0x{:X} bytes at 0x{:016X} through GFx aperture 0x{:016X} ..".format(
                            size, pa, igd_addr + pa_off
                        )
                    )
                ...
                if self.logger.HAL:
                    self.logger.log(
                        "[igd] writing 0x{:X} bytes to 0x{:016X} through GFx aperture 0x{:016X} ..".format(
                            size, pa, igd_addr + pa_off
                        )
                    )
        ...
        if self.logger.HAL:
            self.logger.log(
                "[igd] restoring GFx PTE{:d} 0x{:X}..".format(pte_num, pte_orig)
            )
