...
class DMAR(ACPI_TABLE):
    ...
    def parse(self, table_content):
        ...
        while off < len(table_content) - 1:
            (_type, length) = struct.unpack(
                struct_fmt, table_content[off : off + struct.calcsize(struct_fmt)]
            )
            if 0 == length:
                break
            self.dmar_structures.append(
                self._get_structure_DMAR(_type, table_content[off : off + length])
            )
            off += length
        (self.HostAddrWidth, self.Flags, self.Reserved) = struct.unpack_from(
            ACPI_TABLE_FORMAT_DMAR, table_content
        )
        return
...
""".format(
            self.HostAddrWidth,
            self.Flags,
            "".join("{:02x} ".format(ord(c)) for c in bytestostring(self.Reserved)),
        )
    ...
    def _get_structure_DMAR(self, _type, DataStructure):
        if 0x00 == _type:
            return self._get_DMAR_structure_DRHD(DataStructure)
        elif 0x01 == _type:
            return self._get_DMAR_structure_RMRR(DataStructure)
        elif 0x02 == _type:
            return self._get_DMAR_structure_ATSR(DataStructure)
        elif 0x03 == _type:
            return self._get_DMAR_structure_RHSA(DataStructure)
        elif 0x04 == _type:
            return self._get_DMAR_structure_ANDD(DataStructure)
        else:
    ...
    def _get_DMAR_structure_DRHD(self, structure):
        ...
        while off < len(structure) - 1:
            ...
            if 0 == length:
                break
            path_sz = length - struct.calcsize(
                self.DMAR_TABLE_FORMAT["DeviceScope_FORMAT"]
            )
            f = self.DMAR_TABLE_FORMAT["DeviceScope_FORMAT"] + ("{:d}s".format(path_sz))
            device_scope.append(
                ACPI_TABLE_DMAR_DeviceScope(
                    *struct.unpack_from(f, structure[off : off + length])
                )
            )
            off += length
        return ACPI_TABLE_DMAR_DRHD(
            *struct.unpack_from(self.DMAR_TABLE_FORMAT["DRHD_FORMAT"], structure),
    ...
    def _get_DMAR_structure_RMRR(self, structure):
        ...
        while off < len(structure) - 1:
            ...
            off += length
        return ACPI_TABLE_DMAR_RMRR(
            *struct.unpack_from(self.DMAR_TABLE_FORMAT["RMRR_FORMAT"], structure),
            DeviceScope=device_scope
        )
    ...
    def _get_DMAR_structure_ATSR(self, structure):
        ...
        while off < len(structure) - 1:
            ...
            off += length
        return ACPI_TABLE_DMAR_ATSR(
            *struct.unpack_from(self.DMAR_TABLE_FORMAT["ATSR_FORMAT"], structure),
            DeviceScope=device_scope
        )

    def _get_DMAR_structure_RHSA(self, structure):
        return ACPI_TABLE_DMAR_RHSA(
            *struct.unpack_from(self.DMAR_TABLE_FORMAT["RHSA_FORMAT"], structure)
        )

    def _get_DMAR_structure_ANDD(self, structure):
        sz = struct.calcsize("=H")
        length = struct.unpack("=H", structure[sz : sz + sz])[0]
        f = self.DMAR_TABLE_FORMAT["ANDD_FORMAT"] + (
            "{:d}s".format(
                length - struct.calcsize(self.DMAR_TABLE_FORMAT["ANDD_FORMAT"])
            )
        )
...
class ACPI_TABLE_DMAR_DeviceScope(
    namedtuple(
        "ACPI_TABLE_DMAR_DeviceScope",
        "Type Length Reserved EnumerationID StartBusNum Path",
    )
):
    __slots__ = ()
...
""".format(
            DMAR_DS_TYPE[self.Type],
            self.Type,
            self.Length,
            self.Reserved,
            self.EnumerationID,
            self.StartBusNum,
            "".join("{:02x} ".format(ord(c)) for c in bytestostring(self.Path)),
        )
...
class ACPI_TABLE_DMAR_DRHD(
    namedtuple(
        "ACPI_TABLE_DMAR_DRHD",
        "Type Length Flags Reserved SegmentNumber RegisterBaseAddr DeviceScope",
    )
):
    __slots__ = ()
...
class ACPI_TABLE_DMAR_RMRR(
    namedtuple(
        "ACPI_TABLE_DMAR_RMRR",
        "Type Length Reserved SegmentNumber RMRBaseAddr RMRLimitAddr DeviceScope",
    )
):
    __slots__ = ()
...
class ACPI_TABLE_DMAR_ATSR(
    namedtuple(
        "ACPI_TABLE_DMAR_ATSR", "Type Length Flags Reserved SegmentNumber DeviceScope"
    )
):
    __slots__ = ()
...
class ACPI_TABLE_DMAR_RHSA(
    namedtuple(
        "ACPI_TABLE_DMAR_RHSA", "Type Length Reserved RegisterBaseAddr ProximityDomain"
    )
):
    __slots__ = ()
...
class ACPI_TABLE_DMAR_ANDD(
    namedtuple("ACPI_TABLE_DMAR_ANDD", "Type Length Reserved ACPIDevNum ACPIObjectName")
):
    __slots__ = ()
...
class APIC(ACPI_TABLE):
    ...
    def parse(self, table_content):
        ...
        while cont < len(table_content) - 1:
            ...
            if 0 == length:
                break
            self.apic_structs.append(
                self.get_structure_APIC(value, table_content[cont : cont + length])
            )
            cont += length
        return
...
""".format(
    ...
    def get_structure_APIC(self, value, DataStructure):
        if 0x00 == value:
            return ACPI_TABLE_APIC_PROCESSOR_LAPIC(
                *struct.unpack_from(
                    self.APIC_TABLE_FORMAT["PROCESSOR_LAPIC"], DataStructure
                )
            )
        elif 0x01 == value:
            return ACPI_TABLE_APIC_IOAPIC(
                *struct.unpack_from(self.APIC_TABLE_FORMAT["IOAPIC"], DataStructure)
            )
        elif 0x02 == value:
            return ACPI_TABLE_APIC_INTERRUPT_SOURSE_OVERRIDE(
                *struct.unpack_from(
                    self.APIC_TABLE_FORMAT["INTERRUPT_SOURSE_OVERRIDE"], DataStructure
                )
            )
        elif 0x03 == value:
            return ACPI_TABLE_APIC_NMI_SOURCE(
                *struct.unpack_from(self.APIC_TABLE_FORMAT["NMI_SOURCE"], DataStructure)
            )
        elif 0x04 == value:
            return ACPI_TABLE_APIC_LAPIC_NMI(
                *struct.unpack_from(self.APIC_TABLE_FORMAT["LAPIC_NMI"], DataStructure)
            )
        elif 0x05 == value:
            return ACPI_TABLE_APIC_LAPIC_ADDRESS_OVERRIDE(
                *struct.unpack_from(
                    self.APIC_TABLE_FORMAT["LAPIC_ADDRESS_OVERRIDE"], DataStructure
                )
            )
        elif 0x06 == value:
            return ACPI_TABLE_APIC_IOSAPIC(
                *struct.unpack_from(self.APIC_TABLE_FORMAT["IOSAPIC"], DataStructure)
            )
        elif 0x07 == value:
            return ACPI_TABLE_APIC_PROCESSOR_LSAPIC(
                *struct.unpack_from(
                    "{}{}s".format(
                        self.APIC_TABLE_FORMAT["PROCESSOR_LSAPIC"],
                        str(len(DataStructure) - 16),
                    ),
                    DataStructure,
                )
            )
        elif 0x08 == value:
            return ACPI_TABLE_APIC_PLATFORM_INTERRUPT_SOURCES(
                *struct.unpack_from(
                    self.APIC_TABLE_FORMAT["PLATFORM_INTERRUPT_SOURCES"], DataStructure
                )
            )
        elif 0x09 == value:
            return ACPI_TABLE_APIC_PROCESSOR_Lx2APIC(
                *struct.unpack_from(
                    self.APIC_TABLE_FORMAT["PROCESSOR_Lx2APIC"], DataStructure
                )
            )
        elif 0x0A == value:
            return ACPI_TABLE_APIC_Lx2APIC_NMI(
                *struct.unpack_from(
                    self.APIC_TABLE_FORMAT["Lx2APIC_NMI"], DataStructure
                )
            )
        elif 0x0B == value:
            return ACPI_TABLE_APIC_GICC_CPU(
                *struct.unpack_from(self.APIC_TABLE_FORMAT["GICC_CPU"], DataStructure)
            )
        elif 0x0C == value:
            return ACPI_TABLE_APIC_GIC_DISTRIBUTOR(
                *struct.unpack_from(
                    self.APIC_TABLE_FORMAT["GIC_DISTRIBUTOR"], DataStructure
                )
            )
        elif 0x0D == value:
            return ACPI_TABLE_APIC_GIC_MSI(
                *struct.unpack_from(self.APIC_TABLE_FORMAT["GIC_MSI"], DataStructure)
            )
        elif 0x0E == value:
            return ACPI_TABLE_APIC_GIC_REDISTRIBUTOR(
                *struct.unpack_from(
                    self.APIC_TABLE_FORMAT["GIC_REDISTRIBUTOR"], DataStructure
                )
            )
        else:
...
class ACPI_TABLE_APIC_PROCESSOR_LAPIC(
    namedtuple("ACPI_TABLE_APIC_PROCESSOR_LAPIC", "Type Length ACPIProcID APICID Flags")
):
    __slots__ = ()
...
class ACPI_TABLE_APIC_IOAPIC(
    namedtuple(
        "ACPI_TABLE_APIC_IOAPIC",
        "Type Length IOAPICID Reserved IOAPICAddr GlobalSysIntBase",
    )
):
    __slots__ = ()
...
""".format(
            ...
            self.Reserved,
            self.IOAPICAddr,
            self.GlobalSysIntBase,
        )


class ACPI_TABLE_APIC_INTERRUPT_SOURSE_OVERRIDE(
    namedtuple(
        "ACPI_TABLE_APIC_INTERRUPT_SOURSE_OVERRIDE",
        "Type Length Bus Source GlobalSysIntBase Flags",
    )
):
    __slots__ = ()
...
class ACPI_TABLE_APIC_NMI_SOURCE(
    namedtuple("ACPI_TABLE_APIC_NMI_SOURCE", "Type Length Flags GlobalSysIntBase")
):
    __slots__ = ()
...
class ACPI_TABLE_APIC_LAPIC_NMI(
    namedtuple(
        "ACPI_TABLE_APIC_LAPIC_NMI", "Type Length ACPIProcessorID Flags LocalAPICLINT"
    )
):
    __slots__ = ()
...
class ACPI_TABLE_APIC_LAPIC_ADDRESS_OVERRIDE(
    namedtuple(
        "ACPI_TABLE_APIC_LAPIC_ADDRESS_OVERRIDE",
        "Type Length Reserved LocalAPICAddress",
    )
):
    __slots__ = ()
...
class ACPI_TABLE_APIC_IOSAPIC(
    namedtuple(
        "ACPI_TABLE_APIC_IOSAPIC",
        "Type Length IOAPICID Reserved GlobalSysIntBase IOSAPICAddress",
    )
):
    __slots__ = ()
...
class ACPI_TABLE_APIC_PROCESSOR_LSAPIC(
    namedtuple(
        "ACPI_TABLE_APIC_PROCESSOR_LSAPIC",
        "Type Length ACPIProcID LocalSAPICID LocalSAPICEID Reserved Flags ACPIProcUIDValue ACPIProcUIDString",
    ),
):
    __slots__ = ()
...
""".format(
            ...
            self.Reserved,
            self.Flags,
            self.ACPIProcUIDValue,
            self.ACPIProcUIDString,
        )


class ACPI_TABLE_APIC_PLATFORM_INTERRUPT_SOURCES(
    namedtuple(
        "ACPI_TABLE_APIC_PLATFORM_INTERRUPT_SOURCES",
        "Type Length Flags InterruptType ProcID ProcEID IOSAPICVector GlobalSystemInterrupt PlatIntSourceFlags",
    )
):
    __slots__ = ()
...
""".format(
            ...
            self.ProcEID,
            self.IOSAPICVector,
            self.GlobalSystemInterrupt,
            self.PlatIntSourceFlags,
        )


class ACPI_TABLE_APIC_PROCESSOR_Lx2APIC(
    namedtuple(
        "ACPI_TABLE_APIC_PROCESSOR_Lx2APIC",
        "Type Length Reserved x2APICID Flags ACPIProcUID",
    )
):
    __slots__ = ()
...
class ACPI_TABLE_APIC_Lx2APIC_NMI(
    namedtuple(
        "ACPI_TABLE_APIC_Lx2APIC_NMI",
        "Type Length Flags ACPIProcUID Localx2APICLINT Reserved",
    )
):
    __slots__ = ()
...
class ACPI_TABLE_APIC_GICC_CPU(
    namedtuple(
        "ACPI_TABLE_APIC_GICC_CPU",
        "Type Length Reserved CPUIntNumber ACPIProcUID Flags ParkingProtocolVersion PerformanceInterruptGSIV ParkedAddress PhysicalAddress GICV GICH VGICMaintenanceINterrupt GICRBaseAddress MPIDR",
    )
):
    __slots__ = ()
...
""".format(
            ...
            self.ACPIProcUID,
            self.Flags,
            self.ParkingProtocolVersion,
            self.PerformanceInterruptGSIV,
            self.ParkedAddress,
            self.PhysicalAddress,
            self.GICV,
            self.GICH,
            self.VGICMaintenanceINterrupt,
            self.GICRBaseAddress,
            self.MPIDR,
        )
...
class ACPI_TABLE_APIC_GIC_DISTRIBUTOR(
    namedtuple(
        "ACPI_TABLE_APIC_GIC_DISTRIBUTOR",
        "Type Length Reserved GICID PhysicalBaseAddress SystemVectorBase Reserved2 ",
    )
):
    __slots__ = ()
...
class ACPI_TABLE_APIC_GIC_MSI(
    namedtuple(
        ...
        "Type Length Reserved GICMSIFrameID PhysicalBaseAddress Flags SPICount SPIBase",
    )
):
    __slots__ = ()
...
class ACPI_TABLE_APIC_GIC_REDISTRIBUTOR(
    namedtuple(
        "ACPI_TABLE_APIC_GIC_REDISTRIBUTOR",
        "Type Length Reserved DiscoverRangeBaseAdd DiscoverRangeLength",
    )
):
    __slots__ = ()
...
""".format(
            ...
            self.Length,
            self.Reserved,
            self.DiscoverRangeBaseAdd,
            self.DiscoverRangeLength,
        )
...
------------------------------------------------------------------
  ...
  ACPI_DIS: {}
""".format(
            ("0x{:08X}".format(self.dsdt)),
            ("0x{:016X}".format(self.x_dsdt))
            if self.x_dsdt is not None
            else "Not found",
            "0x{:04X}".format(self.smi),
            "0x{:01X}".format(self.acpi_enable),
            "0x{:01X}".format(self.acpi_disable),
        )
...
class BERT(ACPI_TABLE):
    ...
    def parseSectionType(self, table_content):
        ...
        results = """0x{:08X} 0x{:08X} 0x{:08X} 0x{:08X} - """.format(
            val1, val2, val3, val4
        )
    ...
    def parseTime(self, table_content):
        ...
        return """ {:d}:{:d}:{:d} {:d}/{:d}/{:d}{:d} [m/d/y] {}""".format(
            hours, minutes, seconds, month, day, century, year, precision_str
        )
...
""".format(
    ...
    def parse(self, table_content):
        ...
        bootRegion = self.cs.mem.read_physical_mem(
            self.BootRegionAddr, self.BootRegionLen
        )
...
------------------------------------------------------------------
  ...
  Boot Region - {}
""".format(
            self.BootRegionLen, self.BootRegionAddr, self.BootRegion
        )
...
class EINJ(ACPI_TABLE):
    ...
    def parseInjection(self, table_content):
        errorInjectActions = [
            ...
            "GET_EXECUTE_OPERATION_TIMING",
            "not recognized as valid aciton",
        ]
        injectionInstructions = [
            ...
            "NOOP",
            "not recognized as valid instruction",
        ]
  ...
  Injection Instruction Entry
    ...
    def parseInjectionActionTable(self, table_contents, numInjections):
        ...
        while curInjection < numInjections:
            self.parseInjection(
                table_contents[curInjection * 32 : (curInjection + 1) * 32]
            )
            curInjection += 1
...
class ERST(ACPI_TABLE):
    ...
    def parseInstructionEntry(self, table_content):
        ...
        serializationActions = [
            ...
            "GET_ERROR_LOG_ADDRESS_RANGE_ATTEIBUTES",
...
""".format(
            headerSz, headerSz, reserved, reserved_str, instrCountEntry, instrCountEntry
        )
...
class HEST(ACPI_TABLE):
    ...
    def parseErrEntry(self, table_content):
        ...
        if type == 0:  # Arch Machine Check Execption Structure
            ...
            type == 1
        ):  # Arch Corrected Mach Check Structure or ArchitectureDeferred machine Check Structure
            return self.parseAMCS(table_content, type)
        elif type == 2:  # NMI Error Structure
            return self.parseNMIStructure(table_content)
        elif (
            type == 6 or type == 7 or type == 8
        ):  # PCIe Root Port AER Structure or PCIe Device AER Structure or PCIe Bridge AER Structure
            return self.parsePCIe(table_content, type)
        elif (
            type == 9 or type == 10
        ):  # Generic hardware Error Source Structure or Generic Hardware Error Source version 2
    ...
    def parseNotify(self, table_content):
      ...
      Error Threshold Window                      : {:d} milliseconds""".format(
            errorType,
            typeStr,
            length,
            configWrEn,
    ...
    def machineBankParser(self, table_content):
        ...
        self.resultsStr = self.resultsStr + (
            """Machine Check Error Bank Structure
      Bank Number                                 : 0x{:04X}
      ...
      Misc Register MSR Address	                  : 0x{:04X}{}""".format(
                bankNum,
                clearStatus,
                clearStatus_str,
                statusDataFormat,
                statusDataFormat_str,
                reserved1,
                controlRegMsrAddr,
                controlRegMsrAddr_str,
                controlInitData,
    ...
    def parseAMCES(self, table_content):
        ...
        if (flags & 1) == 1:
            ...
            firmware_first = 0
            firmware_first_str = (
                "System firmware does not handle errors from the source first"
            )
  ...
  Architecture Machine Check Exception Structure
	...
	""".format(
        ...
        while curBankNum < numHardwareBanks:
            self.machineBankParser(
                table_content[40 + curBankNum * 28 : 40 + (curBankNum + 1) * 28]
            )
            curBankNum += 1
	...
	""".format(
        ...
        while currBank < numHardwareBanks:
            self.machineBankParser(
                table_content[48 + currBank * 28 : 48 + (currBank + 1) * 28]
            )
            numHardwareBanks == 1
  ...
  Architecture NMI Error Structure
    ...
    def parsePCIe(self, table_content, type):
        ...
        advancedErrorCapabilitiesAndControl = struct.unpack("<L", table_content[40:44])[
            0
        ]
        if type == 6:
            ...
            extra_str = """
    Root Error Command                            : 0x{:08X}""".format(
                rootErrCommand
            )
            size = 48
        elif type == 8:
    ...
    Secondary Advanced Capabilities and Control   : 0x{:08X}""".format(
                secondaryUncorrErrMask,
                secondaryUncorrErrServ,
                secondaryAdvCapabAndControl,
            )
            size = 56
        else:
        ...
        if firmware_first != 0:
            isFirmware_str = (
                " - This field should be ignored since FIRMWARE_FIRST is set"
            )
  ...
  {}
    ...
    Read Ack Write                                : 0x{:016X}""".format(
                readAckReg_str, readAckPresv, readAckWr
            )
...
class SPMI(ACPI_TABLE):
  ...
  PCI Function Number                                     : 0x{:02X}""".format(
            pciSegGrpNum, pciBusNum, pciDevNum, pciFuncNum
        )

    def parseUID(self, table_content):
        uid = struct.unpack("<L", table_content[0:4])[0]
        return """  UID                                                     : 0x{:02X}""".format(
            uid
        )
...
""".format(
    ...
    def parseProxDomInfoStruct(self, table_contents, num):
        ...
        result = ""
        while val < num:
            result = result + self.parseProx(
                table_contents[22 * val : 22 * (val + 1)], val
            )
            val = val + 1
        return result
    ...
    def parse(self, table_content):
        ...
        proxDomInfoStructStr = self.parseProxDomInfoStruct(
            table_content[20:], maxNumProxDoms
        )
        self.results = """==================================================================
...
""".format(
    ...
    def flushHintAddrStruct(self, tableLen, table_content):
        ...
        while curLine < numFlushHintAddr:
            lineInfo = struct.unpack(
                "<Q", table_content[curLine * 8 + 8 : curLine * 8 + 16]
            )[0]
            lines += """
        Flush Hint Address {}                                     : 0x{:016X} """.format(
                (curLine + 1), lineInfo
            )
            curLine += 1
        return (
...
""".format(
            ...
            reserved,
            serialNum,
            regionFormatInterfaceCode,
            rfic_r1,
            rfic_fif,
            rfic_r2,
            rfic_fcf,
            numBlockControlWindows,
        )
    ...
    def smbiosManagementInfo(self, tableLen, table_content):
        smbios_tables = [
            ...
            "Out-of-Band Remote Address",
            "Boot Integrity Services (BIS) Entry Point",
            "System Boot Information",
            "64-Bit Mmemory Error Information",
            "Management Device",
            ...
            "Mangement Controller Host Interface",
        ]
        reserved = struct.unpack("<L", table_content[4:8])[0]
        curPos = 8
        dataStr = ""
...
""".format(
    ...
    def interleave(self, tableLen, table_content):
        ...
        while curLine < numLinesDescribed:
            lineInfo = struct.unpack(
                "<L", table_content[curLine * 4 + 16 : curLine * 4 + 20]
            )[0]
            lines += """
        Line {:d} Offset                                            : 0x{:08X} ( {:d} bytes ) """.format(
                (curLine + 1), lineInfo, lineInfo
            )
            curLine += 1
        return (
            (curLine - 1) * 4 + 20,
...
""".format(
            ...
            nvdimmControlRegionSz,
            nvdimmRegionSz,
            regionOffset,
            nvdimmPhysicalAddressRegionBase,
            interleaveStructIndex,
            interleaveWays,
            nvdimmStateFlags,
            reserve,
        )
    ...
    def parseSPA(self, tableLen, table_content):
        ...
        nvdimmBlckDataWindowRegionGUID = [
            int("0x91af0530", 16),
            int("0x5d86", 16),
            int("0x470e", 16),
            int("0xa6", 16),
        ...
        addressRangeTypeGUID = [
            addressRangeTypeGUID_1,
            addressRangeTypeGUID_2,
            addressRangeTypeGUID_3,
            addressRangeTypeGUID_4,
            addressRangeTypeGUID_5,
            addressRangeTypeGUID_6,
            addressRangeTypeGUID_7,
            addressRangeTypeGUID_8,
            addressRangeTypeGUID_9,
            addressRangeTypeGUID_10,
            addressRangeTypeGUID_11,
        ]
    ...
    def parseStructures(self, table_content):
        ...
        while notFinished:
            ...
            if tableType == 0:
                ...
                result += self.nvdimmControlRegionStructMark(
                    tableLen, table_content[curPos:]
                )
                curPos += tableLen
            elif tableType == 5:
                result += self.nvdimmBlockDataWindowsRegionStruct(
                    tableLen, table_content[curPos:]
                )
                curPos = curPos + tableLen
            elif tableType == 6:
                sz, result_str = self.flushHintAddrStruct(
                    tableLen, table_content[curPos:]
                )
                result += result_str
                curPos = curPos + tableLen
...
=================================================================="""
  ...
  Data Offset                : {:d}""".format(
        ...
        self.smi = struct.unpack(
            "I", table_content[content_offset : content_offset + 4]
        )[0]
        content_offset += 4
        self.buf_addr = struct.unpack(
            "Q", table_content[content_offset : content_offset + 8]
        )[0]
        content_offset += 8
        self.results += """
  SW SMI NUM                 : {}
  Buffer Ptr Address         : {:X}""".format(
            self.smi, int(self.buf_addr)
        )
...
class WSMT(ACPI_TABLE):
    ...
    def parse(self, table_content):
        ...
        self.comm_buffer_nested_ptr_protection = bool(
            mitigations & WSMT.COMM_BUFFER_NESTED_PTR_PROTECTION
        )
        self.system_resource_protection = bool(
            mitigations & WSMT.SYSTEM_RESOURCE_PROTECTION
        )
...
SYSTEM_RESOURCE_PROTECTION          : {}
    """.format(
            self.fixed_comm_buffers,
            self.comm_buffer_nested_ptr_protection,
...
class GAS:
    def __init__(self, table_content):
        ...
        accessSizeList = [
            ...
            "Dword Access",
            "QWord Access",
            "Not a defined value, check if defined by Address Space ID",
        ]
