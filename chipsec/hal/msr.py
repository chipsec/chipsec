...
class Msr:
    ...
    def get_cpu_thread_count(self):
        ...
        if thread_count is None or thread_count < 0:
            if logger().HAL:
                logger().log(
                    "helper.get_threads_count didn't return anything. Reading MSR 0x35 to find out number of logical CPUs (use CPUID Leaf B instead?)"
                )
            thread_count = self.cs.read_register_field(
                "IA32_MSR_CORE_THREAD_COUNT", "Thread_Count"
            )
    ...
    def get_cpu_core_count(self):
        core_count = self.cs.read_register_field(
            "IA32_MSR_CORE_THREAD_COUNT", "Core_Count"
        )
        return core_count
    ...
    def read_msr(self, cpu_thread_id, msr_addr):
        ...
        if logger().HAL:
            logger().log(
                "[cpu{:d}] RDMSR( 0x{:x} ): EAX = 0x{:08X}, EDX = 0x{:08X}".format(
                    cpu_thread_id, msr_addr, eax, edx
                )
            )
        return (eax, edx)
    ...
    def write_msr(self, cpu_thread_id, msr_addr, eax, edx):
        ...
        if logger().HAL:
            logger().log(
                "[cpu{:d}] WRMSR( 0x{:x} ): EAX = 0x{:08X}, EDX = 0x{:08X}".format(
                    cpu_thread_id, msr_addr, eax, edx
                )
            )
        return
    ...
    def get_IDTR(self, cpu_thread_id):
        (limit, base, pa) = self.get_Desc_Table_Register(
            cpu_thread_id, DESCRIPTOR_TABLE_CODE_IDTR
        )
        if logger().HAL:
            logger().log(
                "[cpu{:d}] IDTR Limit = 0x{:04X}, Base = 0x{:016X}, Physical Address = 0x{:016X}".format(
                    cpu_thread_id, limit, base, pa
                )
            )
    ...
    def get_GDTR(self, cpu_thread_id):
        (limit, base, pa) = self.get_Desc_Table_Register(
            cpu_thread_id, DESCRIPTOR_TABLE_CODE_GDTR
        )
        if logger().HAL:
            logger().log(
                "[cpu{:d}] GDTR Limit = 0x{:04X}, Base = 0x{:016X}, Physical Address = 0x{:016X}".format(
                    cpu_thread_id, limit, base, pa
                )
            )
        return (limit, base, pa)
    ...
    def get_LDTR(self, cpu_thread_id):
        (limit, base, pa) = self.get_Desc_Table_Register(
            cpu_thread_id, DESCRIPTOR_TABLE_CODE_LDTR
        )
        if logger().HAL:
            logger().log(
                "[cpu{:d}] LDTR Limit = 0x{:04X}, Base = 0x{:016X}, Physical Address = 0x{:016X}".format(
                    cpu_thread_id, limit, base, pa
                )
            )
        return (limit, base, pa)
    ...
    def dump_Descriptor_Table(self, cpu_thread_id, code, num_entries=None):
        ...
        logger().log(
            "[cpu{:d}] # of entries    : {:d}".format(cpu_thread_id, total_num)
        )
        logger().log(
            "[cpu{:d}] Contents ({:d} entries):".format(cpu_thread_id, num_entries)
        )
        print_buffer(dt)
        logger().log("--------------------------------------")
        ...
        for i in range(0, num_entries):
            ...
            logger().log(
                "{:03d}  {:04X}:{:016X}  0x{:04X}".format(i, segsel, offset, attr)
            )
    ...
    def IDT(self, cpu_thread_id, num_entries=None):
        ...
        return self.dump_Descriptor_Table(
            cpu_thread_id, DESCRIPTOR_TABLE_CODE_IDTR, num_entries
        )
    ...
    def GDT(self, cpu_thread_id, num_entries=None):
        ...
        return self.dump_Descriptor_Table(
            cpu_thread_id, DESCRIPTOR_TABLE_CODE_GDTR, num_entries
        )
