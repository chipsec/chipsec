...
class TcgPcrEvent(object):
    ...
    def parse(cls, log):
        ...
        if len(event) != event_size:
            logger().warn(
                "[tpm_eventlog] event data length " "does not match the expected size"
            )
...
class SCRTMVersion(TcgPcrEvent):
    ...
    def __str__(self):
        ...
        try:
            ...
            if logger().HAL:
                logger().warn("[tpm_eventlog] CRTM Version is not " "a valid string")
        return _str
