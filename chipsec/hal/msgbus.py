...
class MsgBus(hal_base.HALBase):
    ...
    def __MB_MESSAGE_MCR(self, port, reg, opcode):
        mcr = 0x0
        mcr = self.cs.set_register_field(
            "MSG_CTRL_REG", mcr, "MESSAGE_WR_BYTE_ENABLES", 0xF
        )
        mcr = self.cs.set_register_field(
            "MSG_CTRL_REG", mcr, "MESSAGE_ADDRESS_OFFSET", reg
        )
    ...
    def __MB_MESSAGE_MCRX(self, reg):
        mcrx = 0x0
        mcrx = self.cs.set_register_field(
            "MSG_CTRL_REG_EXT",
            mcrx,
    ...
    def __hide_p2sb(self, hide):
        ...
        if hide:
            self.cs.write_register_field(
                self.p2sbHide["reg"], self.p2sbHide["field"], 1
            )
        else:
            self.cs.write_register_field(
                self.p2sbHide["reg"], self.p2sbHide["field"], 0
            )
        return hidden
    ...
    def msgbus_read_message(self, port, register, opcode):
        ...
        if self.logger.HAL:
            self.logger.log(
                "[msgbus] read: port 0x{:02X} + 0x{:08X} (op = 0x{:02X})".format(
                    port, register, opcode
                )
            )
            self.logger.log(
                "[msgbus]       MCR = 0x{:08X}, MCRX = 0x{:08X}".format(mcr, mcrx)
            )
    ...
    def msgbus_write_message(self, port, register, opcode, data):
        ...
        if self.logger.HAL:
            self.logger.log(
                "[msgbus] write: port 0x{:02X} + 0x{:08X} (op = 0x{:02X}) < data = 0x{:08X}".format(
                    port, register, opcode, data
                )
            )
            self.logger.log(
                "[msgbus]        MCR = 0x{:08X}, MCRX = 0x{:08X}, MDR = 0x{:08X}".format(
                    mcr, mcrx, mdr
                )
            )
    ...
    def msgbus_send_message(self, port, register, opcode, data=None):
        ...
        if self.logger.HAL:
            self.logger.log(
                "[msgbus] message: port 0x{:02X} + 0x{:08X} (op = 0x{:02X})".format(
                    port, register, opcode
                )
            )
            if data is not None:
    ...
    def msgbus_reg_read(self, port, register):
        return self.msgbus_read_message(
            port, register, MessageBusOpcode.MB_OPCODE_REG_READ
        )

    def msgbus_reg_write(self, port, register, data):
        return self.msgbus_write_message(
            port, register, MessageBusOpcode.MB_OPCODE_REG_WRITE, data
        )
    ...
    def mm_msgbus_reg_read(self, port, register):
        ...
        reg_val = self.cs.mmio.read_MMIO_reg_dword(
            mmio_addr, ((port & 0xFF) << 16) | (register & 0xFFFF)
        )
    ...
    def mm_msgbus_reg_write(self, port, register, data):
        ...
        if self.cs.is_register_defined("P2SBC"):
            was_hidden = self.__hide_p2sb(False)
        mmio_addr = self.cs.mmio.get_MMIO_BAR_base_address("SBREGBAR")[0]
        reg_val = self.cs.mmio.write_MMIO_reg_dword(
            mmio_addr, ((port & 0xFF) << 16) | (register & 0xFFFF), data
        )
        if self.cs.is_register_defined("P2SBC") and was_hidden:
            self.__hide_p2sb(True)
        return reg_val
