...
class MsgBus(hal_base.HALBase):
    ...
    def msgbus_read_message( self, port, register, opcode ):
        ...
        if logger().HAL: logger().log( "[msgbus] read: port 0x{:02X} + 0x{:08X} (op = 0x{:02X})".format(port, register, opcode) )
        if logger().VERBOSE: logger().log( "[msgbus]       MCR = 0x{:08X}, MCRX = 0x{:08X}".format(mcr, mcrx) )
    ...
    def msgbus_write_message( self, port, register, opcode, data ):
        ...
        if logger().HAL: logger().log( "[msgbus] write: port 0x{:02X} + 0x{:08X} (op = 0x{:02X}) < data = 0x{:08X}".format(port, register, opcode, data) )
        if logger().VERBOSE: logger().log( "[msgbus]        MCR = 0x{:08X}, MCRX = 0x{:08X}, MDR = 0x{:08X}".format(mcr, mcrx, mdr) )
