...
class Ucode(hal_base.HALBase):
    ...
    def ucode_update_id(self, cpu_thread_id: int) -> int:
        ...
        if (bios_sign_id_lo & IA32_MSR_BIOS_SIGN_ID_STATUS):
            self.logger.log_hal(f'[ucode] CPU{cpu_thread_id:d}: last Microcode update failed (current microcode id = 0x{ucode_update_id:08X})')
        else:
            self.logger.log_hal(f'[ucode] CPU{cpu_thread_id:d}: Microcode update ID = 0x{ucode_update_id:08X}')
    ...
    def update_ucode_all_cpus(self, ucode_file: str) -> bool:
        if not (os.path.exists(ucode_file) and os.path.isfile(ucode_file)):
            self.logger.log_error(f"Ucode file not found: '{ucode_file:.256}'")
            return False
    ...
    def update_ucode(self, cpu_thread_id: int, ucode_file: str) -> int:
        if not (os.path.exists(ucode_file) and os.path.isfile(ucode_file)):
            self.logger.log_error(f"Ucode file not found: '{ucode_file:.256}'")
            return False
        _ucode_buf = self.read_ucode_file(ucode_file)
        return self.load_ucode_update(cpu_thread_id, _ucode_buf)

    def load_ucode_update(self, cpu_thread_id: int, ucode_buf: AnyStr) -> int:
        self.logger.log_hal(f'[ucode] Loading microcode update on CPU{cpu_thread_id:d}')
        self.helper.load_ucode_update(cpu_thread_id, ucode_buf)
        return self.ucode_update_id(cpu_thread_id)

    def dump_ucode_update_header(self, pdb_ucode_buffer: bytes) -> UcodeUpdateHeader:
        if len(pdb_ucode_buffer) < UCODE_HEADER_SIZE:
            self.logger.log('Buffer is too small')
            return None
        ucode_header = UcodeUpdateHeader(*struct.unpack_from('12I', pdb_ucode_buffer))
        self.logger.log_hal(str(ucode_header))
        return ucode_header

    def read_ucode_file(self, ucode_filename: str) -> bytes:
        ucode_buf = read_file(ucode_filename)
        if (ucode_filename.endswith('.pdb')):
            self.logger.log_hal(f"[ucode] PDB file '{ucode_filename:256}' has ucode update header (size = 0x{UCODE_HEADER_SIZE:X})")
            self.dump_ucode_update_header(ucode_buf)
