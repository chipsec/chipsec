...
from chipsec.cfg.parsers.hob_parser import HOBCommands, HobDefinition, HOB_IP_NAME
...
class HOB(hal_base.HALBase):
    ...
    def get_HOB_list(self) -> Tuple[bool, int, List[Hob]]:
        ...
        into the platform hierarchy under the vendor's HOB IP.
    ...
    def _publish_decoded_registers(self) -> None:
        """Add decoded HOB registers to the platform hierarchy as VID.HOB.NAME."""
        ...
        for hob in self.hobs:
            ...
            try:
                ...
                vendor.get_ip(HOB_IP_NAME).add_register(reg.name.upper(), [reg])
            except PlatformConfigError:
                self.logger.log_hal(f'[hob] Could not publish {reg.name} under {reg.vid_str}.{HOB_IP_NAME}')
    ...
    def search_guid_ext_HOBs(self, guid: str) -> List[Hob]:
        """
        Search the HOB list for HOBs containing a specific GUID.

        Args:
            guid: The GUID to search for (as a string).

        Returns:
            A list of HOBs containing the specified GUID.
        """
        guidexthobs = self.search_HOBs_by_type(EFI_HOB_TYPE_GUID_EXTENSION)
        return [hob for hob in guidexthobs if "Name" in hob.fields.keys() and guid == hob.fields['Name']]
    ...
    def get_HOB_registers(self, name: str) -> List[HobRegister]:
        """
        Get the decoded register objects for a declared definition name.

        Args:
            name: Definition name declared in XML (e.g. 'PEI_PCD_DATABASE').

        Returns:
            A list of HobRegister objects, one per matching HOB in the list.
        """
        return [hob.register for hob in self.hobs
                if hob.register is not None and hob.register.name == name]

    def decode_HOBs_by_name(self, name: str) -> List[Dict[str, object]]:
        """
        Decode every HOB in the list matching a declared definition name.
        ...
        """
        registers = self.get_HOB_registers(name)
        if not registers and self.definitions.get_by_name(name) is None:
