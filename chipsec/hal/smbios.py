...
class SMBIOS_2_x_ENTRY_POINT(
    namedtuple(
        "SMBIOS_2_x_ENTRY_POINT",
        "Anchor EntryCs EntryLen MajorVer MinorVer MaxSize EntryRev \
    ...
    def __str__(self):
        return SMBIOS_2_x_FORMAT_STRING.format(
            bytestostring(self.Anchor),
            self.EntryCs,
            self.EntryLen,
            self.MajorVer,
            self.MinorVer,
...
class SMBIOS_3_x_ENTRY_POINT(
    namedtuple(
        "SMBIOS_3_x_ENTRY_POINT",
        "Anchor EntryCs EntryLen MajorVer MinorVer Docrev EntryRev \
    ...
    def __str__(self):
        return SMBIOS_3_x_FORMAT_STRING.format(
            bytestostring(self.Anchor),
            self.EntryCs,
            self.EntryLen,
            self.MajorVer,
...
class SMBIOS_STRUCT_HEADER(namedtuple("SMBIOS_STRUCT_HEADER", "Type Length Handle")):
    ...
    def __str__(self):
        return SMBIOS_STRUCT_HEADER_FORMAT_STRING.format(
            self.Type, self.Length, self.Handle
        )
...
class SMBIOS_BIOS_INFO_2_0(
    namedtuple(
        "SMBIOS_BIOS_INFO_2_0_ENTRY",
        "type length handle vendor_str version_str segment \
    ...
    def __str__(self):
        ...
        return SMBIOS_BIOS_INFO_2_0_FORMAT_STRING.format(
            self.type,
            self.length,
            self.handle,
            ven_str,
...
class SMBIOS_SYSTEM_INFO_2_0(
    namedtuple(
        "SMBIOS_SYSTEM_INFO_2_0_ENTRY",
        "type length handle manufacturer_str product_str \
    ...
    def __str__(self):
        ...
        return SMBIOS_SYSTEM_INFO_2_0_FORMAT_STRING.format(
            self.type, self.length, self.handle, man_str, pro_str, ver_str, ser_str
        )
...
struct_decode_tree = {
    SMBIOS_BIOS_INFO_ENTRY_ID: {
        "class": SMBIOS_BIOS_INFO_2_0,
        "format": SMBIOS_BIOS_INFO_2_0_ENTRY_FMT,
    },
    SMBIOS_SYSTEM_INFO_ENTRY_ID: {
        "class": SMBIOS_SYSTEM_INFO_2_0,
        "format": SMBIOS_SYSTEM_INFO_2_0_ENTRY_FMT,
    },
}
...
class SMBIOS(hal_base.HALBase):
    ...
    def __get_raw_struct(self, table, start_offset):
        ...
        if logger().HAL:
            logger().log(
                "Start Offset: 0x{:04X}, Table Size: 0x{:04X}".format(
                    start_offset, table_len
                )
            )
        if start_offset >= table_len:
            if logger().HAL:
                logger().log(
                    "- Bad table length (table_len): 0x{:04X}".format(table_len)
                )
            return (None, None)
        ...
        if size_left < SMBIOS_STRUCT_HEADER_SIZE:
            if logger().HAL:
                logger().log(
                    "- Table too small (size_left): 0x{:04X}".format(size_left)
                )
            return (None, None)

        # Read the header to determine structure fixed size
        try:
            header = SMBIOS_STRUCT_HEADER(
                *struct.unpack_from(
                    SMBIOS_STRUCT_HEADER_FMT,
        ...
        if str_offset + SMBIOS_STRUCT_TERM_SIZE >= table_len:
            if logger().HAL:
                logger().log(
                    "- Not enough space for termination (str_offset): 0x{:04X}".format(
                        str_offset
                    )
                )
            return (None, None)
        ...
        while tmp_offset + SMBIOS_STRUCT_TERM_SIZE < table_len:
            (value,) = struct.unpack_from(
                SMBIOS_STRUCT_TERM_FMT,
                table[tmp_offset : tmp_offset + SMBIOS_STRUCT_TERM_SIZE],
            )
    ...
    def __validate_ep_2_values(self, pa):
        ...
        try:
            ...
            ep_data = SMBIOS_2_x_ENTRY_POINT(
                *struct.unpack_from(SMBIOS_2_x_ENTRY_POINT_FMT, mem_buffer)
            )
        except:
            if logger().HAL:
        ...
        if ep_data.Anchor != SMBIOS_2_x_SIG:
            ...
            return None
        if not (
            ep_data.EntryLen == SMBIOS_2_x_ENTRY_SIZE
            or ep_data.EntryLen == SMBIOS_2_x_ENTRY_SIZE_OLD
        ):
            if logger().HAL:
    ...
    def __validate_ep_3_values(self, pa):
        ...
        try:
            ...
            ep_data = SMBIOS_3_x_ENTRY_POINT(
                *struct.unpack_from(SMBIOS_3_x_ENTRY_POINT_FMT, mem_buffer)
            )
        except:
            if logger().HAL:
                logger().log("- Memory read failed")
    ...
    def find_smbios_table(self):
        ...
        if ect_found:
            ...
            if SMBIOS_2_x_GUID in ect.VendorTables:
                ...
                if logger().HAL:
                    logger().log(
                        "+ Potential 2.x table address: 0x{:016X}".format(
                            ect.VendorTables[SMBIOS_2_x_GUID]
                        )
                    )
            ...
            if SMBIOS_3_x_GUID in ect.VendorTables:
                ...
                if logger().HAL:
                    logger().log(
                        "+ Potential 3.x table address: 0x{:016X}".format(
                            ect.VendorTables[SMBIOS_3_x_GUID]
                        )
                    )
        ...
        if logger().HAL:
            logger().log(
                "Scanning memory for {:d} signature(s)".format(entries_to_find)
            )
        while pa >= SCAN_LOW_LIMIT:
            ...
            if sig_pa >= pa and self.smbios_2_pa is None:
                if logger().HAL:
                    logger().log(
                        "+ Found SMBIOS 2.x signature @ 0x{:08X}".format(sig_pa)
                    )
            ...
            if sig_pa >= pa and self.smbios_3_pa is None:
                if logger().HAL:
                    logger().log(
                        "+ Found SMBIOS 3.x signature @ 0x{:08X}".format(sig_pa)
                    )
        ...
        if (
            self.smbios_2_ep is not None
            and self.smbios_2_ep.TableAddr != 0
            and self.smbios_2_ep.TableLen != 0
        ):
            self.smbios_2_data = self.cs.mem.read_physical_mem(
                self.smbios_2_ep.TableAddr, self.smbios_2_ep.TableLen
            )
            if self.smbios_2_data is None and logger().HAL:
                logger().log("- Failed to read 32bit SMBIOS data")
        if (
            self.smbios_3_ep is not None
            and self.smbios_3_ep.TableAddr != 0
            and self.smbios_3_ep.MaxSize != 0
        ):
            self.smbios_3_data = self.cs.mem.read_physical_mem(
                self.smbios_3_ep.TableAddr, self.smbios_3_ep.MaxSize
            )
    ...
    def get_raw_structs(self, struct_type=None, force_32bit=False):
        ...
        while next_offset is not None:
            if struct_type is None:
                ...
                header = SMBIOS_STRUCT_HEADER(
                    *struct.unpack_from(
                        SMBIOS_STRUCT_HEADER_FMT, raw_data[:SMBIOS_STRUCT_HEADER_SIZE]
                    )
                )
        ...
        while tmp_offset + index + 1 < raw_data_size:
            ...
            if value == 0:
                ...
                (string,) = struct.unpack_from(
                    "={:d}s".format(index), raw_data[tmp_offset:]
                )
    ...
    def get_decoded_structs(self, struct_type=None, force_32bit=False):
        ...
        for data in structs:
            ...
            if header.Type not in struct_decode_tree:
                if logger().HAL:
                    logger().log(
                        "- Structure {:d} not in decode list".format(header.Type)
                    )
                continue
