...
class IOBAR(hal_base.HALBase):
    ...
    def is_IO_BAR_defined(self, bar_name):
        ...
        try:
            _bar = self.cs.get_io_def(bar_name)
            if _bar is not None:
                if 'register' in _bar:
    ...
    def read_IO_BAR_reg(self, bar_name, offset, size):
        self.logger.log_hal('[iobar] read {} + 0x{:X} ({:d})'.format(bar_name, offset, size))
        (bar_base, bar_size) = self.get_IO_BAR_base_address(bar_name)
        io_port = bar_base + offset
        if offset > bar_size and self.logger.HAL:
            self.logger.hal('offset 0x{:X} is outside {} size (0x{:X})'.format(offset, bar_name, size))
    ...
    def write_IO_BAR_reg(self, bar_name, offset, size, value):
        (bar_base, bar_size) = self.get_IO_BAR_base_address(bar_name)
        self.logger.log_hal('[iobar] write {} + 0x{:X} ({:d}): 0x{:X}'.format(bar_name, offset, size, value))
        io_port = bar_base + offset
        if offset > bar_size and self.logger.HAL:
            self.logger.hal('offset 0x{:X} is oustide {} size (0x{:X})'.format(offset, bar_name, size))
        ...
        for vid in self.cs.Cfg.IO_BARS:
            for dev in self.cs.Cfg.IO_BARS[vid]:
                for _bar_name in self.cs.Cfg.IO_BARS[vid][dev]:
                    ...
                    try:
                        (_base, _size) = self.get_IO_BAR_base_address(bar_name)
                    except CSReadError:
                    ...
                    self.logger.log(' {:12} | {:24} | {:016X} | {:08X} | {:d}   | {}'.format(_bar_name, _s, _base, _size, _en, _bar['desc']))
    ...
    def dump_IO_BAR(self, bar_name, size=1):
        ...
        for i in range(n):
            ...
            self.logger.log('{:+04X}: {:{form}}'.format(i * size, reg, form=fmt))
