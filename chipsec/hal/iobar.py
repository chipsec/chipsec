...
class IOBAR(hal_base.HALBase):
    ...
    def is_IO_BAR_defined(self, bar_name):
        try:
            ...
            if logger().HAL:
                logger().error(
                    "'%s' I/O BAR definition not found in XML config" % bar_name
                )
    ...
    def get_IO_BAR_base_address(self, bar_name):
        ...
        if "register" in bar:
            ...
            if "base_field" in bar:
                ...
                try:
                    ...
                    base = 0
                try:
                    empty_base = self.cs.get_register_field_mask(
                        bar_reg, base_field, preserve_field_position=True
                    )
                except Exception:
                    empty_base = 0
                ...
                try:
                    empty_base = self.cs.get_register_field_mask(
                        bar_reg, preserve_field_position=True
                    )
                except Exception:
                    empty_base = 0
        else:
        ...
        if "fixed_address" in bar and (base == empty_base or base == 0):
            ...
            if logger().HAL:
                logger().log(
                    "[iobar] Using fixed address for {}: 0x{:016X}".format(
                        bar_name, base
                    )
                )
        ...
        if logger().HAL:
            logger().log(
                "[iobar] {}: 0x{:04X} (size = 0x{:X})".format(bar_name, base, size)
            )
        if base == 0:
    ...
    def write_IO_BAR_reg(self, bar_name, offset, size, value):
        ...
        if logger().HAL:
            logger().log(
                "[iobar] write {} + 0x{:X} ({:d}): 0x{:X}".format(
                    bar_name, offset, size, value
                )
            )
        io_port = bar_base + offset
        if offset > bar_size and logger().HAL:
    ...
    def list_IO_BARs(self):
        ...
        for _bar_name in self.cs.Cfg.IO_BARS:
            ...
            logger().log(
                " {:12} | {:14} | {:016X} | {:08X} | {:d}   | {}".format(
                    _bar_name, _s, _base, _size, _en, _bar["desc"]
                )
            )
