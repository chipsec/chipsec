...
class TPM(hal_base.HALBase):
    def __init__(self, cs):
        ...
        self.cs.set_scope({
            ...
            "TPM_ACCESS": "8086.TPM",
            "TPM_STS": "8086.TPM",
            "TPM_DID_VID": "8086.TPM",
            "TPM_RID": "8086.TPM",
            "TPM_INTF_CAPABILITY": "8086.TPM",
            "TPM_INT_ENABLE": "8086.TPM",
        })
        self.TPM_BASE = (int(self.cs.get_mem_def("TPM")["address"], 16))
    ...
    def dump_register(self, register_name, locality):
        register = self.cs.get_register_def(register_name)
        base = int(register['address'], 16)
        offset = int(register['offset'], 16) ^ LOCALITY[locality]
        size = int(register['size'], 16)
        data = self.cs.mmio.read_MMIO_reg(base, offset, size)
        self.log_register_header(register_name, locality)
        self.cs.print_register(register_name, data)
