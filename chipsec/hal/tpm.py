...
LOCALITY = {"0": 0x0000, "1": 0x1000, "2": 0x2000, "3": 0x3000, "4": 0x4000}
...
class TPM_RESPONSE_HEADER(
    namedtuple("TPM_RESPONSE_HEADER", "ResponseTag DataSize ReturnCode")
):
    __slots__ = ()
...
class TPM(hal_base.HALBase):
    ...
    def dump_register(self, register_name, locality):
        self.cs.Cfg.REGISTERS[register_name]["address"] = hex(
            int(self.cs.Cfg.REGISTERS[register_name]["address"], 16)
            ^ LOCALITY[locality]
        )
        ...
        for field in register["FIELDS"]:
            self.logger.log(
                "\t{}{}: {}".format(
                    field,
                    " " * (max_field_len - len(field)),
                    hex(register["FIELDS"][field]["value"]),
                )
            )
