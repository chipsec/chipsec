...
class MMIO(hal_base.HALBase):
    ...
    def read_MMIO_reg(self, bar_base, offset, size=4 ):
        ...
        if logger().VERBOSE: logger().log( '[mmio] 0x{:08X} + 0x{:08X} = 0x{:08X}'.format(bar_base, offset, reg_value) )
        return reg_value
    ...
    def write_MMIO_reg(self, bar_base, offset, value, size=4 ):
        if logger().VERBOSE: logger().log( '[mmio] write 0x{:08X} + 0x{:08X} = 0x{:08X}'.format(bar_base, offset, value) )
    ...
    def is_MMIO_BAR_defined(self, bar_name):
        ...
        if not is_bar_defined:
            if logger().VERBOSE: logger().warn( "'{}' MMIO BAR definition not found/correct in XML config".format(bar_name) )
    ...
    def get_MMIO_BAR_base_address(self, bar_name):
        ...
        if logger().VERBOSE: logger().log( '[mmio] {}: 0x{:016X} (size = 0x{:X})'.format(bar_name,base,size) )
        return base, size
    ...
    def read_mmcfg_reg(self, bus, dev, fun, off, size):
        ...
        if logger().VERBOSE: logger().log( "[mmcfg] reading {:02d}:{:02d}.{:d} + 0x{:02X} (MMCFG + 0x{:08X}): 0x{:08X}".format(bus, dev, fun, off, pciexbar_off, value) )
        if 1 == size:
            return (value & 0xFF)
        elif 2 == size:
            return (value & 0xFFFF)
    ...
    def write_mmcfg_reg(self, bus, dev, fun, off, size, value):
        ...
        if logger().VERBOSE: logger().log( "[mmcfg] writing {:02d}:{:02d}.{:d} + 0x{:02X} (MMCFG + 0x{:08X}): 0x{:08X}".format(bus, dev, fun, off, pciexbar_off, value) )
        return True
