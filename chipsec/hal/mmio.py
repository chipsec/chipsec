...
class MMIO(hal_base.HALBase):
    ...
    def read_MMIO_reg(self, bar_base, offset, size=4, bar_size=None):
        if size > 8:
            if self.logger.HAL:
                self.logger.warn("MMIO read cannot exceed 8")
        reg_value = self.cs.helper.read_mmio_reg(bar_base, size, offset, bar_size)
        if self.logger.HAL:
            self.logger.log(
                "[mmio] 0x{:08X} + 0x{:08X} = 0x{:08X}".format(
                    bar_base, offset, reg_value
                )
            )
        return reg_value
    ...
    def write_MMIO_reg(self, bar_base, offset, value, size=4, bar_size=None):
        if self.logger.HAL:
            self.logger.log(
                "[mmio] write 0x{:08X} + 0x{:08X} = 0x{:08X}".format(
    ...
    def dump_MMIO(self, bar_base, size):
        self.logger.log(
            "[mmio] MMIO register range [0x{:016X}:0x{:016X}+{:08X}]:".format(
                bar_base, bar_base, size
            )
        )
        size -= size % 4
        for offset in range(0, size, 4):
            self.logger.log(
                "+{:08X}: {:08X}".format(offset, self.read_MMIO_reg(bar_base, offset))
            )
    ...
    def is_MMIO_BAR_defined(self, bar_name):
        ...
        if not is_bar_defined:
            if self.logger.HAL:
                self.logger.warn(
                    "'{}' MMIO BAR definition not found/correct in XML config".format(
                        bar_name
                    )
                )
    ...
    def get_MMIO_BAR_base_address(self, bar_name, bus=None):
        ...
        if "register" in bar:
            ...
            if "base_field" in bar:
                ...
                try:
                    base = self.cs.read_register_field(
                        bar_reg, base_field, preserve, bus=_bus
                    )
                except CSReadError:
                    if self.logger.HAL:
                        self.logger.log(
                            "[mmio] Unable to determine MMIO Base.  Using Base = 0x0"
                        )
                    base = 0
                try:
                    reg_mask = self.cs.get_register_field_mask(
                        bar_reg, base_field, preserve
                    )
                except CSReadError:
                    if self.logger.HAL:
                        self.logger.log(
                            "[mmio] Unable to determine MMIO Mask.  Using Mask = 0xFFFF"
                        )
                    reg_mask = 0xFFFF
            else:
                base = self.cs.read_register(bar_reg, bus=_bus)
                reg_mask = self.cs.get_register_field_mask(
                    bar_reg, preserve_field_position=preserve
                )
        ...
        if "fixed_address" in bar and (base == reg_mask or base == 0):
            ...
            if self.logger.HAL:
                self.logger.log(
                    "[mmio] Using fixed address for {}: 0x{:016X}".format(
                        bar_name, base
                    )
                )
        if "mask" in bar:
        ...
        if "align_bits" in bar:
            ...
            _bus = _buses[0] if _buses else None
            start = self.cs.read_register_field(
                bar["base_reg"], bar["base_addr"], bus=_bus
            )
        ...
        if self.logger.HAL:
            self.logger.log(
                "[mmio] {}: 0x{:016X} (size = 0x{:X})".format(bar_name, base, size)
            )
        if base == 0:
            if self.logger.HAL:
    ...
    def is_MMIO_BAR_enabled(self, bar_name, bus=None):
        ...
        if "register" in bar:
            ...
            if "enable_field" in bar:
                ...
                is_enabled = 1 == self.cs.read_register_field(
                    bar_reg, bar_en_field, bus=bus
                )
        else:
            # this method is not preferred (less flexible)
            if bus is not None:
    ...
    def is_MMIO_BAR_programmed(self, bar_name):
        ...
        if "register" in bar:
            ...
            if "base_field" in bar:
                base_field = bar["base_field"]
                base = self.cs.read_register_field(
                    bar_reg, base_field, preserve_field_position=True
                )
            else:
    ...
    def list_MMIO_BARs(self):
        ...
        self.logger.log(
            " MMIO Range   | BUS | BAR Register   | Base             | Size     | En? | Description"
        )
        self.logger.log(
            "--------------------------------------------------------------------------------------"
        )
        for _bar_name in self.cs.Cfg.MMIO_BARS:
            ...
            if "register" in _bar:
                ...
                if bus_data is None:
                    if "bus" in self.cs.get_register_def(_bar["register"]):
                        bus_data = [
                            int(self.cs.get_register_def(_bar["register"])["bus"], 16)
                        ]
                    else:
            ...
            for bus in bus_data:
                ...
                self.logger.log(
                    " {:12} |  {:02X} | {:14} | {:016X} | {:08X} | {:d}   | {}".format(
                        _bar_name, bus or 0, _s, _base, _size, _en, _bar["desc"]
                    )
                )
    ...
    def get_MMCFG_base_address(self):
        ...
        if (
            self.cs.register_has_field("PCI0.0.0_PCIEXBAR", "LENGTH")
            and not self.cs.is_server()
        ):
        ...
        if self.cs.register_has_field("MmioCfgBaseAddr", "BusRange"):
            ...
            if num_buses <= 8:
                ...
                if self.logger.HAL:
                    self.logger.log(
                        "[mmcfg] Unexpected MmioCfgBaseAddr bus range: 0x{:01X}".format(
                            num_buses
                        )
                    )
        if self.logger.HAL:
            self.logger.log(
                "[mmcfg] Memory Mapped CFG Base: 0x{:016X}".format(bar_base)
            )
    ...
    def read_mmcfg_reg(self, bus, dev, fun, off, size):
        ...
        if self.logger.HAL:
            self.logger.log(
                "[mmcfg] reading {:02d}:{:02d}.{:d} + 0x{:02X} (MMCFG + 0x{:08X}): 0x{:08X}".format(
    ...
    def write_mmcfg_reg(self, bus, dev, fun, off, size, value):
        ...
        if self.logger.HAL:
            self.logger.log(
                "[mmcfg] writing {:02d}:{:02d}.{:d} + 0x{:02X} (MMCFG + 0x{:08X}): 0x{:08X}".format(
                    bus, dev, fun, off, pciexbar_off, value
                )
            )
        return True
