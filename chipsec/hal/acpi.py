...
class ACPI_TABLE_HEADER(
    namedtuple(
        "ACPI_TABLE_HEADER",
        "Signature Length Revision Checksum OEMID OEMTableID OEMRevision CreatorID CreatorRevision",
    )
):
    __slots__ = ()
    ...
    def __init__(self, table_content):
        if len(table_content) == ACPI_RSDP_SIZE:
            (
                ...
                self.Revision,
                self.RsdtAddress,
            ) = struct.unpack(ACPI_RSDP_FORMAT, table_content)
        else:
            (
                self.Signature,
                self.Checksum,
                ...
                self.ExtChecksum,
                self.Reserved,
            ) = struct.unpack(ACPI_RSDP_FORMAT + ACPI_RSDP_EXT_FORMAT, table_content)
    ...
    def __str__(self):
        ...
        if hasattr(self, "Length"):
            default += (
                ...
                self.XsdtAddress,
                self.ExtChecksum,
                self.Reserved.encode("hex")
                if (isinstance(self.Reserved, str))
                else self.Reserved.hex(),
            )
        return default
...
class ACPI(hal_base.HALBase):
    ...
    def _find_RSDP_in_EBDA(self):
        ...
        ebda_ptr_addr = 0x40E
        ebda_addr = (
            struct.unpack("<H", self.cs.mem.read_physical_mem(ebda_ptr_addr, 2))[0] << 4
        )
        if ebda_addr > 0x400 and ebda_addr < 0xA0000:
            ...
            if -1 != pos:
                ...
                if rsdp.is_RSDP_valid():
                    if logger().HAL:
                        logger().log(
                            "[acpi] found RSDP in EBDA at: 0x{:016X}".format(rsdp_pa)
                        )
                else:
                    rsdp_pa = None
        return rsdp, rsdp_pa
    ...
    def _find_RSDP_in_legacy_BIOS_segments(self):
        ...
        if -1 != pos:
            ...
            if rsdp.is_RSDP_valid():
                if logger().HAL:
                    logger().log(
                        "[acpi] found RSDP in BIOS E/F segments: 0x{:016X}".format(
                            rsdp_pa
                        )
                    )
            else:
    ...
    def _find_RSDP_in_EFI_config_table(self):
        ...
        if isFound:
            if RSDP_GUID_ACPI2_0 in ect.VendorTables:
                ...
                if logger().HAL:
                    logger().log(
                        "[acpi] ACPI 2.0+ RSDP {{{}}} in EFI Config Table: 0x{:016X}".format(
                            RSDP_GUID_ACPI2_0, rsdp_pa
                        )
                    )
                ...
                if logger().HAL:
                    logger().log(
                        "[acpi] ACPI 1.0 RSDP {{{}}} in EFI Config Table: 0x{:016X}".format(
                            RSDP_GUID_ACPI1_0, rsdp_pa
                        )
                    )
        ...
        if rsdp_pa:
            ...
            if rsdp.is_RSDP_valid():
                if logger().HAL:
                    logger().log(
                        "[acpi] found RSDP in EFI Config Table: 0x{:016X}".format(
                            rsdp_pa
                        )
                    )
            else:
    ...
    def _find_RSDP_in_EFI(self):
        ...
        while pa > CHUNK_SZ:
            ...
            if -1 != pos:
                ...
                if logger().HAL:
                    logger().log(
                        "[acpi] found '{}' signature at 0x{:16X}. Checking if valid RSDP..".format(
                            ACPI_RSDP_SIG, rsdp_pa
                        )
                    )
                rsdp = self.read_RSDP(rsdp_pa)
                if rsdp.is_RSDP_valid():
                    if logger().HAL:
                        logger().log(
                            "[acpi] found RSDP in EFI memory: 0x{:016X}".format(rsdp_pa)
                        )
                    break
            pa -= CHUNK_SZ
        return rsdp, rsdp_pa
    ...
    def get_SDT(self, search_rsdp=True):
        if search_rsdp:
            ...
            if logger().HAL:
                logger().log(
                    "[acpi] found {} at PA: 0x{:016X}".format(
                        "XSDT" if is_xsdt else "RSDT", sdt_pa
                    )
                )
            sdt_header_buf = self.cs.mem.read_physical_mem(
                sdt_pa, ACPI_TABLE_HEADER_SIZE
            )
    ...
    def get_table_list_from_SDT(self, sdt, is_xsdt):
        if logger().HAL:
            logger().log(
                "[acpi] Getting table list from entries in {}".format(
                    "XSDT" if is_xsdt else "RSDT"
                )
            )
    ...
    def get_DSDT_from_FADT(self):
        ...
        if ACPI_TABLE_SIG_FACP in self.tableList:
            ...
            if logger().HAL:
                logger().warn(
                    "Cannot find FADT in {}".format(
                        "XSDT" if ACPI_TABLE_SIG_XSDT in self.tableList else "RSDT"
                    )
                )
            return
        ...
        if dsdt_address_to_use is None:
            ...
            if logger().HAL:
                logger().error(
                    "  X_DSDT address = 0x{}".format("{:16X}".format(x_dsdt_address))
                    if x_dsdt_address is not None
                    else "Not found"
                )
            return
                ...
                logger().log(
                    " - {}: {}".format(
                        ...
                        ", ".join(
                            [
                                ("0x{:016X}".format(addr))
                                for addr in self.tableList[tableName]
                            ]
                        ),
                    )
                )
    ...
    def get_parse_ACPI_table(self, name, isfile=False):
        ...
        return [
            self._parse_table(name, table_header_blob, table_blob)
            for (table_header_blob, table_blob) in acpi_tables
    ...
    def _parse_table_header(self, header):
        acpi_table_hdr = ACPI_TABLE_HEADER(
            *struct.unpack_from(ACPI_TABLE_HEADER_FORMAT, header)
        )
        if logger().HAL:
