...
EFI_VARIABLE_TIME_BASED_AUTHENTICATED_WRITE_ACCESS = (
    0x00000020  # Variable is time based authenticated
)
...
def IS_EFI_VARIABLE_AUTHENTICATED(attr):
    ...
    ) or IS_VARIABLE_ATTRIBUTE(attr, EFI_VARIABLE_TIME_BASED_AUTHENTICATED_WRITE_ACCESS)
...
sig_types = {
    ...
    "3C5766E8-269C-4E34-AA14-ED776E85B3B6": (
    ...
    "67F8444F-8743-48F1-A328-1EAAB8736080": (
    ...
    "0B6E5233-A65C-44C9-9407-D9AB83BFC8BD": (
        "EFI_CERT_SHA224_GUID",
        parse_sha224,
        0x2C,
        "SHA224",
    ),
    "FF3E5307-9FD0-48C9-85F1-8AD56C701E01": (
        "EFI_CERT_SHA384_GUID",
        parse_sha384,
        0x40,
        "SHA384",
    ),
    "093E0FAE-A6C4-4F50-9F1B-D41E2B89C19A": (
    ...
    "7076876e-80c2-4ee6-aad2-28b349a6865b": (
        "EFI_CERT_X509_SHA384_GUID",
        parse_x509_sha384,
        0x50,
        "X509_SHA384",
    ),
    "446dbf63-2502-4cda-bcfa-2465d2b0fe9d": (
    ...
    "452e8ced-dfff-4b8c-ae01-5118862e682c": (
        "EFI_CERT_EXTERNAL_MANAGEMENT_GUID",
        parse_external,
        0x11,
        "EXTERNAL_MANAGEMENT",
    ),
...
def parse_sb_db(db, decode_dir):
    ...
    while (dof + SIGNATURE_LIST_size) < db_size:
        ...
        if SignatureType in sig_types.keys():
            ...
            logger().log_bad(
                "Unknown signature type {}, skipping signature decode.".format(
                    SignatureType
                )
            )
        ...
        if ((sig_size > 0) and (sig_size == SignatureSize)) or (
            (sig_size == 0) and (SignatureSize >= 0x10)
        ):
            sof = 0
            sig_list = db[
                dof
                + SIGNATURE_LIST_size
                + SignatureHeaderSize : dof
                + SignatureListSize
            ]
            ...
            err_str = "Wrong SignatureSize for {} type: 0x{:X}.".format(
...
def parse_auth_var(db, decode_dir):
    ...
    expected_size = struct.unpack(
        AUTH_CERT_DB_LIST_HEAD, db[dof : dof + AUTH_CERT_DB_LIST_HEAD_size]
    )[0]
    ...
    while dof + AUTH_CERT_DB_DATA_size < db_size:
        ...
        sig_file_name = "{}-{}-{:02X}.bin".format(
            vendor_guid, codecs.encode(var_name), nsig
        )
        sig_file_name = os.path.join(decode_dir, sig_file_name)
...
def parse_esal_var(db, decode_dir):
    ...
    while dof + ESAL_SIG_SIZE <= db_size:
        ...
        key_file_name = os.path.join(
            decode_dir, "AuthVarKeyDatabase-cert-{:02X}.bin".format(nsig)
        )
...
class op_io_pci_mem:
    def __init__(
        ...
        if self.count is not None and self.count > 0 and self.buffer is not None:
            ...
            if len(self.buffer) != sz:
                logger().log(
                    "[?] buffer size (0x{:X}) != Width x Count (0x{:X})".format(
                        len(self.buffer), sz
                    )
                )
            else:
                self.values = list(
                    struct.unpack(
                        (
                            "<{:d}{:1}".format(
                                self.count, script_width_formats[self.width]
                            )
                        ),
                        self.buffer,
                    )
    ...
    def __str__(self):
        ...
        if self.unknown is not None:
            str_r += "  Unknown: 0x{:04X}\n".format(self.unknown)
        if self.count is not None:
            str_r += "  Count  : 0x{:X}\n".format(self.count)
        if self.values is not None:
            fmt = "0x{}".format(
                "{{:0{:d}X}}".format(script_width_sizes[self.width] * 2)
            )
            str_r += "  Values : {}\n".format(
                "  ".join([fmt.format(v) for v in self.values])
            )
...
class S3BOOTSCRIPT_ENTRY:
    ...
    def __str__(self):
        ...
        entry_str += (
            "Entry at offset 0x{:04X} (len = 0x{:X}, header len = 0x{:X}):".format(
                self.offset_in_script, self.length, self.header_length
            )
        )
        if self.data:
...
class EFI_TABLE_HEADER(
    namedtuple("EFI_TABLE_HEADER", "Signature Revision HeaderSize CRC32 Reserved")
):
    __slots__ = ()
  ...
  HeaderSize    : 0x{:08X}
  CRC32         : 0x{:08X}
  Reserved      : 0x{:08X}""".format(
            bytestostring(self.Signature),
...
class EFI_SYSTEM_TABLE(
    namedtuple(
        "EFI_SYSTEM_TABLE",
        "FirmwareVendor FirmwareRevision ConsoleInHandle ConIn ConsoleOutHandle ConOut StandardErrorHandle StdErr RuntimeServices BootServices NumberOfTableEntries ConfigurationTable",
    )
):
    __slots__ = ()
...
""".format(
            ...
            self.ConsoleInHandle,
            self.ConIn,
            self.ConsoleOutHandle,
            self.ConOut,
            self.StandardErrorHandle,
            self.StdErr,
            self.RuntimeServices,
            self.BootServices,
            self.NumberOfTableEntries,
...
class EFI_RUNTIME_SERVICES_TABLE(
    namedtuple(
        ...
        "GetTime SetTime GetWakeupTime SetWakeupTime SetVirtualAddressMap ConvertPointer GetVariable GetNextVariableName SetVariable GetNextHighMonotonicCount ResetSystem UpdateCapsule QueryCapsuleCapabilities QueryVariableInfo",
    )
):
    __slots__ = ()
...
""".format(
            ...
            self.GetWakeupTime,
            self.SetWakeupTime,
            self.SetVirtualAddressMap,
            self.ConvertPointer,
            self.GetVariable,
            self.GetNextVariableName,
            self.SetVariable,
            self.GetNextHighMonotonicCount,
            self.ResetSystem,
            self.UpdateCapsule,
            self.QueryCapsuleCapabilities,
            self.QueryVariableInfo,
        )
...
""".format(
            ...
            self.CloseEvent,
            self.CheckEvent,
            self.InstallProtocolInterface,
            self.ReinstallProtocolInterface,
            self.UninstallProtocolInterface,
            self.HandleProtocol,
            self.Reserved,
            self.RegisterProtocolNotify,
            self.LocateHandle,
            self.LocateDevicePath,
            self.InstallConfigurationTable,
            self.LoadImage,
            self.StartImage,
            self.Exit,
            self.UnloadImage,
            self.ExitBootServices,
            self.GetNextMonotonicCount,
            self.Stall,
            ...
            self.OpenProtocol,
            self.CloseProtocol,
            self.OpenProtocolInformation,
            self.ProtocolsPerHandle,
            self.LocateHandleBuffer,
            self.LocateProtocol,
            self.InstallMultipleProtocolInterfaces,
            self.UninstallMultipleProtocolInterfaces,
            self.CalculateCrc32,
            self.CopyMem,
            self.SetMem,
            self.CreateEventEx,
...
class EFI_CONFIGURATION_TABLE:
    ...
    def __str__(self):
        return "Vendor Tables:\n{}".format(
            "".join(
                [
                    "{{{}}} : 0x{:016X}\n".format(vt, self.VendorTables[vt])
...
class EFI_DXE_SERVICES_TABLE(
    namedtuple(
        ...
        "AddMemorySpace AllocateMemorySpace FreeMemorySpace RemoveMemorySpace GetMemorySpaceDescriptor SetMemorySpaceAttributes GetMemorySpaceMap AddIoSpace AllocateIoSpace FreeIoSpace RemoveIoSpace GetIoSpaceDescriptor GetIoSpaceMap Dispatch Schedule Trust ProcessFirmwareVolume",
    )
):
    __slots__ = ()
...
""".format(
            self.AddMemorySpace,
            self.AllocateMemorySpace,
            self.FreeMemorySpace,
            self.RemoveMemorySpace,
            self.GetMemorySpaceDescriptor,
            self.SetMemorySpaceAttributes,
            self.GetMemorySpaceMap,
            self.AddIoSpace,
            self.AllocateIoSpace,
            self.FreeIoSpace,
            self.RemoveIoSpace,
            self.GetIoSpaceDescriptor,
            self.GetIoSpaceMap,
            self.Dispatch,
            self.Schedule,
            self.Trust,
            self.ProcessFirmwareVolume,
        )
...
FRAMEWORK_PEI_SERVICES_REVISION = (FRAMEWORK_PEI_SPECIFICATION_MAJOR_REVISION << 16) | (
    FRAMEWORK_PEI_SPECIFICATION_MINOR_REVISION
)
...
EFI_TABLES = {
    EFI_SYSTEM_TABLE_SIGNATURE: {
        ...
        "fmt": EFI_SYSTEM_TABLE_FMT,
    },
    EFI_RUNTIME_SERVICES_SIGNATURE: {
        "name": "EFI Runtime Services Table",
        "struct": EFI_RUNTIME_SERVICES_TABLE,
        "fmt": EFI_RUNTIME_SERVICES_TABLE_FMT,
    },
    EFI_BOOT_SERVICES_SIGNATURE: {
        "name": "EFI Boot Services Table",
        "struct": EFI_BOOT_SERVICES_TABLE,
        "fmt": EFI_BOOT_SERVICES_TABLE_FMT,
    },
    EFI_DXE_SERVICES_TABLE_SIGNATURE: {
        "name": "EFI DXE Services Table",
        "struct": EFI_DXE_SERVICES_TABLE,
        "fmt": EFI_DXE_SERVICES_TABLE_FMT,
    }
