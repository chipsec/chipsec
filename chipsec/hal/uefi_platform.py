...
NameSize   : 0x{:08X}
DataSize   : 0x{:08X}
""".format( self.guid0, self.guid1, self.guid2, self.guid3[:2].encode('hex').upper(), self.guid3[-6::].encode('hex').upper(), self.StartId, self.State, self.Reserved, self.Attributes, self.NameSize, self.DataSize)
...
NameSize       : 0x{:08X}
DataSize       : 0x{:08X}
""".format( self.guid0, self.guid1, self.guid2, self.guid3[:2].encode('hex').upper(), self.guid3[-6::].encode('hex').upper(), self.StartId, self.State, self.Reserved, self.Attributes, self.MonotonicCount, self.TimeStamp1, self.TimeStamp2, self.PubKeyIndex, self.NameSize, self.DataSize )
...
DataSize   : 0x{:08X}
Unknown    : 0x{:08X}
""".format( self.guid0, self.guid1, self.guid2, self.guid3[:2].encode('hex').upper(), self.guid3[-6::].encode('hex').upper(), self.StartId, self.State, self.Reserved, self.Attributes, self.NameSize, self.DataSize, self.unknown)
...
def decode_s3bs_opcode_def( data ):
    ...
    if S3BootScriptOpcode_MDE.EFI_BOOT_SCRIPT_IO_WRITE_OPCODE == opcode:
        ...
        if logger().VERBOSE: logger().warn( 'Unrecognized opcode {:X}'.format(opcode) )
...
def encode_s3bs_opcode_def( op ):
    ...
    if S3BootScriptOpcode_MDE.EFI_BOOT_SCRIPT_IO_WRITE_OPCODE == op.opcode:
        ...
        if logger().VERBOSE: logger().warn( 'Unrecognized opcode {:X}'.format(op.opcode) )
...
def decode_s3bs_opcode_edkcompat( data ):
    ...
    if   S3BootScriptOpcode_EdkCompat.EFI_BOOT_SCRIPT_IO_WRITE_OPCODE                == opcode or \
        ...
        if logger().UTIL_TRACE or logger().VERBOSE: logger().warn( 'Cannot parse opcode {:X} yet'.format(opcode) )
...
def parse_s3bootscript_entry( s3bootscript_type, script, off, log_script=False ):
    ...
    if S3BootScriptType.EFI_BOOT_SCRIPT_TYPE_EDKCOMPAT == s3bootscript_type:
        ...
        if remaining_len < hdr_length:
            if logger().VERBOSE: logger().warn( 'the script should have at least 0x{:X} bytes to parse next entry'.format(hdr_length) )
            return (0,None)
        ...
        if remaining_len < (hdr_length + 1):
            if logger().VERBOSE: logger().warn( 'the script should have at least 0x{:X} bytes to parse next entry'.format(hdr_length+1) )
            return (0,None)
