...
class VMM:
    ...
    def init(self):
        (self.membuf0_va, self.membuf0_pa) = self.cs.mem.alloc_physical_mem(
            0x2000, 0xFFFFFFFFFFFFFFFF
        )
        (self.membuf1_va, self.membuf1_pa) = (
            self.membuf0_va + 0x1000,
            self.membuf0_pa + 0x1000,
        )
        if self.membuf0_va == 0:
    ...
    def hypercall(
        self, rax, rbx, rcx, rdx, rdi, rsi, r8=0, r9=0, r10=0, r11=0, xmm_buffer=0
    ):
        return self.helper.hypercall(
    ...
    def hypercall64_memory_based(self, hypervisor_input_value, parameters, size=0):
        self.cs.mem.write_physical_mem(
            self.membuf0_pa, len(parameters[:0x1000]), parameters[:0x1000]
        )
        regs = self.helper.hypercall(
            hypervisor_input_value & ~0x00010000, self.membuf0_pa, self.membuf1_pa
        )
        self.output = (
            self.helper.read_physical_mem(self.membuf1_pa, size) if size > 0 else ""
        )
        return regs

    def hypercall64_fast(self, hypervisor_input_value, param0=0, param1=0):
        return self.helper.hypercall(
            hypervisor_input_value | 0x00010000, param0, param1
        )
    ...
    def hypercall64_extended_fast(self, hypervisor_input_value, parameter_block):
        ...
        return self.helper.hypercall(
            hypervisor_input_value | 0x00010000,
            param0,
            param1,
            0,
            0,
    ...
    def dump_EPT_page_tables(self, eptp, pt_fname=None):
        ...
        if logger().HAL:
            logger().log(
                "[vmm] dumping EPT paging hierarchy at EPTP 0x{:08X}...".format(eptp)
            )
        if pt_fname is None:
        ...
        if paging_ept.failure:
            logger().error("could not dump EPT page tables")
...
class VirtIO_Device:
    ...
    def dump_device(self):
        logger().log(
            "\n[vmm] VirtIO device {:02X}:{:02X}.{:01X}".format(
