...
class PortIO:
    ...
    def _read_port(self, io_port, size ):
        ...
        if logger().VERBOSE: logger().log( "[io] IN 0x{:04X}: value = 0x{:08X}, size = 0x{:02X}".format(io_port, value, size) )
        return value

    def _write_port(self, io_port, value, size ):
        if logger().VERBOSE: logger().log( "[io] OUT 0x{:04X}: value = 0x{:08X}, size = 0x{:02X}".format(io_port, value, size) )
