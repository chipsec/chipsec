...
class PortIO:
    ...
    def _read_port(self, io_port, size):
        ...
        if logger().HAL:
            logger().log(
                "[io] IN 0x{:04X}: value = 0x{:08X}, size = 0x{:02X}".format(
                    io_port, value, size
                )
            )
        return value
    ...
    def _write_port(self, io_port, value, size):
        if logger().HAL:
            logger().log(
                "[io] OUT 0x{:04X}: value = 0x{:08X}, size = 0x{:02X}".format(
                    io_port, value, size
                )
            )
    ...
    def read_port_dword(self, io_port):
        ...
        if logger().HAL:
            logger().log(
                "[io] reading dword from I/O port 0x{:04X} -> 0x{:08X}".format(
                    io_port, value
                )
            )
        return value
    ...
    def read_port_word(self, io_port):
        ...
        if logger().HAL:
            logger().log(
                "[io] reading word from I/O port 0x{:04X} -> 0x{:04X}".format(
                    io_port, value
                )
            )
        return value
    ...
    def read_port_byte(self, io_port):
        ...
        if logger().HAL:
            logger().log(
                "[io] reading byte from I/O port 0x{:04X} -> 0x{:02X}".format(
                    io_port, value
                )
            )
        return value
    ...
    def write_port_byte(self, io_port, value):
        if logger().HAL:
            logger().log(
                "[io] writing byte to I/O port 0x{:04X} <- 0x{:02X}".format(
                    io_port, value
                )
            )
    ...
    def write_port_word(self, io_port, value):
        if logger().HAL:
            logger().log(
                "[io] writing word to I/O port 0x{:04X} <- 0x{:04X}".format(
                    io_port, value
                )
            )
    ...
    def write_port_dword(self, io_port, value):
        if logger().HAL:
            logger().log(
                "[io] writing dword to I/O port 0x{:04X} <- 0x{:08X}".format(
                    io_port, value
                )
            )
    ...
    def dump_IO(self, range_base, range_size, size=1):
        ...
        logger().log(
            "[io] I/O register range [0x{:04X}:0x{:04X}+{:04X}]:".format(
