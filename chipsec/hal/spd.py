...
def dram_device_type_name(dram_type):
    dt_name = (
        DRAM_DEVICE_TYPE[dram_type] if dram_type in DRAM_DEVICE_TYPE else "unknown"
    )
    return dt_name
...
""".format(
            ...
            self.TotalBytes,
            self.DeviceType,
            dram_device_type_name(self.DeviceType),
            self.RowAddressCount,
        )
...
class SPD_DDR2(
    namedtuple("SPD_DDR2", "SPDBytes TotalBytes DeviceType RowAddressCount")
):
    __slots__ = ()
...
class SPD_DDR3(
    namedtuple(
        "SPD_DDR3",
        "SPDBytes Revision DeviceType ModuleType ChipSize Addressing Voltages ModuleOrg BusWidthECC FTB MTBDivident MTBDivisor tCKMin RsvdD CASLo CASHi",
    )
):
    __slots__ = ()
...
""".format(
            self.SPDBytes,
            self.Revision,
            SPD_REVISION(self.Revision),
            self.DeviceType,
            dram_device_type_name(self.DeviceType),
            self.ModuleType,
            module_type_name(self.ModuleType),
            self.ChipSize,
            self.Addressing,
            self.Voltages,
            self.ModuleOrg,
...
class SPD_DDR4(
    namedtuple(
        "SPD_DDR4",
        "SPDBytes Revision DeviceType ModuleType Density Addressing PackageType OptFeatures ThernalRefresh OptFeatures1 ReservedA VDD ModuleOrg BusWidthECC ThermSensor ModuleTypeExt",
    )
):
    __slots__ = ()
...
class SPD:
    ...
    def getDRAMDeviceType(self, device=SPD_SMBUS_ADDRESS):
        ...
        if logger().HAL:
            logger().log(
                "[spd][0x{:02X}] DRAM Device Type (byte 2): 0x{:01X}".format(
                    device, dram_type
                )
            )
        return dram_type
    ...
    def getModuleType(self, device=SPD_SMBUS_ADDRESS):
        ...
        if logger().HAL:
            logger().log(
                "[spd][0x{:02X}] Module Type (byte 3): 0x{:01X}".format(
                    device, module_type
                )
            )
        return module_type
    ...
    def isECC(self, device=SPD_SMBUS_ADDRESS):
        ...
        if DRAM_DEVICE_TYPE_DDR3 == device_type:
            ...
            DRAM_DEVICE_TYPE_DDR == device_type or DRAM_DEVICE_TYPE_DDR2 == device_type
        ):
            ...
            if logger().HAL:
                logger().log(
                    "[spd][0x{:02X}] DDR/DDR2 ECC width (byte {:d}): 0x{:02X}".format(
                        device, SPD_OFFSET_DDR_ECC_SDRAM_WIDTH, ecc_width
                    )
                )
        ...
        if logger().HAL:
            if ecc is None:
                logger().log(
                    "[spd][0x{:02X}] Unable to determine ECC support".format(device)
                )
            else:
                logger().log(
                    "[spd][0x{:02X}] ECC is {}supported by the DIMM (byte {:d} = 0x{:02X})".format(
                        device, "" if ecc_supported else "not ", ecc_off, ecc
                    )
                )
    ...
    def isSPDPresent(self, device=SPD_SMBUS_ADDRESS):
        ...
        if logger().HAL:
            logger().log(
                "[spd][0x{:02X}] Detecting SPD.. {}found (DRAM memory type = 0x{:X})".format(
                    device, "" if is_spd_present else "not ", device_type
                )
            )
    ...
    def decode(self, device=SPD_SMBUS_ADDRESS):
        ...
        logger().log(
            "[spd][0x{:02X}] Serial Presence Detect (SPD) EEPROM contents:".format(
                device
            )
        )
