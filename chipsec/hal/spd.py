...
class SPD:
    ...
    def getDRAMDeviceType( self, device=SPD_SMBUS_ADDRESS ):
        ...
        if logger().VERBOSE: logger().log( "[spd][0x{:02X}] DRAM Device Type (byte 2): 0x{:01X}".format(device,dram_type) )
        return dram_type

    def getModuleType( self, device=SPD_SMBUS_ADDRESS ):
        module_type = self.read_byte( SPD_OFFSET_DDR3_MODULE_TYPE, device ) 
        if logger().VERBOSE: logger().log( "[spd][0x{:02X}] Module Type (byte 3): 0x{:01X}".format(device,module_type) )
        return module_type
    ...
    def isECC( self, device=SPD_SMBUS_ADDRESS ):
        ...
        if logger().HAL: logger().log( "[spd][0x{:02X}] ECC is {}supported by the DIMM (byte {:d} = 0x{:02X})".format(device,'' if ecc_supported else 'not ',ecc_off,ecc) )
