...
EFI_CRC32_GUIDED_SECTION_EXTRACTION_PROTOCOL_GUID = UUID(
...
class EFI_MODULE(object):
    ...
    def name(self):
        return "{} {{{}}} {}".format(
            type(self).__name__.encode("ascii", "ignore"),
            str(self.Guid).upper(),
            self.ui_string.encode("ascii", "ignore") if self.ui_string else "",
        )
...
class EFI_FV(EFI_MODULE):
    ...
    def __str__(self):
        schecksum = (
            (
                "{:04X}h ({:04X}h) *** checksum mismatch ***".format(
                    self.Checksum, self.CalcSum
                )
            )
            if self.CalcSum != self.Checksum
            else ("{:04X}h".format(self.Checksum))
        )
        _s = "\n{}{} +{:08X}h {{{}}}: ".format(
            self.indent, type(self).__name__, self.Offset, self.Guid
        )
        _s += "Size {:08X}h, Attr {:08X}h, HdrSize {:04X}h, ExtHdrOffset {:08X}h, Checksum {}".format(
            self.Size, self.Attributes, self.HeaderSize, self.ExtHeaderOffset, schecksum
        )
...
class EFI_SECTION(EFI_MODULE):
    ...
    def name(self):
        return "{} section of binary {{{}}} {}".format(
    ...
    def __str__(self):
        _s = "{}+{:08X}h {}: Type {:02X}h".format(
            self.indent, self.Offset, self.name(), self.Type
        )
        if self.Guid:
            _s += " GUID {{{}}}".format(self.Guid)
        if self.Attributes:
            _s += " Attr {:04X}h".format(self.Attributes)
        if self.DataOffset:
            _s += " DataOffset {:04X}h".format(self.DataOffset)
        if self.Comments:
            _s += "Comments {}".format(self.Comments)
...
def ValidateFwVolumeHeader(
    ZeroVector, FsGuid, FvLength, HeaderLength, ExtHeaderOffset, Reserved, size
):
...
def NextFwVolume(buffer, off=0):
    ...
    while (fof + vf_header_size) < size:
        ...
        (
            ...
            EFI_FIRMWARE_VOLUME_HEADER, buffer[fof : fof + vf_header_size]
        )
        fvh = struct.pack(
...
def GetFvHeader(buffer, off=0):
    ...
    header_size = struct.calcsize(EFI_FIRMWARE_VOLUME_HEADER) + struct.calcsize(
    ...
    (
        ...
        EFI_FIRMWARE_VOLUME_HEADER,
        buffer[off : off + struct.calcsize(EFI_FIRMWARE_VOLUME_HEADER)],
    )
    numblocks, lenblock = struct.unpack(
        EFI_FV_BLOCK_MAP_ENTRY,
        buffer[fof : fof + struct.calcsize(EFI_FV_BLOCK_MAP_ENTRY)],
    )
    if logger().HAL:
        logger().log(
            "{}".format(
    ...
    while not (numblocks == 0 and lenblock == 0):
        ...
        numblocks, lenblock = struct.unpack(
            EFI_FV_BLOCK_MAP_ENTRY, buffer[fof : fof + EFI_FV_BLOCK_MAP_ENTRY_SZ]
        )
    if FvLength != size:
...
def NextFwFile(FvImage, FvLength, fof, polarity):
    ...
    while cur_offset + file_header_size < min(FvLength, len(FvImage)):
        ...
        # Get File size
        if Attributes & FFS_ATTRIB_LARGE_FILE and len(FvImage) > fof + struct.calcsize(
        ...
        if fsize == 0 or fsize > FvLength - cur_offset:
            ...
            break
        # Get next_offset
        update_or_deleted = (bit_set(State, EFI_FILE_MARKED_FOR_UPDATE, polarity)) or (
            bit_set(State, EFI_FILE_DELETED, polarity)
        )
        ...
        if Attributes & FFS_ATTRIB_CHECKSUM:
            fsum = FvChecksum8(
                FvImage[cur_offset + file_header_size : cur_offset + fsize]
            )
        else:
        ...
        res = EFI_FILE(
            ...
            IntegrityCheck,
            fsize,
            FvImage[cur_offset : cur_offset + fsize],
            header_size,
            update_or_deleted,
            CalcSum,
        )
...
def NextFwFileSection(sections, ssize, sof, polarity):
    ...
    while curr_offset + EFI_COMMON_SECTION_HEADER_size < ssize:
        ...
        if (
            Size == 0xFFFFFF
            and (curr_offset + EFI_COMMON_SECTION_HEADER_size + struct.calcsize("I"))
            < ssize
        ):
            Size = struct.unpack(
                "I",
        ...
        res = EFI_SECTION(
            curr_offset, sec_name, Type, sec_body, Header_Size, align(Size, 4)
        )
        break
    return res
...
def assemble_uefi_section(image, uncomressed_size, compression_type):
    ...
    SectionHeader = struct.pack(
        ...
        (SectionSize & 0x00FFFFFF) | (SectionType << 24),
        uncomressed_size,
        compression_type,
    )
...
def assemble_uefi_raw(image):
    return align_image(
        struct.pack("<L", ((len(image) + 4) & 0x00FFFFFF) + (EFI_SECTION_RAW << 24))
        + image
    )
