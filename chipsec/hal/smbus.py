...
class SMBus(hal_base.HALBase):
    ...
    def is_SMBus_supported( self ):
        ...
        if logger().VERBOSE: logger().log( "[smbus] SMBus Controller (DID,VID) = (0x{:04X},0x{:04X})".format(did,vid) )
    ...
    def read_byte( self, target_address, offset ):
        ...
        if logger().VERBOSE: logger().log( "[smbus] read device {:X} off {:X} = {:X}".format(target_address, offset, value) )
        return value
    ...
    def write_byte( self, target_address, offset, value ):
        ...
        #chipsec.chipset.write_register( self.cs, self.smb_reg_address, 0x0 )
        #chipsec.chipset.write_register( self.cs, self.smb_reg_command, 0x0 )
        if logger().VERBOSE: logger().log( "[smbus] write to device {:X} off {:X} = {:X}".format(target_address, offset, value) )
        return True
