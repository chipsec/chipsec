...
class SMBus(hal_base.HALBase):
    ...
    def display_SMBus_info(self):
        if self.logger.HAL:
            self.logger.log(
                "[smbus] SMBus Base Address: 0x{:04X}".format(
                    self.get_SMBus_Base_Address()
                )
            )
        self.get_SMBus_HCFG()
    ...
    def is_SMBus_supported(self):
        ...
        if self.logger.HAL:
            self.logger.log(
                "[smbus] SMBus Controller (DID,VID) = (0x{:04X},0x{:04X})".format(
                    did, vid
                )
            )
        if 0x8086 == vid:
            ...
            self.logger.error(
                "Unknown SMBus Controller (DID,VID) = (0x{:04X},0x{:04X})".format(
                    did, vid
                )
            )
            return False
    ...
    def _wait_for_cycle(self):
        for i in range(SMBUS_POLL_COUNT):
            ...
            if 0 == busy and 1 == intr:
                ...
                if self.logger.HAL:
                    self.logger.error("SMBus transaction failed (FAILED/ERROR bit = 1)")
                return False
            else:
                if self.cs.register_has_field(self.smb_reg_status, "DEV_ERR"):
                    if 1 == self.cs.get_register_field(
                        self.smb_reg_status, sts, "DEV_ERR"
                    ):
                        if self.logger.HAL:
                            self.logger.error(
                                "SMBus device error (invalid cmd, unclaimed cycle or time-out error)"
                            )
                        return False
                if self.cs.register_has_field(self.smb_reg_status, "BUS_ERR"):
                    if 1 == self.cs.get_register_field(
                        ...
                        if self.logger.HAL:
                            self.logger.error("SMBus bus error")
                        return False
        return 0 == busy
    ...
    def read_byte(self, target_address, offset):
        ...
        hst_sa = 0x0
        hst_sa = self.cs.set_register_field(
            self.smb_reg_address, hst_sa, "RW", SMBUS_COMMAND_READ
        )
        hst_sa = self.cs.set_register_field(
            self.smb_reg_address, hst_sa, "Address", target_address, True
        )
        ...
        self.cs.write_register_field(
            self.smb_reg_control, "SMB_CMD", SMBUS_COMMAND_BYTE_DATA
        )
        # send SMBus txn
        ...
        if self.logger.HAL:
            self.logger.log(
                "[smbus] read device {:X} off {:X} = {:X}".format(
                    target_address, offset, value
                )
            )
        return value
    ...
    def write_byte(self, target_address, offset, value):
        ...
        hst_sa = 0x0
        hst_sa = self.cs.set_register_field(
            self.smb_reg_address, hst_sa, "RW", SMBUS_COMMAND_WRITE
        )
        hst_sa = self.cs.set_register_field(
            self.smb_reg_address, hst_sa, "Address", target_address, True
        )
        ...
        if self.logger.HAL:
            self.logger.log(
                "[smbus] write to device {:X} off {:X} = {:X}".format(
                    target_address, offset, value
                )
            )
        return True
    ...
    def read_range(self, target_address, start_offset, size):
        ...
        if self.logger.HAL:
            self.logger.log(
                "[smbus] reading {:d} bytes from device 0x{:X} at offset {:X}".format(
                    size, target_address, start_offset
                )
            )
            # print_buffer( buffer )
        return buffer
    ...
    def write_range(self, target_address, start_offset, buffer):
        ...
        if self.logger.HAL:
            self.logger.log(
                "[smbus] writing {:d} bytes to device 0x{:X} at offset {:X}".format(
                    size, target_address, start_offset
                )
            )
            # print_buffer( buffer )
