...
from chipsec.library.exceptions import CSReadError, MMIOBarConfigError, MMIOBARNotFoundError, PCIConfigError, RegisterNotFoundError
...
class MMMsgBus(hal_base.HALBase):
    ...
    def __get_p2sb_bdf(self, reg: Any) -> Tuple[int, int, int]:
        ...
        Raises:
            PCIConfigError: If the device or function is not defined.
        """
        bus = reg.pci.bus if reg.pci.bus is not None else P2SB_DEFAULT_BUS
        if reg.pci.dev is None or reg.pci.fun is None:
            raise PCIConfigError(f'No device and/or function defined for P2SB register {reg.name}')
