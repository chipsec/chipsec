...
class SpiDescriptor(HALBase):
    def __init__(self, cs):
        super(SpiDescriptor, self).__init__(cs)


    def get_spi_flash_descriptor(self, rom: bytes) -> Tuple[int, bytes]:
    ...
    def get_SPI_master(self, flmstr: int) -> Tuple[int, int, int]:
    ...
    def get_spi_regions(self, fd: bytes) -> Optional[List[Tuple[int, str, int, int, int, bool]]]:
    ...
    def parse_spi_flash_descriptor(self, rom: bytes) -> None:
        if not (isinstance(rom, str) or isinstance(rom, bytes)):
            self.logger.log_error(f'Invalid fd object type {type(rom)}')
            return

        pos = rom.find(SPI_FLASH_DESCRIPTOR_SIGNATURE)
        if (-1 == pos) or (pos < 0x10):
            desc_signature = struct.unpack('=I', SPI_FLASH_DESCRIPTOR_SIGNATURE)[0]
            self.logger.log_error(f'Valid SPI flash descriptor is not found (should have signature {desc_signature:08X})')
            return None

        fd_off = pos - 0x10
        self.logger.log(f'[spi_fd] Valid SPI flash descriptor found at offset 0x{fd_off:08X}')
        ...
        self.logger.log(f'+ 0x0000 Reserved : 0x{fd[0x0:0xF].hex().upper()}')
        self.logger.log(f'+ 0x0010 Signature: 0x{fd_sig:08X}')
        ...
        self.logger.log('========================================================')
        self.logger.log(f'  Flash Component Base Address: 0x{fcba:08X}')
        self.logger.log(f'  Flash Region Base Address   : 0x{frba:08X}')
        ...
        self.logger.log(f'  Flash Master Base Address   : 0x{fmba:08X}')
        self.logger.log(f'  Number of Masters           : {nm:d}')
        self.logger.log(f'  Flash PCH Strap Base Address: 0x{fpsba:08X}')
        self.logger.log(f'  PCH Strap Length            : 0x{psl:X}')
        ...
        #
        self.logger.log('')
        self.logger.log(f'+ 0x{fcba:04X} Component Section:')
        self.logger.log('========================================================')

        flcomp = struct.unpack_from('=I', fd[fcba + 0x0:fcba + 0x4])[0]
        self.logger.log(f'+ 0x{fcba:04X} FLCOMP   : 0x{flcomp:08X}')
        flil = struct.unpack_from('=I', fd[fcba + 0x4:fcba + 0x8])[0]
        self.logger.log(f'+ 0x{fcba + 0x4:04X} FLIL     : 0x{flil:08X}')
        flpb = struct.unpack_from('=I', fd[fcba + 0x8:fcba + 0xC])[0]
        self.logger.log(f'+ 0x{fcba + 0x8:04X} FLPB     : 0x{flpb:08X}')

        #
        # Flash Descriptor Region Section
        #
        self.logger.log('')
        self.logger.log(f'+ 0x{frba:04X} Region Section:')
        self.logger.log('========================================================')

        flregs = self.get_spi_regions(fd)
        ...
        #
        self.logger.log('')
        self.logger.log(f'+ 0x{fmba:04X} Master Section:')
        self.logger.log('========================================================')
        ...
        for m in range(nm):
            ...
            self.logger.log(f'+ 0x{flmstr_off:04X} FLMSTR{m + 1:d}   : 0x{flmstr:08X}')
        ...
        #
        self.logger.log('')
        self.logger.log(f'+ 0x{0xEFC:04X} Flash Descriptor Upper Map:')
        self.logger.log('========================================================')

        flumap1 = struct.unpack_from('=I', fd[0xEFC:0xF00])[0]
        self.logger.log(f'+ 0x{0xEFC:04X} FLUMAP1   : 0x{flumap1:08X}')

        vtba = ((flumap1 & 0x000000FF) << 4)
        vtl = (((flumap1 & 0x0000FF00) >> 8) & 0xFF)
        self.logger.log(f'  VSCC Table Base Address    = 0x{vtba:08X}')
        self.logger.log(f'  VSCC Table Length          = 0x{vtl:02X}')

        #
        # OEM Section
        #
        self.logger.log('')
        self.logger.log(f'+ 0x{0xF00:04X} OEM Section:')
        self.logger.log('========================================================')
