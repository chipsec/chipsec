...
from chipsec.hal.uefi_fv import (
    ...
    EFI_CERT_TYPE_RSA_2048_SHA256_GUID_size,
    EFI_SECTION,
    EFI_FV,
    EFI_FILE,
)
...
type2ext = {
    ...
    EFI_SECTION_TE: "te",
    EFI_SECTION_PIC: "pic",
    EFI_SECTION_COMPATIBILITY16: "c16",
}
...
def decompress_section_data(
    ...
    uncompressed_image = _uefi.decompress_EFI_binary(
        compressed_name, uncompressed_name, compression_type
    )
    if remove_files:
        try:
...
def compress_image(_uefi, image, compression_type):
    ...
    compressed_image = _uefi.compress_EFI_binary(
        precomress_file, compressed_file, compression_type
    )
...
def modify_uefi_region(data, command, guid, uefi_file=""):
    ...
    while fv is not None:
        ...
        if (fv.Guid == EFI_FIRMWARE_FILE_SYSTEM2_GUID) or (
            fv.Guid == EFI_FIRMWARE_FILE_SYSTEM_GUID
        ):
            ...
            while fwbin is not None:
                ...
                if fwbin.Guid == guid:
                    ...
                    if command == CMD_UEFI_FILE_REMOVE:
                        ...
                        logger().log(
                            "Removing UEFI file with GUID={} at offset={:08X}, size change: {:d} bytes".format(
                                fwbin.Guid, CurFileOffset, FvLengthChange
                            )
                        )
                        ...
                        logger().log(
                            "Inserting UEFI file before file with GUID={} at offset={:08X}, size change: {:d} bytes".format(
                                fwbin.Guid, CurFileOffset, FvLengthChange
                            )
                        )
                        data = (
                            data[:CurFileOffset]
                            + uefi_file.ljust(uefi_file_size, "\xFF")
                            + data[CurFileOffset:]
                        )
                    elif command == CMD_UEFI_FILE_INSERT_AFTER:
                        FvLengthChange += uefi_file_size
                        logger().log(
                            "Inserting UEFI file after file with GUID={} at offset={:08X}, size change: {:d} bytes".format(
                                fwbin.Guid, CurFileOffset, FvLengthChange
                            )
                        )
                        data = (
                        ...
                        logger().log(
                            "Replacing UEFI file with GUID={} at offset={:08X}, new size: {:d}, old size: {:d}, size change: {:d} bytes".format(
                                fwbin.Guid,
                                CurFileOffset,
                                len(uefi_file),
                                fwbin.Size,
...
def build_efi_modules_tree(_uefi, fwtype, data, Size, offset, polarity):
    ...
    while sec is not None:
        ...
        sec_fs_name = "sect{:02d}_{}".format(
            secn, "".join(random.choice(string.ascii_lowercase) for _ in range(4))
        )
        ...
        if sec.Type in EFI_SECTIONS_EXE:
            ...
            if len(sec.Image) < sec.HeaderSize + EFI_GUID_DEFINED_SECTION_size:
                ...
                if len(sec.Image) < sec.HeaderSize + EFI_GUID_SIZE:
                    ...
                    guid0 = struct.unpack(
                        EFI_GUID_FMT,
                        sec.Image[sec.HeaderSize : sec.HeaderSize + EFI_GUID_SIZE],
                    )[0]
                    sec.DataOffset = len(sec.Image) - 1
            else:
                guid0, sec.DataOffset, sec.Attributes = struct.unpack(
                    EFI_GUID_DEFINED_SECTION,
                    sec.Image[
                        sec.HeaderSize : sec.HeaderSize + EFI_GUID_DEFINED_SECTION_size
                    ],
                )
            ...
            if sec.Guid == EFI_CRC32_GUIDED_SECTION_EXTRACTION_PROTOCOL_GUID:
                sec.children = build_efi_modules_tree(
                    _uefi,
                    fwtype,
                    sec.Image[sec.DataOffset :],
                    Size - sec.DataOffset,
                    0,
                    polarity,
                )
            elif (
                sec.Guid == LZMA_CUSTOM_DECOMPRESS_GUID
                or sec.Guid == TIANO_DECOMPRESSED_GUID
            ):
                if sec.Guid == LZMA_CUSTOM_DECOMPRESS_GUID:
                    d = decompress_section_data(
                        _uefi,
                        "",
                        sec_fs_name,
                        sec.Image[sec.DataOffset :],
                ...
                if d is None:
                    ...
                    d = decompress_section_data(
                        ...
                        "",
                        sec_fs_name,
                        sec.Image[sec.HeaderSize + EFI_GUID_DEFINED_SECTION_size :],
                        COMPRESSION_TYPE_UNKNOWN,
                        True,
                    )
                if d:
                    sec.children = build_efi_modules_tree(
                        _uefi, fwtype, d, len(d), 0, polarity
                    )
                ...
                if len(sec.Image) > offset:
                    sec.children = build_efi_modules_tree(
                        ...
                        fwtype,
                        sec.Image[offset:],
                        len(sec.Image[offset:]),
                        0,
                        polarity,
                    )
            else:
                sec.children = build_efi_model(
                    _uefi, sec.Image[sec.HeaderSize :], fwtype
                )
...
def build_efi_file_tree(_uefi, fv_img, fwtype):
    ...
    while fwbin is not None:
        fwbin.calc_hashes()
        if fwbin.Type not in (
            EFI_FV_FILETYPE_ALL,
            EFI_FV_FILETYPE_RAW,
            EFI_FV_FILETYPE_FFS_PAD,
        ) or fwbin.State not in (
            EFI_FILE_HEADER_CONSTRUCTION,
            EFI_FILE_HEADER_INVALID,
            EFI_FILE_HEADER_VALID,
        ):
            fwbin.children = build_efi_modules_tree(
                _uefi, fwtype, fwbin.Image, fwbin.Size, fwbin.HeaderSize, polarity
            )
            fv.append(fwbin)
        elif fwbin.Type == EFI_FV_FILETYPE_RAW:
            if fwbin.Name != NVAR_NVRAM_FS_FILE:
                fwbin.children = build_efi_tree(
                    _uefi, fwbin.Image[fwbin.HeaderSize :], fwtype
                )
                fv.append(fwbin)
            else:
                fwbin.isNVRAM = True
...
def build_efi_tree(_uefi, data, fwtype):
    ...
    while fv is not None:
        ...
        # Detect NVRAM firmware volumes
            ...
            try:
                fv.NVRAMType = (
                    identify_EFI_NVRAM(fv.Image) if fwtype is None else fwtype
                )
            except Exception:
...
def FILENAME(mod, parent, modn):
    ...
    if type(mod) == EFI_FILE:
        type_s = (
            FILE_TYPE_NAMES[mod.Type]
            if mod.Type in FILE_TYPE_NAMES.keys()
            else ("UNKNOWN_{:02X}".format(mod.Type))
        )
...
def dump_efi_module(mod, parent, modn, path):
    ...
    write_file(
        mod_path, mod.Image[mod.HeaderSize :] if type(mod) == EFI_SECTION else mod.Image
    )
...
def search_efi_tree(
    modules, search_callback, match_module_types=EFIModuleType.SECTION_EXE, findall=True
):
    matching_modules = []
    for m in modules:
        if search_callback is not None:
            if (
                (
                    (
                        match_module_types & EFIModuleType.SECTION
                        == EFIModuleType.SECTION
                    )
                    and type(m) == EFI_SECTION
                )
                or (
                    (
                        match_module_types & EFIModuleType.SECTION_EXE
                        == EFIModuleType.SECTION_EXE
                    )
                    and (type(m) == EFI_SECTION and m.Type in EFI_SECTIONS_EXE)
                )
                or (
                    (match_module_types & EFIModuleType.FV == EFIModuleType.FV)
                    and type(m) == EFI_FV
                )
                or (
                    (match_module_types & EFIModuleType.FILE == EFIModuleType.FILE)
                    and type(m) == EFI_FILE
                )
            ):
        ...
        if len(m.children) > 0:
            matches = search_efi_tree(
                m.children, search_callback, match_module_types, findall
            )
            if len(matches) > 0:
...
def save_efi_tree(
    _uefi, modules, parent=None, save_modules=True, path=None, save_log=True, lvl=0
):
    mod_dir_path = None
    modules_arr = []
    modn = 0
    for m in modules:
        ...
        attrs = [
            a
            for a in dir(m)
            if not callable(getattr(m, a))
            and not a.startswith("__")
            and (getattr(m, a) is not None)
        ]
        for a in attrs:
            md[a] = getattr(m, a)
        ...
        if save_modules:
            ...
            try:
                md["file_path"] = os.path.relpath(
                    mod_path[4:] if mod_path.startswith("\\\\?\\") else mod_path
                )
            except Exception:
                ...
                if m.isNVRAM:
                    try:
                        if m.NVRAMType is not None:
                            ...
                            nvram = (
                                parent.Image
                                if (type(m) == EFI_FILE and type(parent) == EFI_FV)
                                else m.Image
                            )
                            _uefi.parse_EFI_variables(
                                os.path.join(mod_dir_path, "NVRAM"),
                                nvram,
                                False,
                                m.NVRAMType,
                            )
                        ...
                        logger().warn(
                            "couldn't extract NVRAM in {{{}}} using type '{}'".format(
                                m.Guid, m.NVRAMType
                            )
                        )
        ...
        if len(m.children) > 0:
            md["children"] = save_efi_tree(
                _uefi, m.children, m, save_modules, mod_dir_path, save_log, lvl + 1
            )
        else:
            del md["children"]
...
def parse_uefi_region_from_file(_uefi, filename, fwtype, outpath=None, filetype=[]):
    ...
    write_file(
        ...
        json.dumps(tree_json, indent=2, separators=(",", ": "), cls=UUIDEncoder),
    )
...
def save_efi_tree_filetype(
    modules, parent=None, path=None, lvl=0, filetype=[], save=False
):
    mod_dir_path = path
    modules_arr = []
    modn = 0
    for m in modules:
        ...
        # save children modules
        if len(m.children) > 0:
            md["children"] = save_efi_tree_filetype(
                m.children, m, mod_dir_path, lvl + 1, filetype
            )
        else:
            del md["children"]
