...
HSFCTL_READ_CYCLE = (
    PCH_RCBA_SPI_HSFCTL_FCYCLE_READ << 1
) | PCH_RCBA_SPI_HSFCTL_FCYCLE_FGO
HSFCTL_WRITE_CYCLE = (
    PCH_RCBA_SPI_HSFCTL_FCYCLE_WRITE << 1
) | PCH_RCBA_SPI_HSFCTL_FCYCLE_FGO
HSFCTL_ERASE_CYCLE = (
    PCH_RCBA_SPI_HSFCTL_FCYCLE_ERASE << 1
) | PCH_RCBA_SPI_HSFCTL_FCYCLE_FGO
HSFCTL_JEDEC_CYCLE = (
    PCH_RCBA_SPI_HSFCTL_FCYCLE_JEDEC << 1
) | PCH_RCBA_SPI_HSFCTL_FCYCLE_FGO
HSFCTL_SFDP_CYCLE = (
    PCH_RCBA_SPI_HSFCTL_FCYCLE_SFDP << 1
) | PCH_RCBA_SPI_HSFCTL_FCYCLE_FGO
...
HSFSTS_CLEAR = (
    PCH_RCBA_SPI_HSFSTS_AEL | PCH_RCBA_SPI_HSFSTS_FCERR | PCH_RCBA_SPI_HSFSTS_FDONE
)
...
class SPI(hal_base.HALBase):
    ...
    def get_SPI_MMIO_base(self):
        ...
        if self.logger.HAL:
            self.logger.log(
                "[spi] SPI MMIO base: 0x{:016X} (assuming below 4GB)".format(spi_base)
            )
        return spi_base
    ...
    def display_SPI_Flash_Regions(self):
        ...
        for (region_id, region) in regions.items():
            ...
            self.logger.log(
                "{:d} {:22} | {:08X}  | {:08X} | {:08X} ".format(
                    region_id, name, freg, base, limit
                )
            )
    ...
    def display_SPI_Ranges_Access_Permissions(self):
        ...
        for region_id in regions:
            self.logger.log(
                "  {:12}: {:1d}".format(
                    SPI_REGION[region_id], (0 != bmwag & (1 << region_id))
                )
            )
        ...
        for region_id in regions:
            self.logger.log(
                "  {:12}: {:1d}".format(
                    SPI_REGION[region_id], (0 != bmrag & (1 << region_id))
                )
            )
        ...
        for region_id in regions:
            self.logger.log(
                "  {:12}: {:1d}".format(
                    SPI_REGION[region_id], (0 != brwa & (1 << region_id))
                )
            )
        ...
        for region_id in regions:
            self.logger.log(
                "  {:12}: {:1d}".format(
                    SPI_REGION[region_id], (0 != brra & (1 << region_id))
                )
            )
    ...
    def display_SPI_Protected_Ranges(self):
        ...
        for j in range(5):
            (
                ...
                pr_reg_off,
                pr_reg_value,
            ) = self.get_SPI_Protected_Range(j)
            self.logger.log(
                "PR{:d} ({:02X})     | {:08X} | {:08X} | {:08X} | {:d}   | {:d} ".format(
    ...
    def display_BIOS_write_protection(self):
        if self.cs.is_register_defined("BC"):
            ...
            if self.logger.HAL:
                self.logger.error(
                    "Could not locate the definition of 'BIOS Control' register.."
                )
    ...
    def disable_BIOS_write_protection(self):
        ...
        if smmbwp == 1:
            if self.logger.HAL:
                self.logger.log(
                    "[spi] SMM BIOS write protection (SmmBiosWriteProtection) is enabled"
                )

        if bioswe == 1:
            if self.logger.HAL:
                self.logger.log(
                    "[spi] BIOS write protection (BiosWriteEnable) is not enabled"
                )
            return True
        elif ble == 0:
            if self.logger.HAL:
                self.logger.log(
                    "[spi] BIOS write protection is enabled but not locked. Disabling.."
                )
        else:  # bioswe == 0 and ble == 1
            if self.logger.HAL:
                self.logger.log(
                    "[spi] BIOS write protection is enabled. Attempting to disable.."
                )
        ...
        if self.logger.HAL:
            self.logger.log_important(
                "BIOS write protection is {} (BiosWriteEnable = {:d})".format(
                    "disabled" if bioswe else "still enabled", bioswe
                )
            )
    ...
    def _wait_SPI_flash_cycle_done(self):
        if self.logger.HAL:
            self.logger.log("[spi] wait for SPI cycle ready/done..")
        ...
        if not cycle_done:
            if self.logger.HAL:
                self.logger.log("[spi] SPI cycle still in progress. Waiting 0.1 sec..")
            time.sleep(0.1)
            hsfsts = self.spi_reg_read(self.hsfs_off, 1)
            cycle_done = not (hsfsts & PCH_RCBA_SPI_HSFSTS_SCIP)
        ...
        if cycle_done:
            if self.logger.HAL:
                self.logger.log("[spi] clear FDONE/FCERR/AEL bits..")
            self.spi_reg_write(self.hsfs_off, HSFSTS_CLEAR, 1)
            hsfsts = self.spi_reg_read(self.hsfs_off, 1)
            cycle_done = not (
                (hsfsts & PCH_RCBA_SPI_HSFSTS_AEL)
                or (hsfsts & PCH_RCBA_SPI_HSFSTS_FCERR)
            )

        if self.logger.HAL:
            self.logger.log("[spi] HSFS: 0x{:02X}".format(hsfsts))
    ...
    def _send_spi_cycle(self, hsfctl_spi_cycle_cmd, dbc, spi_fla):
        if self.logger.HAL:
            self.logger.log(
                "[spi] > send SPI cycle 0x{:x} to address 0x{:08X}..".format(
                    hsfctl_spi_cycle_cmd, spi_fla
                )
            )
        ...
        if self.logger.HAL:
            self.logger.log(
                "[spi] SPI cycle GO (DBC <- 0x{:02X}, HSFC <- 0x{:x})".format(
                    dbc, hsfctl_spi_cycle_cmd
                )
            )
        ...
        if not cycle_done:
            ...
            if self.logger.HAL:
                self.logger.log("[spi] < SPI cycle done")
    ...
    def read_spi(self, spi_fla, data_byte_count):
        ...
        if self.logger.UTIL_TRACE or self.logger.HAL:
            self.logger.log(
                "[spi] reading 0x{:x} bytes from SPI at FLA = 0x{:x} (in {:d} 0x{:x}-byte chunks + 0x{:x}-byte remainder)".format(
                    data_byte_count, spi_fla, n, dbc, r
                )
            )
        ...
        for i in range(n):
            if self.logger.HAL:
                self.logger.log(
                    "[spi] reading chunk {:d} of 0x{:x} bytes from 0x{:x}".format(
                        i, dbc, spi_fla + i * dbc
                    )
                )
            if not self._send_spi_cycle(HSFCTL_READ_CYCLE, dbc - 1, spi_fla + i * dbc):
                ...
                for fdata_idx in range(0, dbc // 4):
                    ...
                    if self.logger.HAL:
                        self.logger.log(
                            "[spi] FDATA00 + 0x{:x}: 0x{:x}".format(
                                fdata_idx * 4, dword_value
                            )
                        )
        ...
        if 0 != r:
            if self.logger.HAL:
                self.logger.log(
                    "[spi] reading remaining 0x{:x} bytes from 0x{:x}".format(
                        r, spi_fla + n * dbc
                    )
                )
            if not self._send_spi_cycle(HSFCTL_READ_CYCLE, r - 1, spi_fla + n * dbc):
                ...
                for fdata_idx in range(0, n_dwords):
                    ...
                    if self.logger.HAL:
                        self.logger.log(
                            "[spi] FDATA00 + 0x{:x}: 0x{:08X}".format(
                                fdata_idx * 4, dword_value
                            )
                        )
    ...
    def write_spi(self, spi_fla, buf):
        ...
        if self.logger.UTIL_TRACE or self.logger.HAL:
            self.logger.log(
                "[spi] writing 0x{:x} bytes to SPI at FLA = 0x{:x} (in {:d} 0x{:x}-byte chunks + 0x{:x}-byte remainder)".format(
                    data_byte_count, spi_fla, n, dbc, r
                )
            )
        ...
        for i in range(n):
            if self.logger.UTIL_TRACE or self.logger.HAL:
                self.logger.log(
                    "[spi] writing chunk {:d} of 0x{:x} bytes to 0x{:x}".format(
                        i, dbc, spi_fla + i * dbc
                    )
                )
            dword_value = (
        ...
        if 0 != r:
            if self.logger.UTIL_TRACE or self.logger.HAL:
                self.logger.log(
                    "[spi] writing remaining 0x{:x} bytes to FLA = 0x{:x}".format(
                        r, spi_fla + n * dbc
                    )
                )
            dword_value = 0
    ...
    def get_SPI_SFDP(self):
        ...
        for component in range(0, 2):
            ...
            if sfdp_signature == SFDP_HEADER:
                self.logger.log(
                    "  * Found valid SFDP header for Flash device {:d}".format(
                        component + 1
                    )
                )
                ret = True
                ...
                self.logger.log(
                    "  * Didn't find a valid SFDP header for Flash device {:d}".format(
                        component + 1
                    )
                )
                continue
            ...
            self.logger.log(
                "    SFDP version number: {}.{}".format(
                    sfdp_major_version, sfdp_minor_version
                )
            )
            num_of_param_headers = ((sfdp_data >> 16) & 0xFF) + 1
            self.logger.log(
                "    Number of parameter headers: {:d}".format(num_of_param_headers)
            )
            ...
            self.logger.log(
                "    ** Parameter version number: {}.{}".format(
                    param1_major_version, param1_minor_version
                )
            )
            self.logger.log(
                "    ** Parameter length in double words: {}".format(hex(param1_length))
            )
            if (num_of_param_headers > 1) and self.cs.register_has_field(
                "HSFS", "FCYCLE"
            ):
                self.check_hardware_sequencing()
                ...
                for i in range(1, num_of_param_headers):
                    ...
                    self.logger.log(
                        "    ** Parameter version number:{}.{}".format(
                            param_major_version, param_minor_version
                        )
                    )
                    self.logger.log(
                        "    ** Pramaeter length in double words: {}".format(
                            hex(param_length)
                        )
                    )
                    self.logger.log("    ** Parameter ID: {}".format(hex(id_manuf)))
                    self.logger.log(
                        "    ** Parameter Table Pointer(byte address): {} ".format(
                            hex(param_table_pointer)
                        )
                    )
