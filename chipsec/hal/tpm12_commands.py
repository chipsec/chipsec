...
STARTUP = {1: 0x0100, 2: 0x0200, 3: 0x0300}
...
def pcrread(command_argv):
    ...
    try:
        ...
        return
    command = struct.pack(
        COMMAND_FORMAT, TPM_TAG_RQU_COMMAND, Size, TPM_ORD_PCRREAD, Pcr, 0, 0
    )
    size = Size >> 0x18
    return (command, size)
...
def nvread(command_argv):
    ...
    command = struct.pack(
        ...
        int(command_argv[0], 16),
        int(command_argv[1], 16),
        int(command_argv[2], 16),
    )
    size = Size >> 0x18
    return (command, size)
...
def startup(command_argv):
    ...
    Size = 0x0E000000
    command = struct.pack(
        COMMAND_FORMAT, TPM_TAG_RQU_COMMAND, Size, TPM_ORD_STARTUP, startupType, 0, 0
    )
    size = Size >> 0x18
    return (command, size)
...
def continueselftest(command_argv):
    ...
    Size = 0x0A000000
    command = struct.pack(
        COMMAND_FORMAT, TPM_TAG_RQU_COMMAND, Size, TPM_ORD_CONTINUESELFTEST, 0, 0, 0
    )
    size = Size >> 0x18
    return (command, size)
...
def forceclear(command_argv):
    Size = 0x0A000000
    command = struct.pack(
        COMMAND_FORMAT, TPM_TAG_RQU_COMMAND, Size, TPM_ORD_FORCECLEAR, 0, 0, 0
    )
    size = Size >> 0x18
    return (command, size)
