...
class c_translation(object):
    ...
    def is_translation_exist(self, addr, mask, size):
        return ((addr & mask) in self.translation) and (
            self.translation[addr & mask]["size"] == size
        )
    ...
    def get_mem_range(self, noattr=False):
        ...
        perm = {
            self.translation[a]["addr"]: self.translation[a]
            for a in self.translation.keys()
        }
        mem_range = []
    ...
    def expand_pages(self, exp_size):
        ...
        for virt in self.translation.keys():
            ...
            if size == exp_size:
                for i in range(512):
                    self.add_page(
                        virt + i * pgsize, phys + i * pgsize, SIZE[exp_size], attr
                    )
        return
...
class c_reverse_translation(object):
    def __init__(self, translation):
        ...
        for virt in translation.keys():
            ...
            self.reverse_translation[phys].append(
                {"addr": virt, "size": size, "attr": attr}
            )
    ...
    def get_reverse_translation(self, addr):
        ...
        addr &= ADDR_4KB
        return (
            self.reverse_translation[addr] if addr in self.reverse_translation else []
        )
...
class c_paging_with_2nd_level_translation(c_paging_memory_access):
    ...
    def readmem(self, name, addr, size=4096):
        ...
        return super(c_paging_with_2nd_level_translation, self).readmem(
            name, phys, size
        )
...
class c_paging(c_paging_with_2nd_level_translation, c_translation):
    ...
    def read_entries(self, info, addr, size=8):
        data = self.readmem(
            "{}_{}_0x{:08X}".format(self.name, info, addr), addr, 0x1000
        )
    ...
    def print_info(self, name):
        ...
        for index in range(len(mem_range)):
            i = mem_range[index]
            logger().log(
                "    0x{:013X} - 0x{:013X} {:8d}  {}".format(
                    i[0], i[1] - 1, (i[1] - i[0]) >> 12, i[2]
                )
            )
        ...
        logger().log("\n")
        logger().log(
            "  {} size: {:d} KB, address space: {:d} MB".format(
                name, len(self.pt.keys()) * 4, self.get_address_space() >> 20
            )
        )
        return
        ...
        for addr in addr_list:
            for i in range(len(mem_range)):
                if (mem_range[i][0] <= addr) and (addr < mem_range[i][1]):
                    print(
                        "*** WARNING: PAGE TABLES MISCONFIGURATION  0x{:013X}".format(
                            addr
                        )
                    )
        return
    ...
    def read_pt_and_show_status(self, path, name, ptr):
        ...
        try:
            if logger().HAL:
                logger().log(
                    "[paging] reading {} page tables at 0x{:016X} ...".format(name, ptr)
                )
            # self.out = txt
            ...
            if logger().HAL:
                logger().log(
                    "[paging] size: {:d} KB, address space: {:d} MB".format(
                        len(self.pt.keys()) * 4, self.get_address_space() >> 20
                    )
                )
            ...
            self.failure = True
            if logger().HAL:
                logger().error("    ERROR: Invalid {} Page Tables!".format(name))
        # finally:
        #    #txt.close()
        return
...
class c_ia32e_page_tables(c_4level_page_tables):
    ...
    def get_attr(self, entry):
        ...
        US_DESC = ["S", "U"]
        return (
            RW_DESC[self.get_field(entry, self.RW)]
            + " "
            + US_DESC[self.get_field(entry, self.US)]
        )
...
class c_extended_page_tables(c_4level_page_tables):
    ...
    def get_attr(self, entry):
        ...
        return (
            XWR_DESC[self.get_field(entry, self.XWR)]
            + " "
            + MEM_DESC[self.get_field(entry, self.MEM_TYPE)]
        )
...
class c_vtd_page_tables(c_extended_page_tables):
    ...
    def print_context_entry(self, source_id, cee):
        if self.get_field(cee[0], self.CE_LO_P):
            ...
            logger().log(
                "  {:02X}:{:02X}.{:X}  DID: {:02X}  AVAIL: {:X}  AW: {:X}  T: {:X}  FPD: {:X}  SLPTPTR: {:016X}".format(
                    *info
                )
            )
        return
