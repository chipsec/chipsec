...
class IOMMU(hal_base.HALBase):
    ...
    def __init__(self, cs):
        ...
        snames = ["", "_VER", "_GSTS", "_RTADDR", "_IRTA", "_PMEN",
                  "_PLMBASE", "_PLMLIMIT", "_PHMBASE", "_PHMLIMIT",
                  "_CAP", "_ECAP", "_FSTS", "_FRCDL", "_FRCDH", "_ICS"]
        scope = {}
        for prefix in ["VTBAR", "GFXVTBAR"]:
            for name in snames:
                if self.cs.is_server():
                    scope[prefix + name] = "8086.MemMap_VTd"
                else:
                    scope[prefix + name] = "8086.HOSTCTL"
        self.cs.set_scope(scope)

    def get_IOMMU_Base_Address(self, iommu_engine):
        if iommu_engine in IOMMU_ENGINES:
            vtd_base_name = IOMMU_ENGINES[iommu_engine]
        else:
            raise IOMMUError('IOMMUError: unknown IOMMU engine 0x{:X}'.format(iommu_engine))

        if self.cs.mmio.is_MMIO_BAR_defined(vtd_base_name):
            (base, size) = self.cs.mmio.get_MMIO_BAR_base_address(vtd_base_name)
        else:
    ...
    def is_IOMMU_Engine_Enabled(self, iommu_engine):
        if iommu_engine in IOMMU_ENGINES:
            ...
            raise IOMMUError('IOMMUError: unknown IOMMU engine 0x{:X}'.format(iommu_engine))
        return self.cs.mmio.is_MMIO_BAR_defined(vtd_base_name) and self.cs.mmio.is_MMIO_BAR_enabled(vtd_base_name)
