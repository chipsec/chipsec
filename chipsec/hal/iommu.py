...
IOMMU_ENGINES = {IOMMU_ENGINE_GFX: "GFXVTBAR", IOMMU_ENGINE_DEFAULT: "VTBAR"}
...
class IOMMU(hal_base.HALBase):
    ...
    def get_IOMMU_Base_Address(self, iommu_engine):
        if iommu_engine in IOMMU_ENGINES:
            ...
            raise IOMMUError(
                "IOMMUError: unknown IOMMU engine 0x{:X}".format(iommu_engine)
            )
        ...
        if self.mmio.is_MMIO_BAR_defined(vtd_base_name):
            ...
            raise IOMMUError(
                "IOMMUError: IOMMU BAR {} is not defined in the config".format(
                    vtd_base_name
                )
            )
        return base
    ...
    def is_IOMMU_Engine_Enabled(self, iommu_engine):
        ...
        return self.mmio.is_MMIO_BAR_defined(
            vtd_base_name
        ) and self.mmio.is_MMIO_BAR_enabled(vtd_base_name)
    ...
    def set_IOMMU_Translation(self, iommu_engine, te):
        return self.cs.write_register_field(
            IOMMU_ENGINES[iommu_engine] + "_GCMD", "TE", te
        )
    ...
    def dump_IOMMU_configuration(self, iommu_engine):
        ...
        self.logger.log(
            "Version                   : {:X}.{:X}".format(ver_max, ver_min)
        )
    ...
    def dump_IOMMU_page_tables(self, iommu_engine):
        ...
        rtaddr_rta = self.cs.get_register_field(
            vtd + "_RTADDR", rtaddr_reg, "RTA", True
        )
        rtaddr_rtt = self.cs.get_register_field(vtd + "_RTADDR", rtaddr_reg, "RTT")
        # rtaddr_rta = self.cs.read_register_field( vtd + '_RTADDR', 'RTA', True )
        # rtaddr_rtt = self.cs.read_register_field( vtd + '_RTADDR', 'RTT' )
        self.logger.log(
            "[iommu] Root Table Address/Type: 0x{:016X}/{:X}".format(
                rtaddr_rta, rtaddr_rtt
            )
        )
        ...
        self.logger.log(
            "[iommu] PASID / ECS            : {:X} / {:X}".format(pasid, ecs)
        )

        if 0xFFFFFFFFFFFFFFFF != rtaddr_reg:
            if te:
                self.logger.log(
                    "[iommu] dumping VT-d page table hierarchy at 0x{:016X} (vtd_context_{:08X})..".format(
                        rtaddr_rta, rtaddr_rta
                    )
                )
                paging_vtd = paging.c_vtd_page_tables(self.cs)
                paging_vtd.read_vtd_context(
                    "vtd_context_{:08X}".format(rtaddr_rta), rtaddr_rta
                )
                self.logger.log(
                    "[iommu] total VTd domains: {:d}".format(len(paging_vtd.domains))
                )
                for domain in paging_vtd.domains:
                    paging_vtd.read_pt_and_show_status(
                        "vtd_{:08X}".format(domain), "VTd", domain
                    )
                    # if paging_vtd.failure: self.logger.error( "couldn't dump VT-d page tables" )
            else:
                self.logger.log(
                    "[iommu] translation via VT-d engine '{}' is not enabled".format(
                        iommu_engine
                    )
                )
        else:
