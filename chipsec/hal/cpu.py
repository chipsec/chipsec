...
class CPU(hal_base.HALBase):
    ...
    def dump_page_tables(self, cr3, pt_fname=None):
        ...
        self.logger.log_hal('[cpu] dumping paging hierarchy at physical base (CR3) = 0x{:08X}...'.format(cr3))
        if pt_fname is None:
        ...
        if hpt.failure:
            self.logger.error('could not dump page tables')

    def dump_page_tables_all(self):
        for tid in range(self.cs.msr.get_cpu_thread_count()):
            cr3 = self.read_cr(tid, 3)
            self.logger.log_hal('[cpu{:d}] found paging hierarchy base (CR3): 0x{:08X}'.format(tid, cr3))
