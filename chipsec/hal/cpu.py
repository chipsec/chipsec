...
class CPU(hal_base.HALBase):
    ...
    def read_cr(self, cpu_thread_id, cr_number):
        ...
        if logger().HAL:
            logger().log(
                "[cpu{:d}] read CR{:d}: value = 0x{:08X}".format(
                    cpu_thread_id, cr_number, value
                )
            )
        return value
    ...
    def write_cr(self, cpu_thread_id, cr_number, value):
        if logger().HAL:
            logger().log(
                "[cpu{:d}] write CR{:d}: value = 0x{:08X}".format(
                    cpu_thread_id, cr_number, value
                )
            )
    ...
    def cpuid(self, eax, ecx):
        ...
        if logger().HAL:
            logger().log(
                "[cpu] CPUID out: EAX=0x{:08X}, EBX=0x{:08X}, ECX=0x{:08X}, EDX=0x{:08X}".format(
                    eax, ebx, ecx, edx
                )
            )
    ...
    def check_vmm(self):
        ...
        if ecx & 0x80000000:
            ...
            if is_xen:
                return VMM_XEN
            is_hyperv = (
                (ebx == 0x7263694D) and (ecx == 0x666F736F) and (edx == 0x76482074)
            )
            if is_hyperv:
                return VMM_HYPER_V
            is_vmware = (
                (ebx == 0x61774D56) and (ecx == 0x4D566572) and (edx == 0x65726177)
            )
            if is_vmware:
                return VMM_VMWARE
    ...
    def get_TSEG(self):
        if self.cs.is_server():
            ...
            tseg_base = self.cs.read_register_field(
                "TSEG_BASE", "base", preserve_field_position=True
            )
            tseg_limit = self.cs.read_register_field(
                "TSEG_LIMIT", "limit", preserve_field_position=True
            )
            tseg_limit += 0xFFFFF
        else:
            # TSEG base is in TSEGMB, TSEG limit is BGSM - 1
            tseg_base = self.cs.read_register_field(
                "PCI0.0.0_TSEGMB", "TSEGMB", preserve_field_position=True
            )
            bgsm = self.cs.read_register_field(
                "PCI0.0.0_BGSM", "BGSM", preserve_field_position=True
            )
            tseg_limit = bgsm - 1
    ...
    def dump_page_tables(self, cr3, pt_fname=None):
        ...
        if logger().HAL:
            logger().log(
                "[cpu] dumping paging hierarchy at physical base (CR3) = 0x{:08X}...".format(
                    cr3
                )
            )
        if pt_fname is None:
        ...
        if hpt.failure:
            logger().error("could not dump page tables")
    ...
    def dump_page_tables_all(self):
        for tid in range(self.cs.msr.get_cpu_thread_count()):
            ...
            if logger().HAL:
                logger().log(
                    "[cpu{:d}] found paging hierarchy base (CR3): 0x{:08X}".format(
                        tid, cr3
                    )
                )
