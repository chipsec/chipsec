...
class EC(hal_base.HALBase):
    ...
    def read_range(self, start_offset, size):
        ...
        if self.logger.HAL:
            self.logger.log(
                "[ec] read EC memory from offset {:X} size {:X}:".format(
                    start_offset, size
                )
            )
            print_buffer(buffer)
    ...
    def write_range(self, start_offset, buffer):
        ...
        if self.logger.HAL:
            self.logger.log(
                "[ec] write EC memory to offset {:X} size {:X}:".format(
                    start_offset, size
                )
            )
            print_buffer(buffer)
    ...
    def read_idx(self, offset):
        ...
        if self.logger.HAL:
            self.logger.log(
                "[ec] index read: offset 0x{:02X} > 0x{:02X}:".format(offset, value)
            )
        return value
    ...
    def write_idx(self, offset, value):
        if self.logger.HAL:
            self.logger.log(
                "[ec] index write: offset 0x{:02X} < 0x{:02X}:".format(offset, value)
            )
        self.cs.io.write_port_byte(IO_PORT_EC_INDEX_ADDRL, offset & 0xFF)
        self.cs.io.write_port_byte(IO_PORT_EC_INDEX_ADDRH, (offset >> 8) & 0xFF)
