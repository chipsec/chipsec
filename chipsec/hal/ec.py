...
class EC(hal_base.HALBase):
    ...
    def read_range(self, start_offset, size):
        ...
        for i in range(size):
            ...
            if start_offset + i < 0x100:
                buffer[i] = self.read_memory(start_offset + i)
            else:
                buffer[i] = self.read_memory_extended(start_offset + i)

        self.logger.log_hal( "[ec] read EC memory from offset {:X} size {:X}:".format(start_offset, size) )
        if self.logger.HAL:
            print_buffer(buffer)
        return buffer
    ...
    def write_range(self, start_offset, buffer):
        ...
        self.logger.log_hal("[ec] write EC memory to offset {:X} size {:X}:".format(start_offset, size))
        if self.logger.HAL:
            print_buffer(buffer)
        return True
