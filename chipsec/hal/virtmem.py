...
class VirtMemory(hal_base.HALBase):
    ...
    def read_virtual_mem_dword(self, virt_address):
        ...
        if logger().HAL:
            logger().log(
                "[mem] dword at VA = 0x{:016X}: 0x{:08X}".format(virt_address, value)
            )
        return value
    ...
    def read_virtual_mem_word(self, virt_address):
        ...
        if logger().HAL:
            logger().log(
                "[mem] word at VA = 0x{:016X}: 0x{:04X}".format(virt_address, value)
            )
        return value
    ...
    def read_virtual_mem_byte(self, virt_address):
        ...
        if logger().HAL:
            logger().log(
                "[mem] byte at VA = 0x{:016X}: 0x{:02X}".format(virt_address, value)
            )
        return value
    ...
    def write_virtual_mem(self, virt_address, length, buf):
        if logger().HAL:
            logger().log(
                "[mem] buffer len = 0x{:X} to VA = 0x{:016X}".format(
                    length, virt_address
                )
            )
            print_buffer(buf)
    ...
    def write_virtual_mem_dword(self, virt_address, dword_value):
        if logger().HAL:
            logger().log(
                "[mem] dword to VA = 0x{:016X} <- 0x{:08X}".format(
                    virt_address, dword_value
                )
            )
        phys_address = self.va2pa(virt_address)
        return self.helper.write_physical_mem(
            phys_address, 4, struct.pack("I", dword_value)
        )

    def write_virtual_mem_word(self, virt_address, word_value):
        if logger().HAL:
            logger().log(
                "[mem] word to VA = 0x{:016X} <- 0x{:04X}".format(
                    virt_address, word_value
                )
            )
        phys_address = self.va2pa(virt_address)
        return self.helper.write_physical_mem(
            phys_address, 2, struct.pack("H", word_value)
        )

    def write_virtual_mem_byte(self, virt_address, byte_value):
        if logger().HAL:
            logger().log(
                "[mem] byte to VA = 0x{:016X} <- 0x{:02X}".format(
                    virt_address, byte_value
                )
            )
        phys_address = self.va2pa(virt_address)
        return self.helper.write_physical_mem(
            phys_address, 1, struct.pack("B", byte_value)
        )
