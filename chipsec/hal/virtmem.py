...
class VirtMemory(hal_base.HALBase):
    ...
    def read_virtual_mem(self, virt_address, length):
        self.logger.log_hal("[mem] 0x{:016X}".format(virt_address))
    ...
    def read_virtual_mem_dword(self, virt_address):
        ...
        self.logger.log_hal('[mem] dword at VA = 0x{:016X}: 0x{:08X}'.format(virt_address, value))
        return value
    ...
    def read_virtual_mem_word(self, virt_address):
        ...
        self.logger.log_hal('[mem] word at VA = 0x{:016X}: 0x{:04X}'.format(virt_address, value))
        return value
    ...
    def read_virtual_mem_byte(self, virt_address):
        ...
        self.logger.log_hal('[mem] byte at VA = 0x{:016X}: 0x{:02X}'.format(virt_address, value))
        return value

    # Writing virtual memory

    def write_virtual_mem(self, virt_address, length, buf):
        self.logger.log_hal('[mem] buffer len = 0x{:X} to VA = 0x{:016X}'.format(length, virt_address))
        if self.logger.HAL:
            print_buffer(buf)
        phys_address = self.va2pa(virt_address)
    ...
    def write_virtual_mem_dword(self, virt_address, dword_value):
        self.logger.log_hal('[mem] dword to VA = 0x{:016X} <- 0x{:08X}'.format(virt_address, dword_value))
        phys_address = self.va2pa(virt_address)
    ...
    def write_virtual_mem_word(self, virt_address, word_value):
        self.logger.log_hal('[mem] word to VA = 0x{:016X} <- 0x{:04X}'.format(virt_address, word_value))
        phys_address = self.va2pa(virt_address)
    ...
    def write_virtual_mem_byte(self, virt_address, byte_value):
        self.logger.log_hal('[mem] byte to VA = 0x{:016X} <- 0x{:02X}'.format(virt_address, byte_value))
        phys_address = self.va2pa(virt_address)
    ...
    def free_virtual_mem(self, virt_address):
        ...
        self.logger.log_hal('[mem] Deallocated : VA = 0x{:016X}'.format(virt_address))
