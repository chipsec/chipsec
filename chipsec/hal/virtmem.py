...
class VirtMemory(hal_base.HALBase):
    ...
    def read_virtual_mem( self, virt_address, length ):
        logger().log_hal("[mem] 0x{:016X}".format(virt_address))
    ...
    def read_virtual_mem_dword( self, virt_address ):
        ...
        logger().log_hal( '[mem] dword at VA = 0x{:016X}: 0x{:08X}'.format(virt_address, value) )
        return value
    ...
    def read_virtual_mem_word( self, virt_address ):
        ...
        logger().log_hal( '[mem] word at VA = 0x{:016X}: 0x{:04X}'.format(virt_address, value) )
        return value
    ...
    def read_virtual_mem_byte( self, virt_address ):
        ...
        logger().log_hal( '[mem] byte at VA = 0x{:016X}: 0x{:02X}'.format(virt_address, value) )
        return value
    ...
    def write_virtual_mem_dword( self, virt_address, dword_value ):
        logger().log_hal( '[mem] dword to VA = 0x{:016X} <- 0x{:08X}'.format(virt_address, dword_value) )
        phys_address = self.va2pa(virt_address)
    ...
    def write_virtual_mem_word( self, virt_address, word_value ):
        logger().log_hal( '[mem] word to VA = 0x{:016X} <- 0x{:04X}'.format(virt_address, word_value) )
        phys_address = self.va2pa(virt_address)
    ...
    def write_virtual_mem_byte( self, virt_address, byte_value ):
        logger().log_hal( '[mem] byte to VA = 0x{:016X} <- 0x{:02X}'.format(virt_address, byte_value) )
        phys_address = self.va2pa(virt_address)
    ...
    def alloc_virtual_mem( self, length, max_phys_address=0xFFFFFFFFFFFFFFFF ):
        ...
        logger().log_hal( '[mem] Allocated: PA = 0x{:016X}, VA = 0x{:016X}'.format(pa, va) )
        return (va, pa)

    def va2pa( self, va ):
        (pa, error_code) = self.helper.va2pa( va )
        logger().log_hal( '[mem] VA (0x{:016X}) -> PA (0x{:016X})'.format(va, pa) )
        if error_code:
            logger().log_hal( '[mem] Looks like VA (0x{:016X}) not mapped'.format(va) )
            return
        return pa
    ...
    def free_virtual_mem(self, virt_address):
        ...
        logger().log_hal( '[mem] Deallocated : VA = 0x{:016X}'.format(virt_address) )
