...
from chipsec.hal import hal_base, mmio, spi
from chipsec.lib import uefi_platform
from chipsec.lib.uefi_common import EFI_VENDOR_TABLE, EFI_VENDOR_TABLE_SIZE, EFI_VENDOR_TABLE_FORMAT, EFI_TABLE_HEADER_SIZE, EFI_TABLE_HEADER, EFI_TABLES,  MAX_EFI_TABLE_SIZE
from chipsec.lib.uefi_common import S3BootScriptOpcode, S3_BOOTSCRIPT_VARIABLES, parse_efivar_file, EFI_REVISIONS, AUTH_SIG_VAR, ESAL_SIG_VAR
from chipsec.lib.uefi_common import EFI_VARIABLE_AUTHENTICATED_WRITE_ACCESS, EFI_VARIABLE_TIME_BASED_AUTHENTICATED_WRITE_ACCESS, EFI_VARIABLE_APPEND_WRITE, EFI_VARIABLE_NON_VOLATILE
from chipsec.lib.uefi_common import EFI_VARIABLE_BOOTSERVICE_ACCESS, EFI_VARIABLE_RUNTIME_ACCESS, EFI_VARIABLE_HARDWARE_ERROR_RECORD, SECURE_BOOT_SIG_VAR
from chipsec.lib.uefi_common import IS_VARIABLE_ATTRIBUTE, EFI_TABLE_HEADER_FMT, EFI_SYSTEM_TABLE_SIGNATURE, EFI_RUNTIME_SERVICES_SIGNATURE, EFI_BOOT_SERVICES_SIGNATURE
from chipsec.lib.uefi_common import EFI_DXE_SERVICES_TABLE_SIGNATURE, EFI_CONFIGURATION_TABLE, ACPI_VARIABLE_SET_STRUCT_SIZE
...
class UEFI(hal_base.HALBase):
    ...
    def find_s3_bootscript( self ):
        ...
        logger().log_hal( "[uefi] searching for EFI variable(s): " + str(S3_BOOTSCRIPT_VARIABLES) )
        ...
        for efivar_name in efivars:
            ...
            if efivar_name in S3_BOOTSCRIPT_VARIABLES:
                ...
                logger().log_hal( '[uefi] ACPI Boot-Script table base = 0x{:016X}'.format(AcpiBootScriptTable) )
                found   = True
    ...
    def get_s3_bootscript( self, log_script=False ):
        ...
        logger().log_hal( '[uefi] Found {:d} S3 resume boot-scripts'.format(len(bootscript_PAs)) )

        for bootscript_pa in bootscript_PAs:
            if (bootscript_pa == 0): continue
            logger().log_hal( '[uefi] S3 resume boot-script at 0x{:016X}'.format(bootscript_pa) )
            #
    ...
    def delete_EFI_variable( self, name, guid ):
        logger().log_hal( '[uefi] deleting EFI variable {}:{}'.format(guid, name) )
    ...
    def find_EFI_Table( self, table_sig ):
        ...
        logger().log_hal( "[uefi] searching memory for EFI table with signature '{}' ..".format(table_sig) )
        table_pa, table_header, table, table_buf = None, None, None, None
        pa = smram_base - CHUNK_SZ
        isFound = False
        while pa > CHUNK_SZ:
            logger().log_hal( '[uefi] reading 0x{:016X}..'.format(pa) )
            try:
                membuf = self.cs.mem.read_physical_mem( pa, CHUNK_SZ )
            except OsHelperError as err:
                logger().log_hal("[uefi] Unable to read memory at pa: {:016X} Error: {}".format(pa, err))
                pa -= CHUNK_SZ
                continue
            pos = bytestostring(membuf).find( table_sig )
            if -1 != pos:
                table_pa = pa + pos
                logger().log_hal( "[uefi] found signature '{}' at 0x{:016X}..".format(table_sig, table_pa) )
