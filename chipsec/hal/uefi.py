...
from chipsec.hal.uefi_common import (
    EFI_VENDOR_TABLE,
    EFI_VENDOR_TABLE_SIZE,
    EFI_VENDOR_TABLE_FORMAT,
    EFI_TABLE_HEADER_SIZE,
    EFI_TABLE_HEADER,
    EFI_TABLES,
    MAX_EFI_TABLE_SIZE,
)
...
SECURE_BOOT_VARIABLES = (
    EFI_VAR_NAME_SecureBoot,
    EFI_VAR_NAME_SetupMode,
) + SECURE_BOOT_KEY_VARIABLES
SECURE_BOOT_VARIABLES_ALL = (
    EFI_VAR_NAME_CustomMode,
    EFI_VAR_NAME_SignatureSupport,
) + SECURE_BOOT_VARIABLES
AUTHENTICATED_VARIABLES = (
    EFI_VAR_NAME_AuthVarKeyDatabase,
    EFI_VAR_NAME_certdb,
) + SECURE_BOOT_KEY_VARIABLES
                ...
                logger().log(
                    "Decoded Header ({}):".format(
                        uefi_platform.EFI_VAR_DICT[
                            uefi_platform.FWType.EFI_FW_TYPE_UEFI
                        ]["name"]
                    )
                )
    ...
    for name in efi_vars.keys():
        ...
        for (off, buf, hdr, data, guid, attrs) in efi_vars[name]:
            ...
            var_fname = os.path.join(
                nvram_pth, "{}_{}_{}_{:d}.bin".format(name, guid, attr_str.strip(), n)
            )
...
def identify_EFI_NVRAM(buffer):
    ...
    for fw_type in uefi_platform.fw_types:
        ...
        if uefi_platform.EFI_VAR_DICT[fw_type]["func_getnvstore"]:
            (offset, size, hdr) = uefi_platform.EFI_VAR_DICT[fw_type][
                "func_getnvstore"
            ](b)
            if offset != -1:
                return fw_type
...
class UEFI(hal_base.HALBase):
    ...
    def find_EFI_variable_store(self, rom_buffer):
        ...
        if uefi_platform.EFI_VAR_DICT[self._FWType]["func_getnvstore"]:
            (offset, size, nvram_header) = uefi_platform.EFI_VAR_DICT[self._FWType][
                "func_getnvstore"
            ](rom)
            if -1 == offset:
                logger().error(
                    "'func_getnvstore' is defined but could not find EFI NVRAM. Exiting.."
                )
                return None
        else:
            logger().log(
                "[uefi] 'func_getnvstore' is not defined in EFI_VAR_DICT. Assuming start offset 0.."
            )
...
=================================================================="""
    ...
    def decompress_EFI_binary(
        self, compressed_name, uncompressed_name, compression_type
    ):
        if logger().HAL:
            logger().log(
                "[uefi] decompressing EFI binary (type = 0x{:X})\n       {} ->\n       {}".format(
                    compression_type, compressed_name, uncompressed_name
                )
            )
        if compression_type in COMPRESSION_TYPES:
            if self.cs.helper.decompress_file(
            ...
            logger().error(
                "Unknown EFI compression type 0x{:X}".format(compression_type)
            )
            return None
    ...
    def compress_EFI_binary(self, uncompressed_name, compressed_name, compression_type):
        if logger().HAL:
            logger().log(
                "[uefi] compressing EFI binary (type = 0x{:X})\n       {} ->\n       {}".format(
                    compression_type, uncompressed_name, compressed_name
                )
            )
        if compression_type in COMPRESSION_TYPES:
            if self.cs.helper.compress_file(
                uncompressed_name, compressed_name, compression_type
            ):
    ...
    def find_s3_bootscript(self):
        ...
        if logger().HAL:
            logger().log(
                "[uefi] searching for EFI variable(s): " + str(S3_BOOTSCRIPT_VARIABLES)
            )
        ...
        for efivar_name in efivars:
            ...
            if efivar_name in S3_BOOTSCRIPT_VARIABLES:
                ...
                if 4 == varsz:
                    ...
                    logger().error(
                        "Unrecognized format of '{}' UEFI variable (data size = 0x{:X})".format(
                            efivar_name, varsz
                        )
                    )
                    break
                ...
                if 0 == AcpiGlobalAddr:
                    logger().error(
                        "Pointer to ACPI Global Data structure in {} variable is 0".format(
                            efivar_name
                        )
                    )
                    break
                if logger().HAL:
                    logger().log(
                        "[uefi] Pointer to ACPI Global Data structure: 0x{:016X}".format(
                            AcpiGlobalAddr
                        )
                    )
                if logger().HAL:
                ...
                (
                    ...
                    AcpiFacsTable,
                ) = struct.unpack_from(AcpiVariableSet_fmt, AcpiVariableSet)
                if logger().HAL:
                    logger().log(
                        "[uefi] ACPI Boot-Script table base = 0x{:016X}".format(
                            AcpiBootScriptTable
                        )
                    )
                found = True
    ...
    def get_s3_bootscript(self, log_script=False):
        ...
        if logger().HAL:
            logger().log(
                "[uefi] Found {:d} S3 resume boot-scripts".format(len(bootscript_PAs))
            )
        ...
        for bootscript_pa in bootscript_PAs:
            ...
            if logger().HAL:
                logger().log(
                    "[uefi] S3 resume boot-script at 0x{:016X}".format(bootscript_pa)
                )
            #
    ...
    def set_EFI_variable(self, name, guid, var, datasize=None, attrs=None):
        if logger().HAL:
            logger().log(
                "[uefi] writing EFI variable {}:{} {}".format(
                    guid,
                    name,
                    "" if attrs is None else ("(attributes = {})".format(attrs)),
                )
            )
            # print_buffer( var )
        return self.helper.set_EFI_variable(name, guid, var, datasize, attrs)

    def set_EFI_variable_from_file(
        self, name, guid, filename, datasize=None, attrs=None
    ):
        if filename is None:
    ...
    def find_EFI_Table(self, table_sig):
        ...
        if logger().HAL:
            logger().log(
                "[uefi] searching memory for EFI table with signature '{}' ..".format(
                    table_sig
                )
            )
        ...
        while pa > CHUNK_SZ:
            ...
            try:
                ...
                if logger().HAL:
                    logger().log(
                        "[uefi] Unable to read memory at pa: {:016X} Error: {}".format(
                            pa, err
                        )
                    )
                pa -= CHUNK_SZ
            ...
            if -1 != pos:
                ...
                if (
                    0 != table_header.Reserved
                    or 0 == table_header.CRC32
                    or table_header.Revision not in EFI_REVISIONS
                    or table_header.HeaderSize > MAX_EFI_TABLE_SIZE
                ):
                    if logger().HAL:
                        logger().log(
                            "[uefi] found '{}' at 0x{:016X} but doesn't look like an actual table. keep searching..".format(
                                table_sig, table_pa
                            )
                        )
                    ...
                    if logger().HAL:
                        logger().log(
                            "[uefi] found EFI table at 0x{:016X} with signature '{}'..".format(
                                table_pa, table_sig
                            )
                        )
                    ...
                    if pos < (CHUNK_SZ - EFI_TABLE_HEADER_SIZE - table_size):
                        table_buf = membuf[
                            pos : pos + EFI_TABLE_HEADER_SIZE + table_size
                        ]
                    else:
        ...
        if (not isFound) and logger().HAL:
            logger().log(
                "[uefi] could not find EFI table with signature '{}'".format(table_sig)
            )
            ...
            if 0 != est.BootServices:
                ...
                if not ect_pa:
                    ...
                    (
                        ...
                        rst,
                        rst_buf,
                    ) = self.find_EFI_RuntimeServices_Table()
                    if rst_found:
                        if logger().HAL:
                            logger().warn(
                                "Attempting to derive configuration table address"
                            )
                        ect_pa = rst_pa + (est.ConfigurationTable - est.RuntimeServices)
                    else:
                        if logger().HAL:
                            logger().warn("Can't find UEFI ConfigurationTable")
                        return (None, ect_pa, ect, ect_buf)

        if logger().HAL:
            logger().log(
                "[uefi] EFI Configuration Table ({:d} entries): VA = 0x{:016X}, PA = 0x{:016X}".format(
                    est.NumberOfTableEntries, est.ConfigurationTable, ect_pa
                )
            )

        found = ect_pa is not None
        if found:
            ect_buf = self.cs.mem.read_physical_mem(
                ect_pa, EFI_VENDOR_TABLE_SIZE * est.NumberOfTableEntries
            )
            ect = EFI_CONFIGURATION_TABLE()
            for i in range(est.NumberOfTableEntries):
                vt = EFI_VENDOR_TABLE(
                    *struct.unpack_from(
                        EFI_VENDOR_TABLE_FORMAT, ect_buf[i * EFI_VENDOR_TABLE_SIZE :]
                    )
                )
