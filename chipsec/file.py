...
def read_file( filename, size=0 ):
    ...
    if logger().VERBOSE: logger().log( "[file] read {:d} bytes from '{:.256}'".format( len(_file), filename ) )
    return _file
...
def write_file( filename, buffer, append=False ):
    ...
    if logger().VERBOSE: logger().log( "[file] wrote {:d} bytes to '{:.256}'".format( len(buffer), filename ) )
    return True
