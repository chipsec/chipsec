...
def display_deltas(deltas, hide_time, start_time):
    logger().log("")
    logger().log(
        "[CHIPSEC] **********************  TEST DELTA SUMMARY  *********************"
    )
    if not hide_time:
        logger().log(
            "[CHIPSEC] Time elapsed          {:.3f}".format(time.time() - start_time)
        )
    if deltas:
        ...
        for test in deltas:
            logger().log_bad(
                "{:46}| {:10} | {:10}".format(
                    test, deltas[test]["previous"], deltas[test]["current"]
                )
            )
    else:
...
def log_deltas_xml(deltas, outfile):
    ...
    for test in deltas:
        element = ET.SubElement(
            delta_root,
            "test",
            {"current": deltas[test]["current"], "previous": deltas[test]["previous"]},
        )
        element.text = test
        element.tail = "\n    "
    else:
