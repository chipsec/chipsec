...
class CMOSCommand(BaseCommand):
    ...
    def cmos_writel(self):
        self.logger.log( "[CHIPSEC] Writing CMOS low byte 0x{:X} <- 0x{:X} ".format(self.offset, self.value) )
        self._cmos.write_cmos_low( self.offset, self.value )

    def cmos_readh(self):
        val = self._cmos.read_cmos_high( self.offset )
        self.logger.log( "[CHIPSEC] CMOS high byte 0x{:X} = 0x{:X}".format(self.offset, val) )

    def cmos_writeh(self):
        self.logger.log( "[CHIPSEC] Writing CMOS high byte 0x{:X} <- 0x{:X} ".format(self.offset, self.value) )
