...
class MsgBusCommand(BaseCommand):
    def requires_driver(self):
        ...
        parser_read.add_argument(
            "reg", type=lambda x: int(x, 16), help="Register (hex)"
        )
        ...
        parser_mmread.add_argument(
            "reg", type=lambda x: int(x, 16), help="Register (hex)"
        )
        parser_mmread.set_defaults(func=self.msgbus_mm_read)

        parser_mmwrite = subparsers.add_parser("mm_write")
        parser_mmwrite.add_argument(
            "port", type=lambda x: int(x, 16), help="Port (hex)"
        )
        parser_mmwrite.add_argument(
            "reg", type=lambda x: int(x, 16), help="Register (hex)"
        )
        parser_mmwrite.add_argument(
            "val", type=lambda x: int(x, 16), help="Value (hex)"
        )
        ...
        parser_message.add_argument(
            "opcode", type=lambda x: int(x, 16), help="OPCODE (hex)"
        )
        parser_message.add_argument(
            "val",
    ...
    def msgbus_read(self):
        self.logger.log(
            "[CHIPSEC] msgbus read: port 0x{:02X} + 0x{:08X}".format(
                self.port, self.reg
            )
        )
    ...
    def msgbus_write(self):
        self.logger.log(
            "[CHIPSEC] msgbus write: port 0x{:02X} + 0x{:08X} < 0x{:08X}".format(
                self.port, self.reg, self.val
            )
        )
    ...
    def msgbus_mm_read(self):
        self.logger.log(
            "[CHIPSEC] MMIO msgbus read: port 0x{:02X} + 0x{:08X}".format(
                self.port, self.reg
            )
        )
    ...
    def msgbus_mm_write(self):
        self.logger.log(
            "[CHIPSEC] MMIO msgbus write: port 0x{:02X} + 0x{:08X} < 0x{:08X}".format(
                self.port, self.reg, self.val
            )
        )
    ...
    def msgbus_message(self):
        self.logger.log(
            "[CHIPSEC] msgbus message: port 0x{:02X} + 0x{:08X}, opcode: 0x{:02X}".format(
                self.port, self.reg, self.opcode
            )
        )
        if self.val is not None:
            self.logger.log("[CHIPSEC]                 Data: 0x{:08X}".format(self.val))
        return self._msgbus.msgbus_send_message(
            self.port, self.reg, self.opcode, self.val
        )
    ...
    def run(self):
        ...
        self.logger.log(
            "[CHIPSEC] (msgbus) time elapsed {:.3f}".format(time.time() - t)
        )
