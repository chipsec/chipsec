...
class PCICommand(BaseCommand):
    def requires_driver(self):
        ...
        parser_read.add_argument(
            "device", type=lambda x: int(x, 16), help="Device (hex)"
        )
        parser_read.add_argument(
            "function", type=lambda x: int(x, 16), help="Function (hex)"
        )
        parser_read.add_argument(
            "offset", type=lambda x: int(x, 16), help="Offset (hex)"
        )
        parser_read.add_argument(
            "size", type=str, default=None, nargs="?", help="Width"
        )
        ...
        parser_dump.add_argument(
            "bus", type=lambda x: int(x, 16), nargs="?", default=None, help="Bus (hex)"
        )
        parser_dump.add_argument(
            "device",
    ...
    def pci_dump(self):
        if self.bus is not None:
            if self.device is not None and self.function is not None:
                ...
                devices = self.cs.pci.enumerate_devices(
                    self.bus, self.device, self.function
                )

            for (_bus, _device, _function, _vid, _did) in devices:
                self.logger.log(
                    "[CHIPSEC] PCI device {:02X}:{:02X}.{:02X} configuration:".format(
                        _bus, _device, _function
                    )
                )
    ...
    def pci_xrom(self):
        if self.bus is not None:
            ...
            for (_bus, _device, _function, _vid, _did) in devices:
                self.logger.log(
                    "[CHIPSEC] Locating PCI expansion ROM (XROM) of {:02X}:{:02X}.{:02X}...".format(
                        _bus, _device, _function
                    )
                )
                exists, xrom = self.cs.pci.find_XROM(
                    _bus, _device, _function, True, True, self.xrom_addr
                )
                if exists:
                    self.logger.log(
                        "[CHIPSEC] Found XROM of {:02X}:{:02X}.{:02X}".format(
                            _bus, _device, _function
                        )
                    )
                    if xrom is not None:
                        self.logger.log(
                            "[CHIPSEC] XROM enabled = {:d}, base = 0x{:08X}, size = 0x{:08X}".format(
                                xrom.en, xrom.base, xrom.size
                            )
                        )
                        if xrom.header is not None:
                            self.logger.log(
                                "[CHIPSEC] XROM header: {}".format(xrom.header)
                            )
                else:
                    self.logger.log(
                        "[CHIPSEC] Couldn't find XROM of {:02X}:{:02X}.{:02X}".format(
                            _bus, _device, _function
                        )
                    )
        else:
            ...
            self.logger.log(
                "[CHIPSEC] found {:d} PCI expansion ROMs".format(len(_xroms))
            )
            if len(_xroms) > 0:
    ...
    def pci_read(self):
        ...
        if self.size is not None:
            width = (
                get_option_width(self.size)
                if is_option_valid_width(self.size)
                else int(self.size, 16)
            )

        if 1 == width:
            pci_value = self.cs.pci.read_byte(
                self.bus, self.device, self.function, self.offset
            )
        elif 2 == width:
            pci_value = self.cs.pci.read_word(
                self.bus, self.device, self.function, self.offset
            )
        elif 4 == width:
            pci_value = self.cs.pci.read_dword(
                self.bus, self.device, self.function, self.offset
            )
        else:
            ...
            return
        self.logger.log(
            "[CHIPSEC] PCI {:02X}:{:02X}.{:02X} + 0x{:02X}: 0x{:X}".format(
    ...
    def pci_write(self):
        ...
        if 1 == width:
            self.cs.pci.write_byte(
                self.bus, self.device, self.function, self.offset, self.value
            )
        elif 2 == width:
            self.cs.pci.write_word(
                self.bus, self.device, self.function, self.offset, self.value
            )
        elif 4 == width:
            self.cs.pci.write_dword(
                self.bus, self.device, self.function, self.offset, self.value
            )
        else:
            self.logger.error("Width should be one of {}".format(CMD_OPTS_WIDTH))
            return
        self.logger.log(
            "[CHIPSEC] Write 0x{:X} to PCI {:02X}:{:02X}.{:02X} + 0x{:02X}".format(
    ...
    def pci_cmd(self):
        ...
        for (b, d, f, vid, did) in self.cs.pci.enumerate_devices():
            ...
            if (cmd_reg & self.cmd_mask) == 0:
                continue
            self.logger.log(
                "{:02X}:{:02X}.{:X} | {:04X}:{:04X} | {:04X} | {:02X}  | {:02X}".format(
