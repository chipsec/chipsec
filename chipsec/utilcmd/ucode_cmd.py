...
from chipsec.hal.ucode import dump_ucode_update_header, Ucode
...
class UCodeCommand(BaseCommand):
    ...
    def ucode_id(self):
        if self.cpu_thread_id is None:
            for tid in range(self.cs.msr.get_cpu_thread_count()):
                ucode_update_id = self._ucode.ucode_update_id(tid)
                self.logger.log("[CHIPSEC] CPU{:d}: Microcode update ID = 0x{:08X}".format(tid, ucode_update_id))
        else:
            ucode_update_id = self._ucode.ucode_update_id(self.cpu_thread_id)
    ...
    def ucode_load(self):
        if self.cpu_thread_id is None:
            ...
            self._ucode.update_ucode_all_cpus(self.ucode_filename)
        else:
            self.logger.log("[CHIPSEC] Loading Microcode update on CPU{:d} from '{}'".format(self.cpu_thread_id, self.ucode_filename))
            self._ucode.update_ucode(self.cpu_thread_id, self.ucode_filename)
