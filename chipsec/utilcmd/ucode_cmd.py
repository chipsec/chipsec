...
class UCodeCommand(BaseCommand):
    def requires_driver(self):
        ...
        parser_load.add_argument(
            "ucode_filename", type=str, help="ucode file name (.PDB or .BIN format)"
        )
        parser_load.add_argument(
            "cpu_thread_id",
            nargs="?",
        ...
        parser_decode.add_argument(
            "ucode_filename", type=str, help="ucode file name (.PDB format)"
        )
    ...
    def ucode_id(self):
        if self.cpu_thread_id is None:
            ...
            self.logger.log(
                "[CHIPSEC] CPU{:d}: Microcode update ID = 0x{:08X}".format(
                    self.cpu_thread_id, ucode_update_id
                )
            )
    ...
    def ucode_load(self):
        if self.cpu_thread_id is None:
            self.logger.log(
                "[CHIPSEC] Loading Microcode update on all cores from '{}'".format(
                    self.ucode_filename
                )
            )
            ...
            self.logger.log(
                "[CHIPSEC] Loading Microcode update on CPU{:d} from '{}'".format(
                    self.cpu_thread_id, self.ucode_filename
                )
            )
    ...
    def ucode_decode(self):
        ...
        self.logger.log(
            "[CHIPSEC] Decoding Microcode Update header of PDB file: '{}'".format(
                self.ucode_filename
            )
        )
