...
class SPICommand(BaseCommand):
    def requires_driver(self):
        ...
        parser_dump.add_argument(
            ...
            nargs="?",
            default="rom.bin",
            help="Output file name [default=rom.bin]",
        )
        parser_dump.set_defaults(func=self.spi_dump)

        parser_read = subparsers.add_parser("read")
        parser_read.add_argument(
            "spi_fla", type=lambda x: int(x, 16), help="Start Address (hex)"
        )
        parser_read.add_argument(
            "length",
        ...
        parser_read.add_argument(
            ...
            nargs="?",
            default="read.bin",
            help="Output file [default=read.bin",
        )
        ...
        parser_jedec.add_argument(
            "option", type=str, nargs="?", default="", help="Optional decode"
        )
    ...
    def spi_dump(self):
        self.logger.log(
            "[CHIPSEC] Dumping entire SPI flash memory to '{}'".format(self.out_file)
        )
        ...
        spi_size = limit + 1
        self.logger.log(
            "[CHIPSEC] BIOS region: base = 0x{:08X}, limit = 0x{:08X}".format(
                base, limit
            )
        )
        self.logger.log(
            "[CHIPSEC] Dumping 0x{:08X} bytes (to the end of BIOS region)".format(
                spi_size
            )
        )
        ...
        if buf is None:
            self.logger.error(
                "Dumping SPI Flash didn't return any data (turn on VERBOSE)"
            )
        else:
            self.logger.log(
                "[CHIPSEC] Completed SPI flash dump to '{}'".format(self.out_file)
            )

    def spi_read(self):
        self.logger.log(
            "[CHIPSEC] Reading 0x{:x} bytes from SPI Flash starting at FLA = 0x{:X}".format(
                self.length, self.spi_fla
            )
        )
        self.logger.log("[CHIPSEC] {}".format(self._msg))
    ...
    def spi_write(self):
        if not os.path.exists(self.filename):
            ...
            return
        self.logger.log(
            "[CHIPSEC] Writing to SPI flash memory at FLA = 0x{:X} from '{:64s}'".format(
                self.spi_fla, self.filename
            )
        )
    ...
    def spi_erase(self):
        self.logger.log(
            "[CHIPSEC] Erasing SPI flash memory block at FLA = 0x{:X}".format(
                self.spi_fla
            )
        )
    ...
    def spi_jedec(self):
        if self.option.lower() == "decode":
            ...
            if jedec is not False:
                ...
                self.logger.log(
                    "    Device       : 0x{:04X}   - {}".format(jedec & 0xFFFF, part)
                )
                self.logger.log("")
            else:
