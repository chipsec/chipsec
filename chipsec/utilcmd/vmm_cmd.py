...
class VMMCommand(BaseCommand):
    def requires_driver(self):
        ...
        parser_hypercall.add_argument(
            "ax", type=lambda x: int(x, 16), help="rax/eax value (hex)"
        )
        parser_hypercall.add_argument(
            "bx", type=lambda x: int(x, 16), help="rbx/ebx value (hex)"
        )
        parser_hypercall.add_argument(
            "cx", type=lambda x: int(x, 16), help="rcx/ecx value (hex)"
        )
        parser_hypercall.add_argument(
            "dx", type=lambda x: int(x, 16), help="rdx/edx value (hex)"
        )
        parser_hypercall.add_argument(
            "di", type=lambda x: int(x, 16), help="rdi/edi value (hex)"
        )
        parser_hypercall.add_argument(
            "si", type=lambda x: int(x, 16), help="rsi/esi value (hex)"
        )
        parser_hypercall.add_argument(
            "r8", type=lambda x: int(x, 16), nargs="?", default=0, help="r8 value (hex)"
        )
        parser_hypercall.add_argument(
            "r9", type=lambda x: int(x, 16), nargs="?", default=0, help="r9 value (hex)"
        )
        parser_hypercall.add_argument(
            "r10",
        ...
        parser_virtio.add_argument(
            "bdf", type=str, nargs="?", default=None, help="<bus>:<device>.<function>"
        )
