...
class SMICommand(BaseCommand):
    ...
    def requires_driver(self):
        ...
        parser_send.add_argument(
            "thread_id", type=lambda x: int(x, 16), help="Thread ID (hex)"
        )
        parser_send.add_argument(
            "SMI_code_port_value", type=lambda x: int(x, 16), help="SMI Code (hex)"
        )
        parser_send.add_argument(
            "SMI_data_port_value", type=lambda x: int(x, 16), help="SMI Data (hex)"
        )
        parser_send.add_argument(
            "_rax", type=lambda x: int(x, 16), nargs="?", default=None, help="RAX (hex)"
        )
        parser_send.add_argument(
            "_rbx",
        ...
        parser_send.add_argument(
            ...
            nargs="?",
            default=0,
            help="RDI (hex) [default=0]",
        )
        parser_send.set_defaults(func=self.smi_send)

        parser_smmc = subparsers.add_parser("smmc")
        parser_smmc.add_argument(
            "RTC_start", type=lambda x: int(x, 16), help="RTC Code Start (hex)"
        )
        parser_smmc.add_argument(
            "RTC_end", type=lambda x: int(x, 16), help="RT Code End (hex)"
        )
        parser_smmc.add_argument("guid", type=str, help="GUID")
        parser_smmc.add_argument(
            "payload_loc", type=lambda x: int(x, 16), help="Payload Location (hex)"
        )
    ...
    def smi_count(self):
        ...
        for tid in range(self.cs.msr.get_cpu_thread_count()):
            smi_cnt = self.cs.read_register_field(
                "MSR_SMI_COUNT", "Count", cpu_thread=tid
            )
    ...
    def smi_smmc(self):
        ...
        ReturnStatus = self.interrupts.send_smmc_SMI(
            smmc_loc, self.guid, self.payload, self.payload_loc, CommandPort=self.port
        )
        # TODO Translate ReturnStatus to EFI_STATUS enum
        self.logger.log(
            "ReturnStatus: 0x{:x} ({})".format(
                ReturnStatus, EFI_ERROR_STR(ReturnStatus)
            )
        )

    def smi_send(self):
        self.logger.log(
            "[CHIPSEC] Sending SW SMI (code: 0x{:02X}, data: 0x{:02X})..".format(
                self.SMI_code_port_value, self.SMI_data_port_value
            )
        )
        if self._rax is None:
            self.interrupts.send_SMI_APMC(
                self.SMI_code_port_value, self.SMI_data_port_value
            )
        else:
            self.logger.log(
                "          RAX: 0x{:016X} (AX will be overridden with values of SW SMI ports B2/B3)".format(
                    self._rax
                )
            )
            ...
            self.logger.log(
                "          RDX: 0x{:016X} (DX will be overridden with 0x00B2)".format(
                    self._rdx
                )
            )
            self.logger.log("          RSI: 0x{:016X}".format(self._rsi))
            self.logger.log("          RDI: 0x{:016X}".format(self._rdi))
            ret = self.interrupts.send_SW_SMI(
                self.thread_id,
                self.SMI_code_port_value,
                self.SMI_data_port_value,
                self._rax,
                self._rbx,
                self._rcx,
                self._rdx,
