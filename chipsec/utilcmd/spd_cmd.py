...
class SPDCommand(BaseCommand):
    def requires_driver(self):
        ...
        parser_dump.add_argument(
            "dev", type=str, nargs="?", default=None, help="Device"
        )
    ...
    def spd_dump(self):
        if self.dev is not None:
            _dev = self.dev.upper()
            self.dev_addr = (
                spd.SPD_DIMM_ADDRESSES[_dev]
                if _dev in spd.SPD_DIMM_ADDRESSES
                else int(self.dev, 16)
            )
            if not self._spd.isSPDPresent(self.dev_addr):
                self.logger.log(
                    "[CHIPSEC] SPD for DIMM 0x{:X} is not found".format(self.dev_addr)
                )
                return
    ...
    def spd_read(self):
        ...
        self.logger.log(
            "[CHIPSEC] SPD read: offset 0x{:X} = 0x{:X}".format(self.off, val)
        )
    ...
    def spd_write(self):
        ...
        self.logger.log(
            "[CHIPSEC] SPD write: offset 0x{:X} = 0x{:X}".format(self.off, self.val)
        )
