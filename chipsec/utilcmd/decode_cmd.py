...
class DecodeCommand(BaseCommand):
    def requires_driver(self):
        ...
        parser.add_argument(
            "_fwtype", metavar="fw_type", nargs="?", help="firmware type", default=None
        )
    ...
    def decode_types(self):
        self.logger.log(
            "\n<fw_type> should be in [ {} ]\n".format(
                " | ".join(["{}".format(t) for t in uefi_platform.fw_types])
            )
        )
    ...
    def decode_rom(self):
        _uefi = UEFI(self.cs)
        self.logger.log(
            "[CHIPSEC] Decoding SPI ROM image from a file '{}'".format(self._rom)
        )
        ...
        if (-1 == fd_off) or (fd is None):
            self.logger.error(
                "Could not find SPI Flash descriptor in the binary '{}'".format(
                    self._rom
                )
            )
        ...
        self.logger.log(
            "[CHIPSEC] Found SPI Flash descriptor at offset 0x{:X} in the binary '{}'".format(
                fd_off, self._rom
            )
        )
        rom = f[fd_off:]
        ...
        for r in flregs:
            ...
            if not notused:
                ...
                fname = os.path.join(
                    pth, "{:d}_{:04X}-{:04X}_{}.bin".format(idx, base, limit, name)
                )
