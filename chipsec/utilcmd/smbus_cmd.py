...
class SMBusCommand(BaseCommand):
    def requires_driver(self):
        ...
        parser_read.add_argument(
            "dev_addr", type=lambda x: int(x, 16), help="Start Address (hex)"
        )
        parser_read.add_argument(
            "start_off", type=lambda x: int(x, 16), help="Start Offset (hex)"
        )
        parser_read.add_argument(
            "size",
        ...
        parser_write.add_argument(
            "off", type=lambda x: int(x, 16), help="Start Offset (hex)"
        )
        parser_write.add_argument(
            "val", type=lambda x: int(x, 16), help="Byte Value (hex)"
        )
    ...
    def smbus_read(self):
        if self.size is not None:
            ...
            self.logger.log(
                "[CHIPSEC] SMBus read: device 0x{:X} offset 0x{:X} size 0x{:X}".format(
                    self.dev_addr, self.start_off, self.size
                )
            )
            print_buffer(buf)
        else:
            ...
            self.logger.log(
                "[CHIPSEC] SMBus read: device 0x{:X} offset 0x{:X} = 0x{:X}".format(
                    self.dev_addr, self.start_off, val
                )
            )

    def smbus_write(self):
        self.logger.log(
            "[CHIPSEC] SMBus write: device 0x{:X} offset 0x{:X} = 0x{:X}".format(
                self.dev_addr, self.off, self.val
            )
        )
