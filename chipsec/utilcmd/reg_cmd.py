...
class RegisterCommand(BaseCommand):
    def requires_driver(self):
        ...
        parser_read.add_argument(
            "field_name", type=str, nargs="?", default=None, help="Field name"
        )
        ...
        parser_writefield.add_argument(
            "value", type=lambda x: int(x, 16), help="Value (hex)"
        )
        ...
        parser_setcontrol.add_argument(
            "value", type=lambda x: int(x, 16), help="Value (hex)"
        )
    ...
    def reg_read(self):
        if self.field_name is not None:
            ...
            self.logger.log(
                "[CHIPSEC] {}.{}=0x{:X}".format(self.reg_name, self.field_name, value)
            )
        else:
    ...
    def reg_read_field(self):
        if self.cs.register_has_field(self.reg_name, self.field_name):
            ...
            self.logger.error(
                "[CHIPSEC] Register '{}' doesn't have field '{}' defined".format(
                    self.reg_name, self.field_name
                )
            )

    def reg_write(self):
        self.logger.log(
            "[CHIPSEC] Writing {} < 0x{:X}".format(self.reg_name, self.value)
        )
    ...
    def reg_write_field(self):
        if self.cs.register_has_field(self.reg_name, self.field_name):
            self.logger.log(
                "[CHIPSEC] Writing {}.{} < 0x{:X}".format(
                    self.reg_name, self.field_name, self.value
                )
            )
    ...
    def reg_get_control(self):
        if self.cs.is_control_defined(self.control_name):
            ...
            self.logger.error(
                "[CHIPSEC] Control '{}' isn't defined".format(self.control_name)
            )
    ...
    def reg_set_control(self):
        if self.cs.is_control_defined(self.control_name):
            ...
            self.logger.log(
                "[CHIPSEC] Setting control {} < 0x{:X}".format(
                    self.control_name, self.value
                )
            )
        else:
            self.logger.error(
