...
class IgdCommand(BaseCommand):
    def requires_driver(self):
        ...
        parser_read.add_argument(
            "address", type=lambda x: int(x, 16), help="Address (hex)"
        )
        parser_read.add_argument(
            "width",
            ...
            nargs="?",
            default=0x100,
            help="Width of read (hex)",
        )
        parser_read.add_argument(
            "file_name", type=str, nargs="?", default="", help="File name to save data"
        )
        ...
        parser_write.add_argument(
            "size", type=lambda x: int(x, 16), help="Size of data to write (hex)"
        )
        parser_write.add_argument(
            "file_value", type=str, help="Data to write [Value|<file_name>]"
        )
    ...
    def write_dma(self):
        if not os.path.exists(self.file_value):
            ...
            try:
                ...
                self.logger.error(
                    "Incorrect <value> specified: '{}'".format(self.file_value)
                )
                self.logger.error(str(e))
                return
            self.logger.log(
                "[CHIPSEC] Read 0x{:X} hex bytes from command-line: {}'".format(
                    len(buffer), buffer_value
                )
            )
        else:
    ...
    def run(self):
        ...
        if not self.cs.igd.is_device_enabled():
            self.logger.log(
                "[CHIPSEC] Looks like internal graphics device is not enabled"
            )
            return
