...
class CPUCommand(BaseCommand):
    def requires_driver(self):
        ...
        parser_cr.add_argument(
            "value", type=lambda x: int(x, 0), nargs="?", default=None
        )
    ...
    def cpu_info(self):
        ...
        self.logger.log(
            "          Hyper-Threading         : {}".format(
                "Enabled" if ht else "Disabled"
            )
        )
        self.logger.log(
            "          CPU cores per package   : {:d}".format(cores_per_pkg)
        )
        self.logger.log(
            "          CPU threads per core    : {:d}".format(threads_per_core)
        )
        self.logger.log(
            "          CPU threads per package : {:d}".format(threads_per_pkg)
        )
        try:
            threads_count = self.cs.cpu.get_number_threads_from_APIC_table()
            sockets_count = self.cs.cpu.get_number_sockets_from_APIC_table()
            self.logger.log(
                "          Number of sockets       : {:d}".format(sockets_count)
            )
            self.logger.log(
                "          Number of CPU threads   : {:d}".format(threads_count)
            )
        except Exception:
            pass
    ...
    def cpu_cr(self):
        if self.value is not None:
            self.logger.log(
                "[CHIPSEC] CPU{:d}: write CR{:d} <- 0x{:08X}".format(
                    self.thread, self.cr_number, self.value
                )
            )
            ...
            self.logger.log(
                "[CHIPSEC] CPU{:d}: read CR{:d} -> 0x{:08X}".format(
                    self.thread, self.cr_number, value
                )
            )
            return value
        else:
    ...
    def cpu_pt(self):
        if self.cr3 is not None:
            ...
            self.logger.log(
                "[CHIPSEC] paging physical base (CR3): 0x{:016X}".format(self.cr3)
            )
            self.logger.log(
                "[CHIPSEC] dumping paging hierarchy to '{}'...".format(pt_fname)
            )
            ...
            for tid in range(self.cs.msr.get_cpu_thread_count()):
                ...
                self.logger.log(
                    "[CHIPSEC][cpu{:d}] paging physical base (CR3): 0x{:016X}".format(
                        tid, cr3
                    )
                )
                self.logger.log(
                    "[CHIPSEC][cpu{:d}] dumping paging hierarchy to '{}'...".format(
                        tid, pt_fname
                    )
                )
