...
class CONFIGCommand(BaseCommand):
    def requires_driver(self):
        ...
        parser_show.add_argument(
            "name", type=str, nargs="*", help="Specific Name", default=[]
        )
        ...
        for mconfig in config:
            ...
            for name in self.name:
                if mconfig == "REGISTERS":
                    self.logger.log(
                        "\t{} - {}".format(name, self.register_details(cfg[name]))
                    )
                elif mconfig == "CONFIG_PCI":
                    self.logger.log(
                        "\t{} - {}".format(name, self.pci_details(cfg[name]))
                    )
                elif mconfig == "MMIO_BARS":
                    self.logger.log(
                        "\t{} - {}".format(name, self.mmio_details(cfg[name]))
                    )
                elif mconfig == "IO_BARS":
                    self.logger.log(
                        "\t{} - {}".format(name, self.io_details(cfg[name]))
                    )
                elif mconfig == "MEMORY_RANGES":
                    self.logger.log(
                        "\t{} - {}".format(name, self.mem_details(cfg[name]))
                    )
                elif mconfig == "CONTROLS":
                    self.logger.log(
                        "\t{} - {}".format(name, self.control_details(cfg[name]))
                    )
                elif mconfig == "LOCKS":
                    self.logger.log(
                        "\t{} - {}".format(name, self.lock_details(cfg[name]))
                    )
                elif mconfig == "BUS":
                    self.logger.log(
                        "\t{} - {}".format(name, self.bus_details(cfg[name]))
                    )

    def register_details(self, regi):
        if regi["type"] == "pcicfg" or regi["type"] == "mmcfg":
            if "device" in regi.keys():
                ret = "device: {}, offset: {}, size: {}".format(
                    regi["device"], regi["offset"], regi["size"]
                )
            else:
                ret = "bus: {}, dev: {}, func: {}, offset: {}, size: {}".format(
                    regi["bus"], regi["dev"], regi["fun"], regi["offset"], regi["size"]
                )
        elif regi["type"] == "mmio":
            ret = "bar: {}, offset: {}, size: {}".format(
                regi["bar"], regi["offset"], regi["size"]
            )
        elif regi["type"] == "mm_msgbus":
            ret = "port: {}, offset: {}, size: {}".format(
                regi["port"], regi["offset"], regi["size"]
            )
        elif regi["type"] == "io":
            ...
            ret = "access: {}, address: {}, offset: {}, size: {}".format(
                regi["access"], regi["address"], regi["offset"], regi["size"]
            )
        if "FIELDS" in regi.keys():
            for key in regi["FIELDS"].keys():
                ret += "\n\t\t{} - bit {}:{}".format(
                    key,
                    regi["FIELDS"][key]["bit"],
                    int(regi["FIELDS"][key]["size"])
                    + int(regi["FIELDS"][key]["bit"])
                    - 1,
                )
        return ret

    def pci_details(self, regi):
        ret = "bus: {}, dev: {}, func: {}, vid: {}, did: {}".format(
            regi["bus"],
            regi["dev"],
            regi["fun"],
            regi["vid"],
            regi["did"] if "did" in regi.keys() else None,
        )
        return ret
    ...
    def mem_details(self, regi):
        ret = "access: {}, address: {}, size: {}".format(
            regi["access"], regi["address"], regi["size"]
        )
        return ret
    ...
    def lock_details(self, regi):
        ret = "register: {}, field: {}, value: {}".format(
            regi["register"], regi["field"], regi["value"]
        )
        return ret
