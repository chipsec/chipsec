...
class MemCommand(BaseCommand):
    def requires_driver(self):
        ...
        parser_read.add_argument(
            ...
            help="64-bit physical address (hex)",
        )
        parser_read.add_argument(
            "buffer_length",
            type=lambda x: int(x, 16),
        ...
        parser_read.add_argument(
            "file_name", type=str, default="", nargs="?", help="Buffer file name"
        )
        ...
        parser_readval.add_argument(
            ...
            nargs="?",
            default="",
            help="Length to read (byte|word|dword)",
        )
        ...
        parser_write.add_argument(
            "buffer_length", type=lambda x: int(x, 16), help="Length of buffer (hex)"
        )
        parser_write.add_argument(
            "buffer_data", type=str, help="Buffer data or file name"
        )
        ...
        parser_writeval.add_argument(
            "write_data", type=lambda x: int(x, 16), help="Data to write"
        )
        ...
        parser_search.add_argument(
            "length", type=lambda x: int(x, 16), help="Length to search (hex)"
        )
    ...
    def dump_region_to_path(self, path, pa_start, pa_end):
        ...
        with open(fname, "wb") as f:
            ...
            if head_len > 0:
                f.write(
                    self.cs.mem.read_physical_mem(pa_start, ALIGNED_4KB + 1 - head_len)
                )
    ...
    def mem_allocate(self):
        ...
        self.logger.log(
            "[CHIPSEC] Allocated {:X} bytes of physical memory: VA = 0x{:016X}, PA = 0x{:016X}".format(
    ...
    def mem_search(self):
        ...
        if offset != -1:
            self.logger.log(
                "[CHIPSEC] Search buffer from memory: PA = 0x{:016X}, len = 0x{:X}, target address= 0x{:X}..".format(
                    self.phys_address, self.length, self.phys_address + offset
                )
            )
        else:
            self.logger.log(
                "[CHIPSEC] Search buffer from memory: PA = 0x{:016X}, len = 0x{:X}, can not find the target in the searched range..".format(
    ...
    def mem_read(self):
        self.logger.log(
            "[CHIPSEC] Reading buffer from memory: PA = 0x{:016X}, len = 0x{:X}..".format(
    ...
    def mem_readval(self):
        ...
        if self.length:
            try:
                width = (
                    get_option_width(self.length)
                    if is_option_valid_width(self.length)
                    else int(self.length, 16)
                )
            except ValueError:
    ...
    def mem_write(self):
        if not os.path.exists(self.buffer_data):
            try:
                ...
                self.logger.error(
                    "Incorrect <value> specified: '{}'".format(self.buffer_data)
                )
                return
            self.logger.log(
        ...
        self.logger.log(
            "[CHIPSEC] writing buffer to memory: PA = 0x{:016X}, len = 0x{:X}..".format(
                self.phys_address, self.buffer_length
            )
        )
    ...
    def mem_writeval(self):
        ...
        if width not in (0x1, 0x2, 0x4):
            ...
            return
        self.logger.log(
            "[CHIPSEC] Writing {:X}-byte value 0x{:X} to PA 0x{:016X}..".format(
                width, self.write_data, self.phys_address
            )
        )
        if 0x1 == width:
