...
class ACPICommand(BaseCommand):
    def requires_driver(self):
        ...
        parser_table.add_argument(
            "-f", "--file", dest="_file", help="Read from file", action="store_true"
        )
        parser_table.add_argument(
            "_name", metavar="table|filename", nargs=1, help="table to list"
        )
    ...
    def acpi_table(self):
        ...
        if not self._file and not self._acpi.is_ACPI_table_present(name):
            self.logger.error(
                "Please specify table name from {}".format(self._acpi.tableList.keys())
            )
            return
            ...
            return
        self.logger.log(
            "[CHIPSEC] reading ACPI table {} '{}'".format(
                "from file" if self._file else "", name
            )
        )
