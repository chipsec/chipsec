...
Hash is truncated to 28 bits. For test names use full path as seen in example below.

Usage:
    ``chipsec_util  name <test name>``
    ``chipsec_util hash <test hash>``

Examples:
    >>> chipsec_main.py -m common.tools.generate_test_id name chipsec.modules.common.remap
    >>> chipsec_main.py -m common.tools.generate_test_id hash 0x67eb58d
"""
...
class TestIdCommand(BaseCommand):
