...
class MMCfgCommand(BaseCommand):
    def requires_driver(self):
        ...
        parser.add_argument(
            "width",
            type=str,
            choices=["byte", "word", "dword", "1", "2", "4"],
            help="Width [byte,word,dword] or (int)",
        )
        parser.add_argument(
            "value",
    ...
    def run(self):
        ...
        if self.value is not None:
            _mmio.write_mmcfg_reg(
                self.bus, self.device, self.function, self.offset, _width, self.value
            )
            self.logger.log(
                "[CHIPSEC] Writing MMCFG register ({:02d}:{:02d}.{:d} + 0x{:02X}): 0x{:X}".format(
                    self.bus, self.device, self.function, self.offset, self.value
                )
            )
        else:
            data = _mmio.read_mmcfg_reg(
                self.bus, self.device, self.function, self.offset, _width
            )
            self.logger.log(
                "[CHIPSEC] Reading MMCFG register ({:02d}:{:02d}.{:d} + 0x{:02X}): 0x{:X}".format(
