...
from chipsec.hal.spi_uefi import (
    ...
    modify_uefi_region,
    compress_image,
    CMD_UEFI_FILE_REPLACE,
)
from chipsec.hal.spi_uefi import (
    CMD_UEFI_FILE_INSERT_AFTER,
    CMD_UEFI_FILE_INSERT_BEFORE,
    CMD_UEFI_FILE_REMOVE,
)
from chipsec.hal.uefi import (
    UEFI,
    decode_EFI_variables,
    get_attr_string,
    identify_EFI_NVRAM,
)
...
class UEFICommand(BaseCommand):
    def requires_driver(self):
        ...
        parser_var_read.add_argument(
            "filename",
            type=str,
            nargs="?",
            default=None,
            help="output file to store read variable contents to",
        )
        parser_var_read.set_defaults(func=self.var_read)

        # var-write command args
        parser_var_write = subparsers.add_parser("var-write")
        parser_var_write.add_argument(
            "name", type=str, help="name of variable to write"
        )
        parser_var_write.add_argument(
            "guid", type=str, help="guid of variable to write"
        )
        parser_var_write.add_argument(
            "filename", type=str, help="input file containing data to write to variable"
        )
        ...
        parser_var_delete.add_argument(
            "name", type=str, help="name of variable to delete"
        )
        parser_var_delete.add_argument(
            "guid", type=str, help="guid of variable to delete"
        )
        ...
        parser_var_find.add_argument(
            "name_guid", type=str, help="name or guid of variable to find"
        )
        ...
        parser_decode.add_argument(
            "filename", type=str, help="bios image to decompress"
        )
        parser_decode.add_argument(
            "--fwtype", dest="fwtype", type=str, nargs="?", default=None
        )
        parser_decode.add_argument(
            "filetypes", type=str, nargs="*", default=[], help=FILE_TYPE_NAMES.values()
        )
        ...
        parser_keys.add_argument(
            "filename", type=str, help="name of file containing variables"
        )
        ...
        parser_bootscript.add_argument(
            "bootscript_pa", type=lambda x: int(x, 0), nargs="?", help=""
        )
    ...
    def var_read(self):
        self.logger.log(
            "[CHIPSEC] Reading EFI variable Name='{}' GUID={{{}}} to '{}' via Variable API..".format(
                self.name, self.guid, self.filename
            )
        )
    ...
    def var_write(self):
        self.logger.log(
            "[CHIPSEC] writing EFI variable Name='{}' GUID={{{}}} from '{}' via Variable API..".format(
                self.name, self.guid, self.filename
            )
        )
        status = self._uefi.set_EFI_variable_from_file(
            self.name, self.guid, self.filename
        )
    ...
    def var_delete(self):
        self.logger.log(
            "[CHIPSEC] Deleting EFI variable Name='{}' GUID={{{}}} via Variable API..".format(
                self.name, self.guid
            )
        )
        ...
        if status == 0:
            ...
            self.logger.error("deleting EFI variable failed")
    ...
    def var_find(self):
        ...
        if is_guid:
            self.logger.log(
                "[*] Searching for UEFI variable with GUID {{{}}}..".format(_input_var)
            )
            for name in _vars:
                n = 0
                for (off, buf, hdr, data, guid, attrs) in _vars[name]:
                    if _input_var == guid:
                        var_fname = "{}_{}_{}_{:d}.bin".format(
                            name, guid, get_attr_string(attrs).strip(), n
                        )
                        self.logger.log_good(
                            "Found UEFI variable {}:{}. Dumped to '{}'".format(
                                guid, name, var_fname
                            )
                        )
            ...
            self.logger.log(
                "[*] Searching for UEFI variable with name {}..".format(_input_var)
            )
            name = _input_var
    ...
    def decode(self):
        ...
        self.logger.log(
            "[CHIPSEC] Parsing EFI volumes from '{}'..".format(self.filename)
        )
        _orig_logname = self.logger.LOG_FILE_NAME
    ...
    def keys(self):
        if not os.path.exists(self.filename):
            ...
            return
        self.logger.log(
            "<keyvar_file> should contain one of the following EFI variables\n[ %s ]"
            % (" | ".join(["%s" % var for var in SECURE_BOOT_KEY_VARIABLES]))
        )
        self.logger.log(
            "[CHIPSEC] Parsing EFI variable from '{}'..".format(self.filename)
        )
        parse_efivar_file(self.filename)

    def tables(self):
        self.logger.log(
            "[CHIPSEC] Searching memory for and dumping EFI tables (this may take a minute)..\n"
        )
    ...
    def s3bootscript(self):
        ...
        if self.bootscript_pa is not None:
            self.logger.log(
                "[*] Reading S3 boot-script from memory at 0x{:016X}..".format(
                    self.bootscript_pa
                )
            )
    ...
    def insert_before(self):
        ...
        new_image = modify_uefi_region(
            rom_image, CMD_UEFI_FILE_INSERT_BEFORE, self.guid, efi_image
        )
    ...
    def insert_after(self):
        ...
        efi_image = read_file(self.efi_file)
        new_image = modify_uefi_region(
            rom_image, CMD_UEFI_FILE_INSERT_AFTER, self.guid, efi_image
        )
        write_file(self.new_file, new_image)
    ...
    def replace(self):
        ...
        efi_image = read_file(self.efi_file)
        new_image = modify_uefi_region(
            rom_image, CMD_UEFI_FILE_REPLACE, self.guid, efi_image
        )
        write_file(self.new_file, new_image)
    ...
    def assemble(self):
        ...
        if self.file_type == "freeform":
            ...
            if compression_type > 0:
                ...
                wrap_image = assemble_uefi_section(
                    comp_image, len(wrap_image), compression_type
                )
        ...
        self.logger.log(
            "[CHIPSEC]  UEFI file was successfully assembled! Binary file size: {:d}, compressed UEFI file size: {:d}".format(
                len(raw_image), len(uefi_image)
            )
        )
