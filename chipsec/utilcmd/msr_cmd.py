    ...
    def run(self):
        if self.msr_input1 is None:
            for tid in range(self.cs.msr.get_cpu_thread_count()):
                ...
                val64 = (edx << 32) | eax
                self.logger.log(
                    "[CHIPSEC] CPU{:d}: RDMSR( 0x{:x} ) = {:016X} (EAX={:08X}, EDX={:08X})".format(
                        tid, self.msr_addr, val64, eax, edx
                    )
                )
            ...
            self.logger.log(
                "[CHIPSEC] CPU{:d}: RDMSR( 0x{:x} ) = {:016X} (EAX={:08X}, EDX={:08X})".format(
                    cpu_thread_id, self.msr_addr, val64, eax, edx
                )
            )
        else:
            eax = self.msr_input1
            ...
            if self.cpu_id is None:
                self.logger.log(
                    "[CHIPSEC] All CPUs: WRMSR( 0x{:x} ) = {:016X}".format(
                        self.msr_addr, val64
                    )
                )
                for tid in range(self.cs.msr.get_cpu_thread_count()):
                ...
                self.logger.log(
                    "[CHIPSEC] CPU{:d}: WRMSR( 0x{:x} ) = {:016X}".format(
                        cpu_thread_id, self.msr_addr, val64
                    )
                )
