...
class MMIOCommand(BaseCommand):
    def requires_driver(self):
        ...
        parser_dump_abs.add_argument(
            "base", type=lambda x: int(x, 16), help="MMIO region base address"
        )
        parser_dump_abs.add_argument(
            "offset",
            type=lambda x: int(x, 16),
        ...
        parser_read.add_argument(
            "offset", type=lambda x: int(x, 16), help="Offset value (hex)"
        )
        parser_read.add_argument(
            "width",
            type=lambda x: int(x, 16),
        ...
        parser_read.add_argument(
            "bus", type=lambda x: int(x, 16), nargs="?", default=None, help="bus value"
        )
        parser_read.set_defaults(func=self.read_bar)

        parser_read_abs = subparsers.add_parser("read-abs")
        parser_read_abs.add_argument(
            "base", type=lambda x: int(x, 16), help="MMIO region base address"
        )
        parser_read_abs.add_argument(
            "offset", type=lambda x: int(x, 16), help="MMIO register offset"
        )
        parser_read_abs.add_argument(
            "width",
            type=lambda x: int(x, 16),
        ...
        parser_write.add_argument(
            "value", type=lambda x: int(x, 16), help="Value to write (hex)"
        )
        parser_write.add_argument(
            "bus", type=lambda x: int(x, 16), nargs="?", default=None, help="bus value"
        )
    ...
    def dump_bar(self):
        self.logger.log(
            "[CHIPSEC] Dumping {} MMIO space..".format(self.bar_name.upper())
        )
        (bar_base, bar_size) = self._mmio.get_MMIO_BAR_base_address(
            self.bar_name.upper()
        )
    ...
    def dump_bar_abs(self):
        ...
        self.logger.log(
            "[CHIPSEC] Dumping MMIO space 0x{:08X} to 0x{:08X}".format(
                tmp_base, tmp_base + tmp_length
            )
        )
    ...
    def read_bar(self):
        ...
        self.logger.log(
            "[CHIPSEC] Read {} + 0x{:X}: 0x{:08X}".format(bar, self.offset, reg)
        )
    ...
    def read_abs(self):
        ...
        self.logger.log(
            "[CHIPSEC] Read 0x{:X} + 0x{:X}: 0x{:08X}".format(
                self.base, self.offset, reg
            )
        )
    ...
    def write_bar(self):
        bar = self.bar_name.upper()
        self.logger.log(
            "[CHIPSEC] Write {} + 0x{:X}: 0x{:08X}".format(bar, self.offset, self.value)
        )
        self._mmio.write_MMIO_BAR_reg(
            bar, self.offset, self.value, self.width, self.bus
        )

    def write_abs(self):
        self.logger.log(
            "[CHIPSEC] Write 0x{:X} + 0x{:X}: 0x{:08X}".format(
                self.base, self.offset, self.value
            )
        )
        if self.width == 1:
