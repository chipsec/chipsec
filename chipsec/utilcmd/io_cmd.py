...
class PortIOCommand(BaseCommand):
    def requires_driver(self):
        ...
        parser_r.add_argument(
            "_port", metavar="port", type=lambda x: int(x, 0), help="io port"
        )
        parser_r.add_argument(
            "_width", metavar="width", type=int, choices=[0x1, 0x2, 0x4], help="width"
        )
        ...
        parser_w.add_argument(
            "_value", metavar="value", type=lambda x: int(x, 0), help="value"
        )
    ...
    def io_write(self):
        ...
        self.logger.log(
            "[CHIPSEC] OUT 0x{:04X} <- 0x{:08X} (size = 0x{:02X})".format(
                self._port, self._value, self._width
            )
        )
        return
