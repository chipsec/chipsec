...
class VMemCommand(BaseCommand):
    ...
    def run(self):
        ...
        if 'allocate'   == op and 4 == len(self.argv):
            ...
            self.logger.log( '[CHIPSEC] reading %X-byte value from VA0x%016X..' % (width, virt_address) )
            ...
            self.logger.log( '[CHIPSEC] writing %X-byte value 0x%X to VA0x%016X..' % (width, value, virt_address) )
