...
class Chipset:
    ...
    def detect_platform(self):
        ...
        try:
            ...
            if logger().DEBUG:
                logger().error("pci.read_dword couldn't read platform VID/DID")
        if not vid in PCH_ADDRESS:
            if logger().DEBUG:
                logger().error("PCH address unknown for VID 0x{:04X}.".format(vid))
        else:
            try:
                ...
                if logger().DEBUG:
                    logger().error("pci.read_dword couldn't read PCH VID/DID")
    ...
    def get_cpuid(self):
        ...
        ret = "{:01X}{:01X}{:01X}{:01X}{:01X}".format(
            extmodel, ptype, family, model, stepping
        )
        if extfamily == 0:
            return ret
        else:
    ...
    def init(
        ...
        logger().log(
            "[CHIPSEC] API mode: {}".format(
                "using OS native API (not using CHIPSEC kernel module)"
                if self.use_native_api()
                else "using CHIPSEC kernel module API"
            )
        )
        ...
        if platform_code is None:
            ...
            if (
                vid in self.chipset_dictionary
                and did in self.chipset_dictionary[vid]
                and len(self.chipset_dictionary[vid][did]) > 1
                and cpuid in self.detection_dictionary.keys()
            ):
            ...
            msg = "Platform: Actual values: VID = 0x{:04X}, DID = 0x{:04X}, RID = 0x{:02X}".format(
                vid, did, rid
            )
            if cpuid:
        ...
        if req_pch_code is not None:
            ...
            if req_pch_code in self.pch_codes:
                ...
                _unknown_pch = False
                msg = "PCH     : Actual values: VID = 0x{:04X}, DID = 0x{:04X}, RID = 0x{:02X}".format(
                    pch_vid, pch_did, pch_rid
                )
                logger().log("[CHIPSEC] {}".format(msg))
        elif (
            pch_vid in self.pch_dictionary.keys()
            and pch_did in self.pch_dictionary[pch_vid].keys()
        ):
            ...
            if len(pch_list) > 1:
                logger().log(
                    "[!]       Multiple PCHs contain the same DID. Using first in the list."
                )
        ...
        if _unknown_platform:
            msg = "Unknown Platform: VID = 0x{:04X}, DID = 0x{:04X}, RID = 0x{:02X}".format(
                vid, did, rid
            )
            if start_driver:
                logger().error(msg)
        ...
        if not _unknown_platform:  # don't initialize config if platform is unknown
            self.init_cfg()
        if _unknown_pch:
            msg = "Unknown PCH: VID = 0x{:04X}, DID = 0x{:04X}, RID = 0x{:02X}".format(
                pch_vid, pch_did, pch_rid
            )
            if self.reqs_pch and start_driver:
                logger().error(
                    "Chipset requires a supported PCH to be loaded. {}".format(msg)
                )
                raise UnknownChipsetError(msg)
            else:
                logger().log("[!]       {}; Using Default.".format(msg))
    ...
    def print_chipset(self):
        logger().log(
            "[*] Platform: {}\n          VID: {:04X}\n          DID: {:04X}\n          RID: {:02X}".format(
                self.longname, self.vid, self.did, self.rid
            )
        )
    ...
    def print_pch(self):
        logger().log(
            "[*] PCH     : {}\n          VID: {:04X}\n          DID: {:04X}\n          RID: {:02X}".format(
                self.pch_longname, self.pch_vid, self.pch_did, self.pch_rid
            )
        )
    ...
    def init_xml_configuration(self):
        ...
        VID = [
            f
            for f in os.listdir(_cfg_path)
            if os.path.isdir(os.path.join(_cfg_path, f)) and is_hex(f)
        ]
        # create dictionaries
        for vid in VID:
            if logger().DEBUG:
                logger().log(
                    "[*] Entering directory '{}'..".format(os.path.join(_cfg_path, vid))
                )
            ...
            for fxml in os.listdir(os.path.join(_cfg_path, vid)):
                if logger().DEBUG:
                    logger().log(
                        "[*] looking for platform config in '{}'..".format(fxml)
                    )
                ...
                for _cfg in root.iter("configuration"):
                    if "platform" not in _cfg.attrib:
                        if logger().DEBUG:
                            logger().log(
                                "[*] skipping common platform config '{}'..".format(
                                    fxml
                                )
                            )
                        continue
                        ...
                        if logger().DEBUG:
                            logger().log(
                                "[*] found platform config from '{}'..".format(fxml)
                            )
                        if (
                            not _cfg.attrib["platform"].upper()
                            in self.chipset_codes.keys()
                        ):
                    ...
                    for _info in _cfg.iter("info"):
                        if "family" in _info.attrib:
                            ...
                            CHIPSET_FAMILY[family].append(
                                _cfg.attrib["platform"].upper()
                            )
                        if "detection_value" in _info.attrib:
                            for dv in list(_info.attrib["detection_value"].split(",")):
                                if dv[-1].upper() == "X":
                                    rdv = (
                                        int(dv[:-1], 16) << 4
                                    )  #  Assume valid hex value with last nibble removed
                                    ...
                                    rdv = dv.split("-")
                                    for rdv_value in range(
                                        int(rdv[0], 16), int(rdv[1], 16) + 1
                                    ):  #  Assume valid hex values
                                        self.detection_dictionary[
                                            format(rdv_value, "X")
                                        ] = _cfg.attrib["platform"].upper()
                                else:
    ...
    def load_xml_configuration(self):
        ...
        _cfg_path = os.path.join(
            chipsec.file.get_main_dir(), "chipsec/cfg", "{:04X}".format(self.vid)
        )
        for root, subdirs, files in os.walk(_cfg_path):
            _cfg_files.extend(
                [os.path.join(root, x) for x in files if fnmatch.fnmatch(x, "*.xml")]
            )
        _cfg_files.sort()
        if logger().DEBUG:
            ...
            for plat in [c.lower() for c in self.chipset_codes]:
                platform_files.extend(
                    [
                        x
                        for x in _cfg_files
                        if fnmatch.fnmatch(os.path.basename(x), "{}*.xml".format(plat))
                        or os.path.basename(x).startswith(PCH_CODE_PREFIX.lower())
                    ]
                )
            loaded_files.extend(
                [
                    x
                    for x in _cfg_files
                    if x not in loaded_files and x not in platform_files
                ]
            )
        ...
        if self.code and CHIPSET_CODE_UNKNOWN != self.code:
            for _xml in _cfg_files:
                if fnmatch.fnmatch(
                    os.path.basename(_xml), "{}*.xml".format(self.code.lower())
                ):
                    loaded_files.append(_xml)
        ...
        if self.pch_code and CHIPSET_CODE_UNKNOWN != self.pch_code:
            for _xml in _cfg_files:
                if fnmatch.fnmatch(
                    os.path.basename(_xml).lower(),
                    "{}*.xml".format(self.pch_code.lower()),
                ):
                    loaded_files.append(_xml)
                ...
                if "undef" in _named_item.attrib:
                    if _name in config_to_modify:
                        if logger().DEBUG:
                            logger().log(
                                "    - {:16}: {}".format(
                                    _name, _named_item.attrib["undef"]
                                )
                            )
                    ...
                    for _field in _named_item.iter("field"):
                        ...
                        if "lockedby" in _field.attrib:
                            ...
                            if _lockedby in self.Cfg.LOCKEDBY.keys():
                                self.Cfg.LOCKEDBY[_lockedby].append(
                                    (_name, _field_name)
                                )
                            else:
    ...
    def init_cfg_xml(self, fxml, code, pch_code):
        ...
        for _cfg in root.iter("configuration"):
            if "platform" not in _cfg.attrib:
                if logger().DEBUG:
                    logger().log(
                        "[*] loading common platform config from '{}'..".format(fxml)
                    )
            elif code == _cfg.attrib["platform"].lower():
                if logger().DEBUG:
                    logger().log(
                        "[*] loading '{}' platform config from '{}'..".format(
                            code, fxml
                        )
                    )
                ...
                if logger().DEBUG:
                    logger().log(
                        "[*] loading '{}' PCH config from '{}'..".format(pch_code, fxml)
                    )
            else:
                continue
            ...
            if xml_vid and xml_did:
                ...
                for tdid in did_list:
                    ...
                    if cfg_str in replaced_devices.keys():
                        ...
                        self.Cfg.CONFIG_PCI[config_device]["bus"] = "0x{:02X}".format(
                            self.Cfg.BUS[config_device][0]
                        )
                        if logger().DEBUG:
                            buses = ",".join(
                                "0x{:02X}".format(i)
                                for i in self.Cfg.BUS[config_device]
                            )
                            logger().log(
                                " + {:16s}: VID 0x{:04X} - DID 0x{:04X} -> Bus {:s}".format(
                                    config_device, vid, tdid, buses
                                )
                            )
                        break
    ...
    def init_cfg(self):
        if self.code and "" != self.code:
            try:
                ...
                logger().log_good(
                    "imported platform specific configuration: chipsec.cfg.{}".format(
                        self.code
                    )
                )
            ...
            if reg_def["type"] in ["pcicfg", "mmcfg"]:
                ...
                if dev_name in self.Cfg.IMA_REGISTERS:
                    ...
                    if dev["index"] in self.Cfg.REGISTERS:
                        ...
                        logger().error(
                            "Index register {} not found".format(dev["index"])
                        )
                    ...
                    logger().error(
                        "Indirect access device {} not found".format(dev_name)
                    )
        return reg_def
        ...
        if (RegisterType.PCICFG == rtype) or (RegisterType.MMCFG == rtype):
            ...
            if do_check and CONSISTENCY_CHECKING:
                if self.pci.get_DIDVID(b, d, f) == (0xFFFF, 0xFFFF):
                    raise CSReadError(
                        "PCI Device is not available ({}:{}.{})".format(b, d, f)
                    )
            if RegisterType.PCICFG == rtype:
                if 1 == size:
                    ...
                    reg_value = (
                        self.pci.read_dword(b, d, f, o + 4) << 32
                    ) | self.pci.read_dword(b, d, f, o)
            ...
            if self.mmio.get_MMIO_BAR_base_address(reg["bar"], _bus)[0] != 0:
                reg_value = self.mmio.read_MMIO_BAR_reg(
                    reg["bar"], int(reg["offset"], 16), int(reg["size"], 16), _bus
                )
            else:
            ...
            if self.iobar.get_IO_BAR_base_address(reg["bar"])[0] != 0:
                reg_value = self.iobar.read_IO_BAR_reg(
                    reg["bar"], int(reg["offset"], 16), int(reg["size"], 16)
                )
            else:
                raise CSReadError("IO Bar ({}) base address is 0".format(reg["bar"]))
        elif RegisterType.MSGBUS == rtype:
            reg_value = self.msgbus.msgbus_reg_read(
                int(reg["port"], 16), int(reg["offset"], 16)
            )
        elif RegisterType.MM_MSGBUS == rtype:
            reg_value = self.msgbus.mm_msgbus_reg_read(
                int(reg["port"], 16), int(reg["offset"], 16)
            )
            ...
            if reg["access"] == "dram":
                ...
                if 1 == size:
                    ...
                    reg_value = self.mem.read_physical_mem_dword(
                        int(reg["address"], 16)
                    )
                elif 8 == size:
                    reg_value = self.mem.read_physical_mem_qword(
                        int(reg["address"], 16)
                    )
            elif reg["access"] == "mmio":
                reg_value = self.mmio.read_MMIO_reg(
                    int(reg["address"], 16),
                    int(reg["offset"], 16),
                    int(reg["size"], 16),
                )
            ...
            self.write_register(
                reg["index"], int(reg["offset"], 16) + int(reg["base"], 16)
            )
    ...
    def write_register(self, reg_name, reg_value, cpu_thread=0, bus=None):
        ...
        if (RegisterType.PCICFG == rtype) or (RegisterType.MMCFG == rtype):
            ...
            self.mmio.write_MMIO_BAR_reg(
                reg["bar"], int(reg["offset"], 16), reg_value, int(reg["size"], 16), bus
            )
        elif RegisterType.MSR == rtype:
            ...
            self.iobar.write_IO_BAR_reg(
                reg["bar"], int(reg["offset"], 16), int(reg["size"], 16), reg_value
            )
        elif RegisterType.MSGBUS == rtype:
            self.msgbus.msgbus_reg_write(
                int(reg["port"], 16), int(reg["offset"], 16), reg_value
            )
        elif RegisterType.MM_MSGBUS == rtype:
            self.msgbus.mm_msgbus_reg_write(
                int(reg["port"], 16), int(reg["offset"], 16), reg_value
            )
        elif RegisterType.MEMORY == rtype:
            if reg["access"] == "dram":
                self.mem.write_physical_mem(
                    int(reg["address"], 16), int(reg["size"], 16), reg_value
                )
            elif reg["access"] == "mmio":
                self.mmio.write_MMIO_reg(
                    int(reg["address"], 16),
                    int(reg["offset"], 16),
                    reg_value,
                    int(reg["size"], 16),
    ...
    def write_register_all(self, reg_name, reg_values, cpu_thread=0):
        ...
        if RegisterType.MSR == rtype:
            ...
            rtype in [RegisterType.MMCFG, RegisterType.PCICFG, RegisterType.MMIO]
            and bus_data
        ):
            values = len(bus_data)
            if len(reg_values) == values:
                for index in range(values):
                    self.write_register(
                        reg_name, reg_values[index], cpu_thread, bus_data[index]
                    )
                ret = True
        else:
            if len(reg_values) == 1:
        ...
        if not ret and logger().DEBUG:
            logger().log(
                "[write_register_all] There is a mismatch in the number of register values and registers to write"
            )
        return ret
    ...
    def get_register_field_mask(
        self, reg_name, reg_field=None, preserve_field_position=False
    ):
        reg_def = self.get_register_def(reg_name)
    ...
    def get_register_field(
        self, reg_name, reg_value, field_name, preserve_field_position=False
    ):
    ...
    def get_register_field_all(
        self, reg_name, reg_values, field_name, preserve_field_position=False
    ):
        values = []
        for reg_value in reg_values:
            values.append(
                self.get_register_field(
                    reg_name, reg_value, field_name, preserve_field_position
                )
            )
        return values

    def set_register_field(
        self,
        reg_name,
        reg_value,
        field_name,
    ...
    def read_register_field(
        ...
        return self.get_register_field(
            reg_name, reg_value, field_name, preserve_field_position
        )

    def read_register_field_all(
        self, reg_name, field_name, preserve_field_position=False, cpu_thread=0
    ):
        reg_values = self.read_register_all(reg_name, cpu_thread)
        return self.get_register_field_all(
            reg_name, reg_values, field_name, preserve_field_position
        )

    def write_register_field(
        self,
        reg_name,
        field_name,
        field_value,
        ...
        try:
            ...
            reg_value_new = self.set_register_field(
                reg_name, reg_value, field_name, field_value, preserve_field_position
            )
    ...
    def write_register_field_all(
        ...
        reg_values = self.read_register_all(reg_name, cpu_thread)
        reg_values_new = self.set_register_field_all(
            reg_name, reg_values, field_name, field_value, preserve_field_position
        )
    ...
    def print_register(self, reg_name, reg_val, bus=None, cpu_thread=0):
        ...
        if RegisterType.PCICFG == rtype or RegisterType.MMCFG == rtype:
            ...
            if RegisterType.MMCFG == rtype:
                mmcfg_off_str += ", MMCFG + 0x{:X}".format(
                    (b * 32 * 8 + d * 8 + f) * 0x1000 + o
                )
            ...
            reg_str = "[*] {} = {} << {} (MSR 0x{:X} Thread 0x{:X})".format(
                reg_name, reg_val_str, reg["desc"], int(reg["msr"], 16), cpu_thread
            )
        elif RegisterType.PORTIO == rtype:
            reg_str = "[*] {} = {} << {} (I/O port 0x{:X})".format(
                reg_name, reg_val_str, reg["desc"], int(reg["port"], 16)
            )
        elif RegisterType.IOBAR == rtype:
            reg_str = "[*] {} = {} << {} (I/O {} + 0x{:X})".format(
                reg_name, reg_val_str, reg["desc"], reg["bar"], int(reg["offset"], 16)
            )
    ...
    def register_is_pci(self, reg_name):
        if self.is_register_defined(reg_name):
            reg_def = self.Cfg.REGISTERS[reg_name]
            if (reg_def["type"].lower() == "pcicfg") or (
                reg_def["type"].lower() == "mmcfg"
            ):
                return True
        return False
