...
class CONTROLHelper(BaseConfigHelper):
    ...
    def __init__(self, cfg_obj: Dict[str, Any], reg_obj: Any):
        ...
        self.__reg = reg_obj
        # Normalize to the owning device instance. A register's raw ``instance``
        # is the BAR object when the register is BAR-relative, so defer to
        # ``get_instance()``, which unwraps it, to stay comparable with the
        # instance reported by any register.
