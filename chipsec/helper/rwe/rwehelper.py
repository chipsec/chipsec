...
IOCTL_READ_PHYSMEM = CTL_CODE(
    FILE_DEVICE_UNKNOWN, 0xA02, METHOD_BUFFERED, CHIPSEC_CTL_ACCESS
)
IOCTL_WRITE_PHYSMEM = CTL_CODE(
    FILE_DEVICE_UNKNOWN, 0xA03, METHOD_BUFFERED, CHIPSEC_CTL_ACCESS
)
...
IOCTL_READ_IO_PORT_BYTE = CTL_CODE(
    FILE_DEVICE_UNKNOWN, 0xA04, METHOD_BUFFERED, CHIPSEC_CTL_ACCESS
)
IOCTL_READ_IO_PORT_WORD = CTL_CODE(
    FILE_DEVICE_UNKNOWN, 0xA06, METHOD_BUFFERED, CHIPSEC_CTL_ACCESS
)
IOCTL_READ_IO_PORT_DWORD = CTL_CODE(
    FILE_DEVICE_UNKNOWN, 0xA08, METHOD_BUFFERED, CHIPSEC_CTL_ACCESS
)
IOCTL_WRITE_IO_PORT_BYTE = CTL_CODE(
    FILE_DEVICE_UNKNOWN, 0xA05, METHOD_BUFFERED, CHIPSEC_CTL_ACCESS
)
IOCTL_WRITE_IO_PORT_WORD = CTL_CODE(
    FILE_DEVICE_UNKNOWN, 0xA07, METHOD_BUFFERED, CHIPSEC_CTL_ACCESS
)
IOCTL_WRITE_IO_PORT_DWORD = CTL_CODE(
    FILE_DEVICE_UNKNOWN, 0xA09, METHOD_BUFFERED, CHIPSEC_CTL_ACCESS
)
...
IOCTL_ALLOC_PHYSMEM = CTL_CODE(
    FILE_DEVICE_UNKNOWN, 0xA20, METHOD_BUFFERED, CHIPSEC_CTL_ACCESS
)
IOCTL_FREE_PHYSMEM = CTL_CODE(
    FILE_DEVICE_UNKNOWN, 0xA21, METHOD_BUFFERED, CHIPSEC_CTL_ACCESS
)
...
def guid_str(guid0, guid1, guid2, guid3):
    return "{:08X}-{:04X}-{:04X}-{:4}-{:6}".format(
        guid0,
        guid1,
        guid2,
        guid3[:2].encode("hex").upper(),
...
class RweHelper(Helper):
            ...
            if logger().DEBUG:
                logger().warn(
                    "Invalid handle (wtf?): re-opened device '{:.64}' (new handle: {:08X})".format(
                        self.device_file, self.driver_handle
                    )
                )
            return False
        return True
    ...
    def _ioctl(self, ioctl_code, in_buf, out_length):
        ...
        self.get_driver_handle()
        if logger().DEBUG:
            print_buffer(bytestostring(in_buf))
        try:
    ...
    def read_phys_mem(self, phys_address_hi, phys_address_lo, length):
        ...
        out_buf = (c_char * out_length)()
        in_buf = struct.pack(
            "<4IQ", phys_address_lo, phys_address_hi, length, 0, addressof(out_buf)
        )
    ...
    def write_phys_mem(self, phys_address_hi, phys_address_lo, length, buf):
        ...
        # print " *** write_phys_mem *** "
        in_buf = struct.pack(
            "<4IQ", phys_address_lo, phys_address_hi, len(buf), 0, addressof(write_buf)
        )
    ...
    def read_mmio_reg(self, phys_address, size):
        ...
        out_buf = self.read_phys_mem(
            (phys_address >> 32) & 0xFFFFFFFF, phys_address & 0xFFFFFFFF, size
        )
        if size == 8:
    ...
    def EFI_supported(self):
        ...
        return (self.GetFirmwareEnvironmentVariable is not None) or (
            self.GetFirmwareEnvironmentVariableEx is not None
        )
    ...
    def set_EFI_variable(self, name, guid, data, datasize, attrs):
        ...
        if attrs is None:
            ...
            if self.SetFirmwareEnvironmentVariableEx is not None:
                if logger().DEBUG:
                    logger().log(
                        "[helper] -> SetFirmwareEnvironmentVariableEx( name='{}', GUID='{}', length=0x{:X}, length=0x{:X} )..".format(
    ...
    def get_tool_path(self, tool_type):
        ...
        try:
            ...
            tool_path = pkg_resources.resource_filename(
                "{}.{}".format(chipsec.file.TOOLS_DIR, self.os_system.lower()),
                tool_name,
            )
        except ImportError:
            pass
        ...
        if not os.path.isfile(tool_path):
            ...
            if not os.path.isfile(tool_path):
                if logger().DEBUG:
                    logger().error("Couldn't find {}".format(tool_path))
    ...
    def decompress_file(self, CompressedFileName, OutputFileName, CompressionType):
        ...
        if CompressionType == 0:  # not compressed
            ...
            try:
                subprocess.call(
                    [exe, "-d", "-o", OutputFileName, CompressedFileName],
