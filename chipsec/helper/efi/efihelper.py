...
class EfiHelper(Helper):
    ...
    def write_phys_mem(self, phys_address_hi, phys_address_lo, length, buf):
        if 4 == length:
            ...
            edk2.writemem_dword(
                phys_address_lo, phys_address_hi, bytestostring(dword_value)
            )
        else:
    ...
    def get_descriptor_table(self, cpu_thread_id, desc_table_code):
        if logger().DEBUG:
            logger().log_error(
                "EFI helper has not implemented get_descriptor_table yet"
            )
        return 0
    ...
    def get_EFI_variable_full(self, name, guidstr):
        ...
        if Status == 5:
            size = DataSize + 1
            (Status, Attributes, newdata, DataSize) = edk2.GetVariable(
                name, guidstr, size
            )
    ...
    def set_EFI_variable(self, name, guidstr, data, datasize=None, attrs=0x7):
        ...
        (Status, datasize, guidstr) = edk2.SetVariable(
            name, guidstr, int(attrs), data, datasize
        )
    ...
    def list_EFI_variables(self):
        ...
        while not search_complete:
            ...
            (status, namestr, size, guidstr) = edk2.GetNextVariableName(
                size, name, guid
            )
            ...
            if status == 5:
                if logger().DEBUG:
                    logger().log(
                        "[helper] EFI Variable name size was too small increasing to {:d}".format(
                            size
                        )
                    )
                (status, namestr, size, guidstr) = edk2.GetNextVariableName(
            ...
            if logger().DEBUG:
                logger().log(
                    "[helper] Returned {}. Status is {}".format(
            ...
            if status:
                ...
                if logger().DEBUG:
                    logger().log(
                        "[helper] Found variable '{}' - [{}]".format(name, guidstr)
                    )
        ...
        for (name, guidstr) in var_list:
            ...
            if status:
                logger().verbose_log(
                    "[helper] Error reading variable {}.  Status = {:d} - {}".format(
                        name, status, status_dict[status]
                    )
                )
            ...
            if name in variables:
                logger().verbose_log(
                    "[helper] Duplicate variable name {} - {}".format(name, guidstr)
                )
                continue
            else:
                variables[name] = []
