...
from chipsec.exceptions import (
    OsHelperError,
    HWAccessViolationError,
    UnimplementedAPIError,
    UnimplementedNativeAPIError,
)
...
DRIVER_FILE_PATHS = [
    os.path.join("C:\\", "Windows", "System32", "drivers"),
    os.path.join(
        chipsec.file.get_main_dir(),
        "chipsec",
        "helper",
        "win",
        "win7_" + platform.machine().lower(),
    ),
]
...
READ_PCI_CFG_REGISTER = CTL_CODE(
    FILE_DEVICE_UNKNOWN, 0x807, METHOD_BUFFERED, CHIPSEC_CTL_ACCESS
)
WRITE_PCI_CFG_REGISTER = CTL_CODE(
    FILE_DEVICE_UNKNOWN, 0x808, METHOD_BUFFERED, CHIPSEC_CTL_ACCESS
)
IOCTL_READ_PHYSMEM = CTL_CODE(
    FILE_DEVICE_UNKNOWN, 0x809, METHOD_BUFFERED, CHIPSEC_CTL_ACCESS
)
IOCTL_WRITE_PHYSMEM = CTL_CODE(
    FILE_DEVICE_UNKNOWN, 0x80A, METHOD_BUFFERED, CHIPSEC_CTL_ACCESS
)
IOCTL_LOAD_UCODE_PATCH = CTL_CODE(
    FILE_DEVICE_UNKNOWN, 0x80B, METHOD_BUFFERED, CHIPSEC_CTL_ACCESS
)
...
IOCTL_READ_IO_PORT = CTL_CODE(
    FILE_DEVICE_UNKNOWN, 0x80E, METHOD_BUFFERED, CHIPSEC_CTL_ACCESS
)
IOCTL_WRITE_IO_PORT = CTL_CODE(
    FILE_DEVICE_UNKNOWN, 0x80F, METHOD_BUFFERED, CHIPSEC_CTL_ACCESS
)
IOCTL_GET_CPU_DESCRIPTOR_TABLE = CTL_CODE(
    FILE_DEVICE_UNKNOWN, 0x810, METHOD_BUFFERED, CHIPSEC_CTL_ACCESS
)
IOCTL_SWSMI = CTL_CODE(FILE_DEVICE_UNKNOWN, 0x811, METHOD_BUFFERED, CHIPSEC_CTL_ACCESS)
IOCTL_ALLOC_PHYSMEM = CTL_CODE(
    FILE_DEVICE_UNKNOWN, 0x812, METHOD_BUFFERED, CHIPSEC_CTL_ACCESS
)
IOCTL_CPUID = CTL_CODE(FILE_DEVICE_UNKNOWN, 0x813, METHOD_BUFFERED, CHIPSEC_CTL_ACCESS)
IOCTL_HYPERCALL = CTL_CODE(
    FILE_DEVICE_UNKNOWN, 0x814, METHOD_BUFFERED, CHIPSEC_CTL_ACCESS
)
IOCTL_GET_PHYSADDR = CTL_CODE(
    FILE_DEVICE_UNKNOWN, 0x815, METHOD_BUFFERED, CHIPSEC_CTL_ACCESS
)
IOCTL_MAP_IO_SPACE = CTL_CODE(
    FILE_DEVICE_UNKNOWN, 0x816, METHOD_BUFFERED, CHIPSEC_CTL_ACCESS
)
IOCTL_FREE_PHYSMEM = CTL_CODE(
    FILE_DEVICE_UNKNOWN, 0x817, METHOD_BUFFERED, CHIPSEC_CTL_ACCESS
)
...
IOCTL_MSGBUS_SEND_MESSAGE = CTL_CODE(
    FILE_DEVICE_UNKNOWN, 0x81A, METHOD_BUFFERED, CHIPSEC_CTL_ACCESS
)
IOCTL_WRITE_MMIO = CTL_CODE(
    FILE_DEVICE_UNKNOWN, 0x81B, METHOD_BUFFERED, CHIPSEC_CTL_ACCESS
)
IOCTL_READ_MMIO = CTL_CODE(
    FILE_DEVICE_UNKNOWN, 0x81C, METHOD_BUFFERED, CHIPSEC_CTL_ACCESS
)
...
def packl_ctypes(lnum, bitlength):
    ...
    PyLong_AsByteArray(
        lnum, a, len(a), 1, 1
    )  # 4th param is for endianness 0 - big, non 0 - little
    return a.raw
...
#
FirmwareTableProviderSignature_ACPI = (
    0x41435049  # 'ACPI' - The ACPI firmware table provider
)
FirmwareTableProviderSignature_FIRM = (
    0x4649524D  # 'FIRM' - The raw firmware table provider
)
FirmwareTableProviderSignature_RSMB = (
    0x52534D42  # 'RSMB' - The raw SMBIOS firmware table provider
)
...
class EFI_HDR_WIN(
    namedtuple("EFI_HDR_WIN", "Size DataOffset DataSize Attributes guid")
):
    __slots__ = ()
...
def getEFIvariables_NtEnumerateSystemEnvironmentValuesEx2(nvram_buf):
    ...
    while (off + header_size) < bsize:
        ...
        efi_var_data = buffer[
            off
            + efi_var_hdr.DataOffset : off
            + efi_var_hdr.DataOffset
            + efi_var_hdr.DataSize
        ]
        ...
        if sys.version_info[0] < 3:
            efi_var_name = unicode(s, "utf-16-le", errors="replace").split("\u0000")[0]
        else:
            efi_var_name = str(s, "utf-16-le", errors="replace").split("\u0000")[0]
...
class Win32Helper(Helper):
    ...
    def __init__(self):
        ...
        try:
            ...
            self.SetFirmwareEnvironmentVariable = (
        ...
        try:
            self.NtEnumerateSystemEnvironmentValuesEx = (
        ...
        try:
            ...
            self.SetFirmwareEnvironmentVariableEx = (
    ...
    def create(self, start_driver):
        ...
        if logger().DEBUG:
            logger().log(
                "[helper] service control manager opened (handle = {})".format(hscm)
            )
            logger().log(
                "[helper] driver path: '{}'".format(os.path.abspath(self.driver_path))
            )
        ...
        try:
            ...
            if hs:
                if logger().DEBUG:
                    logger().log(
                        "[helper] service '{}' created (handle = 0x{:08X})".format(
                            SERVICE_NAME, int(hs)
                        )
                    )
        except win32service.error as err:
            if winerror.ERROR_SERVICE_EXISTS == err.args[0]:
                if logger().DEBUG:
                    logger().log(
                        "[helper] service '{}' already exists: {} ({:d})".format(
    ...
    def delete(self, start_driver):
        ...
        if (
            ...
            logger().warn(
                "cannot delete service '{}' (not stopped)".format(SERVICE_NAME)
            )
            return False
        ...
        try:
            ...
            if logger().DEBUG:
                logger().warn(
                    "RemoveService failed: {} ({:d})".format(err.args[2], err.args[0])
                )
            return False
    ...
    def start(self, start_driver, driver_exists=False):
        ...
        if self.use_existing_service:
            ...
            if logger().DEBUG:
                logger().log(
                    "[helper] service '{}' already running".format(SERVICE_NAME)
                )
                logger().log(
                    "[helper] trying to connect to existing '{}' service...".format(
                        SERVICE_NAME
                    )
                )
        else:
    ...
    def stop(self, start_driver):
        ...
        try:
            ...
            if logger().DEBUG:
                logger().error(
                    "StopService failed: {} ({:d})".format(err.args[2], err.args[0])
                )
            return False
        finally:
    ...
    def get_driver_handle(self):
        ...
        if (self.driver_handle is not None) and (
            INVALID_HANDLE_VALUE != self.driver_handle
        ):
        ...
        if (self.driver_handle is None) or (INVALID_HANDLE_VALUE == self.driver_handle):
            ...
            if logger().DEBUG:
                logger().log(
                    "[helper] opened device '{:.64}' (handle: {:08X})".format(
                        DEVICE_FILE, int(self.driver_handle)
                    )
                )
        return self.driver_handle
    ...
    def check_driver_handle(self):
        if 0x6 == kernel32.GetLastError():
            ...
            logger().warn(
                "Invalid handle (wtf?): re-opened device '{:.64}' (new handle: {:08X})".format(
                    self.device_file, int(self.driver_handle)
                )
            )
            return False
        return True
    ...
    def _ioctl(self, ioctl_code, in_buf, out_length):
        ...
        try:
            ...
            if STATUS_PRIVILEGED_INSTRUCTION == err_status:
                ...
                _handle_error(
                    "HW Access Error: DeviceIoControl returned status 0x{:X} ({})".format(
                        err_status, _err.args[2]
                    ),
                    err_status,
                )
    ...
    def write_phys_mem(self, phys_address_hi, phys_address_lo, length, buf):
        in_length = length + 12
        in_buf = struct.pack(
            "3I", phys_address_hi, phys_address_lo, length
        ) + stringtobytes(buf)
    ...
    def read_mmio_reg(self, phys_address, size):
        ...
        if logger().DEBUG:
            logger().log(
                "[helper] -> read_mmio_reg( phys_address=0x{:X}, size={} )".format(
                    phys_address, size
                )
            )
        in_buf = struct.pack(
            "3I", (phys_address >> 32) & 0xFFFFFFFF, phys_address & 0xFFFFFFFF, size
        )
    ...
    def hypercall(self, rcx, rdx, r8, r9, r10, r11, rax, rbx, rdi, rsi, xmm_buffer):
        if self.os_machine == "AMD64":
            ...
            out_length = 4
        in_buf = struct.pack(
            "<11" + arg_type, rcx, rdx, r8, r9, r10, r11, rax, rbx, rdi, rsi, xmm_buffer
        )
    ...
    def read_pci_reg(self, bus, device, function, address, size):
        bdf = PCI_BDF(
            bus & 0xFFFF, device & 0xFFFF, function & 0xFFFF, address & 0xFFFF
        )
        out_length = size
    ...
    def load_ucode_update(self, cpu_thread_id, ucode_update_buf):
        ...
        out_buf = (c_char * out_length)()
        in_buf = (
            struct.pack("=IH", cpu_thread_id, len(ucode_update_buf)) + ucode_update_buf
        )
    ...
    def get_EFI_variable_full(self, name, guid, attrs=None):
        ...
        if attrs is None:
            if self.GetFirmwareEnvironmentVariable is not None:
                if logger().DEBUG:
                    logger().log(
                        "[helper] -> GetFirmwareEnvironmentVariable( name='{}', GUID='{}' )..".format(
                            name, "{{{}}}".format(guid)
                        )
                    )
                length = self.GetFirmwareEnvironmentVariable(
                    name, "{{{}}}".format(guid), efi_var, EFI_VAR_MAX_BUFFER_SIZE
                )
        else:
            if self.GetFirmwareEnvironmentVariableEx is not None:
                ...
                if logger().DEBUG:
                    logger().log(
                        "[helper] -> GetFirmwareEnvironmentVariableEx( name='{}', GUID='{}', attrs = 0x{:X} )..".format(
                            name, "{{{}}}".format(guid), attrs
                        )
                    )
        ...
        if (0 == length) or (efi_var is None):
            ...
            if logger().DEBUG:
                logger().error(
                    "GetFirmwareEnvironmentVariable[Ex] returned error: {}".format(
                        WinError()
                    )
                )
            efi_var_data = None
    ...
    def set_EFI_variable(self, name, guid, data, datasize, attrs):
        ...
        if isinstance(attrs, (str, bytes)):
            attrs_data = "{message:\x00<{fill}}".format(
                message=bytestostring(attrs), fill=8
            )[:8]
        ...
        if attrs is None:
            if self.SetFirmwareEnvironmentVariable is not None:
                if logger().DEBUG:
                    logger().log(
                        "[helper] -> SetFirmwareEnvironmentVariable( name='{}', GUID='{}', length=0x{:X} )..".format(
                            name, "{{{}}}".format(guid), var_len
                        )
                    )
                ntsts = self.SetFirmwareEnvironmentVariable(
                    name, "{{{}}}".format(guid), var, var_len
                )
        else:
            if self.SetFirmwareEnvironmentVariableEx is not None:
                if logger().DEBUG:
                    logger().log(
                        "[helper] -> SetFirmwareEnvironmentVariableEx( name='{}', GUID='{}', length=0x{:X}, attrs=0x{:X} )..".format(
                            name, "{{{}}}".format(guid), var_len, attrs
                        )
                    )
                ntsts = self.SetFirmwareEnvironmentVariableEx(
                    name, "{{{}}}".format(guid), var, var_len, attrs
                )
        if 0 != ntsts:
            status = 0  # EFI_SUCCESS
        else:
            ...
            if logger().DEBUG:
                logger().error(
                    "SetFirmwareEnvironmentVariable[Ex] returned error: {}".format(
                        WinError()
                    )
                )
    ...
    def list_EFI_variables(self, infcls=2):
        if logger().DEBUG:
            logger().log(
                "[helper] -> NtEnumerateSystemEnvironmentValuesEx( infcls={:d} )..".format(
                    infcls
                )
            )
        ...
        if 0 != status:
            ...
            if (
                0xC0000001 == status and lasterror == 0x000003E6
            ):  # ERROR_NOACCESS: Invalid access to memory location.  https://docs.microsoft.com/en-us/openspecs/windows_protocols/ms-erref/18d8fbe8-a967-4f1c-ae50-99ca8e491d2d
                if logger().DEBUG:
                    logger().warn(
                        "NtEnumerateSystemEnvironmentValuesEx was not successful (NTSTATUS = 0xC0000001)"
                    )
                if logger().DEBUG:
                    logger().log(
                        "[*] Your Windows has restricted access to UEFI variables.\nTo use UEFI variable functions, chipsec needs to run in an older version of windows or in a different environment"
                    )
                return None
            else:
                if logger().DEBUG:
                    logger().error(
                        "NtEnumerateSystemEnvironmentValuesEx failed (GetLastError = 0x{:X})".format(
                            lasterror
                        )
                    )
    ...
    def send_sw_smi(
        ...
        if (sys.maxsize < 2**32 and self.os_machine == "AMD64") or (
            sys.maxsize > 2**32 and self.os_machine == "i386"
        ):
            logger().log(
                "[helper] Python architecture must match OS architecture.  Run with {} architecture of python".format(
                    self.os_machine
                )
            )
    ...
    def native_get_ACPI_table(self, table_name):
        ...
        if retVal == 0:
            if logger().DEBUG:
                logger().error(
                    "GetSystemFirmwareTable({}) returned error: {}".format(
                        table_name, WinError()
                    )
                )
            return None
