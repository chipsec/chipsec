...
class FileCmds:
    ...
    def Save(self):
        if version[0] == "3":
            js = json.dumps(
                self.data, sort_keys=False, indent=2, separators=(",", ": ")
            )
        else:
            js = json.dumps(
                self.data,
...
class FileHelper(Helper):
    ...
    def read_pci_reg(self, bus, device, function, address, size):
        ...
        return self.filecmds.getElement(
    ...
    def write_pci_reg(self, bus, device, function, address, value, size):
        ...
        return self.filecmds.getElement(
            "write_pci_reg", (bus, device, function, address, size)
        )
    ...
    def read_phys_mem(self, phys_address_hi, phys_address_lo, length):
        ret = self.filecmds.getElement(
            "read_physical_mem", (phys_address_hi, phys_address_lo, length)
        )
        return ret.encode("latin_1")

    def write_phys_mem(self, phys_address_hi, phys_address_lo, length, buf):
        return self.filecmds.getElement(
            "write_physical_mem", (phys_address_hi, phys_address_lo, length, buf)
        )
    ...
    def set_EFI_variable(self, name, guid, data, datasize=None, attrs=None):
        return self.filecmds.getElement(
            "set_EFI_variable", (name, guid, data, datasize, attrs)
        )
