...
BROTLI = os.path.join(
    chipsec.file.get_main_dir(), chipsec.file.TOOLS_DIR, "compression", "bin", "Brotli"
)
...
class LinuxHelper(Helper):
    ...
    def load_chipsec_module(self):
        ...
        driver_path = os.path.join(
            chipsec.file.get_main_dir(), "chipsec", "helper", "linux", "chipsec.ko"
        )
        ...
        try:
            ...
            raise Exception(
                "Could not start Linux Helper, are you running as Admin/root?\n\t{}".format(
                    err
                )
            )
        uid = gid = 0
    ...
    def unload_chipsec_module(self):
        if self.module_loaded:
            ...
            if logger().DEBUG:
                logger().log(
                    "Module for {} unloaded successfully".format(self.DEVICE_NAME)
                )
    ...
    def init(self, start_driver):
        ...
        if start_driver:
            ...
            try:
                ...
                raise OsHelperError(
                    "Unable to open chipsec device. Did you run as root/sudo and load the driver?\n {}".format(
                        str(e)
                    ),
                    e.errno,
                )
    ...
    def devmsr_available(self):
        ...
        try:
            ...
            for cpu in os.listdir("/dev/cpu"):
                ...
                if cpu.isdigit():
                    cpu = int(cpu)
                    self.dev_msr[cpu] = os.open(
                        "/dev/cpu/" + str(cpu) + "/msr", os.O_RDWR
                    )
                    if logger().DEBUG:
            ...
            raise OsHelperError(
                "Unable to open /dev/cpu/CPUNUM/msr.\n"
    ...
    def native_write_phys_mem(self, phys_address_hi, phys_address_lo, length, newval):
        ...
        if self.devmem_available():
            ...
            if written != length:
                if logger().DEBUG:
                    logger().error(
                        "Cannot write {} to memory {:016X} (wrote {:d} of {:d})".format(
                            newval, addr, written, length
                        )
                    )
    ...
    def va2pa(self, va):
        ...
        try:
            ...
            if logger().DEBUG:
                logger().error(
                    "[helper] Error in va2pa: getting PA for VA 0x{:016X} failed with IOError: {}".format(
                        va, err.strerror
                    )
                )
        ...
        if pa > 1 << max_pa:
            if logger().DEBUG:
                logger().error(
                    "[helper] Error in va2pa: PA higher that max physical address: VA (0x{:016X}) -> PA (0x{:016X})".format(
                        va, pa
                    )
                )
            error_code = 1
    ...
    def native_write_pci_reg(
        self, bus, device, function, offset, value, size=4, domain=0
    ):
        device_name = "{domain:04x}:{bus:02x}:{device:02x}.{function}".format(
            domain=domain, bus=bus, device=device, function=function
        )
    ...
    def load_ucode_update(self, cpu_thread_id, ucode_update_buf):
        ...
        in_buf = (
            struct.pack("=BH", cpu_thread_id, len(ucode_update_buf)) + ucode_update_buf
        )
    ...
    def native_write_io_port(self, io_port, newval, size):
        if self.devport_available():
            ...
            if written != size:
                if logger().DEBUG:
                    logger().error(
                        "Cannot write {} to port {:x} (wrote {:d} of {:d})".format(
    ...
    def native_write_msr(self, thread_id, msr_addr, eax, edx):
        if self.devmsr_available():
            ...
            if written != 8:
                if logger().DEBUG:
                    logger().error(
                        "Cannot write {:8X} to MSR {:x}".format(buf, msr_addr)
                    )
    ...
    def msgbus_send_read_message(self, mcr, mcrx):
        mdr_out = 0
        in_buf = struct.pack(
            "5" + self._pack, MSGBUS_MDR_OUT_MASK, mcr, mcrx, 0, mdr_out
        )
    ...
    def EFI_supported(self):
        return os.path.exists("/sys/firmware/efi/vars/") or os.path.exists(
    ...
    def kern_get_EFI_variable_full(self, name, guid):
        ...
        if status == 0x5:
            data_size = (
                new_size + header_size + namelen
            )  # size sent by driver + size of header (size + guid) + size of name
            in_buf = struct.pack(
                "13I" + str(namelen + new_size) + "s",
                data_size,
                guid0,
                guid1,
                guid2,
        ...
        if status > 0:
            if logger().DEBUG:
                logger().error(
                    "Reading variable (GET_EFIVAR) did not succeed: {} ({:d})".format(
                        status_dict.get(status, "UNKNOWN"), status
                    )
                )
            data = ""
            guid = 0
    ...
    def kern_list_EFI_variables(self):
        ...
        try:
            ...
            if logger().DEBUG:
                logger().error(
                    "Failed to read /sys/firmware/efi/[vars|efivars]. Folder does not exist"
                )
            return None
        variables = dict()
        for v in varlist:
    ...
    def kern_set_EFI_variable(self, name, guid, value, attr=0x7):
        ...
        in_buf = struct.pack(
            "15I" + str(namelen) + "s" + str(datalen) + "s",
            data_size,
            guid0,
            guid1,
            guid2,
        ...
        if status != 0:
            if logger().DEBUG:
                logger().error(
                    "Setting EFI (SET_EFIVAR) variable did not succeed: '{}' ({:d})".format(
                        status_dict.get(status, "UNKNOWN"), status
                    )
                )
        else:
    ...
    def VARS_get_efivar_from_sys(self, filename):
        ...
        try:
            ...
            if logger().DEBUG:
                logger().error(
                    "Failed to read files under /sys/firmware/efi/vars/" + filename
                )
            data = ""
            guid = 0
            attr = 0
    ...
    def VARS_list_EFI_variables(self):
        ...
        try:
            ...
            if logger().DEBUG:
                logger().error(
                    "Failed to read /sys/firmware/efi/vars. Folder does not exist"
                )
        variables = dict()
        for v in varlist:
            name = v[:-37]
    ...
    def EFIVARS_list_EFI_variables(self):
        ...
        try:
            ...
            if logger().DEBUG:
                logger().error(
                    "Failed to read /sys/firmware/efi/efivars. Folder does not exist"
                )
            return None
        variables = dict()
        for v in varlist:
            ...
            try:
                if os.path.isfile(path):
                    ...
                    if attrs is not None:
                        if logger().DEBUG:
                            logger().warn(
                                "Changing attributes on an existing variable is not supported. Keeping old attributes..."
                            )
                    f = open(path, "r")
                    sattrs = f.read(4)
                else:
    ...
    def send_sw_smi(
        ...
        self.set_affinity(cpu_thread_id)
        in_buf = struct.pack(
            "7" + self._pack, SMI_code_data, _rax, _rbx, _rcx, _rdx, _rsi, _rdi
        )
