...
class DALHelper(Helper):
    ...
    def target_machine(self):
        return (
            self.base.devicelist[0].devicetype + "-" + self.base.devicelist[0].stepping
        )
    ...
    def pci_addr(self, bus, device, function, offset):
        if (bus >= 256) or (device >= 32) or (function >= 8) or (offset >= 256):
            if logger().DEBUG:
                logger().log(
                    "[WARNING] PCI access out of range. Use mmio functions to access PCIEXBAR."
                )
    ...
    def read_pci_reg(self, bus, device, function, address, size):
        ...
        self.base.threads[ie_thread].dport(
            0xCF8, self.pci_addr(bus, device, function, address)
        )
        ...
        while width >= 1:
            while (length - ptr) >= width:
                v = self.base.threads[self.find_thread()].mem(
                    itpii.Address((phys_address + ptr), itpii.AddressType.physical),
                    width,
                )
    ...
    def read_msr(self, thread, msr_addr):
        if not self.base.threads[thread].isenabled:
            ...
            if logger().DEBUG:
                logger().log(
                    "[WARNING] Selected thread [{:d}] was disabled, using [{:d}].".format(
                        thread, en_thread
                    )
                )
            thread = en_thread
    ...
    def read_cr(self, cpu_thread_id, cr_number):
        ...
        if cr_number == 0:
            ...
            if logger().DEBUG:
                logger().log(
                    "[ERROR] Selected CR{:d} is not supported.".format(cr_number)
                )
            val = 0
        return val
    ...
    def get_threads_count(self):
        ...
        if logger().DEBUG:
            logger().log(
                "[helper] Threads discovered : 0x{:X} ({:d})".format(
