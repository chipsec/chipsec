...
def pack_Q(num_of_q: int, expected_value: int, expected_value_index:int = 0) -> bytes:
    ...
    return pack(f'{num_of_q}Q',*input)

def pack_pci(expected_value: int) -> bytes:
    return pack_Q(5, expected_value, 4)

def pack_cpuinfo(expected_value:int) -> bytes:
    return pack_Q(4, expected_value, 0)

def pack_ioport(expected_value:int) -> bytes:
    return pack_Q(3, expected_value, 2)
