...
class TestVarStore(unittest.TestCase):
    ...
    def test_run_placeholder_functions(self):
        """Test signature parsers with realistic-sized byte data."""
        sha1_data = b'\xda\x39\xa3\xee\x5e\x6b\x4b\x0d\x32\x55\xbf\xef\x95\x60\x18\x90\xaf\xd8\x07\x09'
        sha224_data = b'\x00' * 0x1c
        sha256_data = b'\x00' * 0x20
        sha384_data = b'\x00' * 0x30
        sha512_data = b'\x00' * 0x40
        rsa2048_data = b'\x00' * 0x100
        x509_data = b'\x30\x82\x01\x00' + b'\x00' * 0x100  # minimal DER stub
        pkcs7_data = b'\x30\x82\x01\x00' + b'\x00' * 0x100  # minimal DER stub
        external_data = b'\x00' * 0x01

        varstore.parse_sha256(sha256_data)
        varstore.parse_rsa2048(rsa2048_data)
        varstore.parse_rsa2048_sha256(rsa2048_data)
        varstore.parse_sha1(sha1_data)
        varstore.parse_rsa2048_sha1(rsa2048_data)
        varstore.parse_x509(x509_data)
        varstore.parse_sha224(sha224_data)
        varstore.parse_sha384(sha384_data)
        varstore.parse_sha512(sha512_data)
        varstore.parse_x509_sha256(sha256_data)
        varstore.parse_x509_sha384(sha384_data)
        varstore.parse_x509_sha512(sha512_data)
        varstore.parse_pkcs7(pkcs7_data)
        varstore.parse_external(external_data)
