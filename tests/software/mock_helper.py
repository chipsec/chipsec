...
class SPIHelper(TestHelper):
    ...
    def read_mmio_reg(self, pa, size, offset=0, bar_size=None):
        ...
        elif pa + offset >= self.SPIBAR_ADDR and pa < self.SPIBAR_END:
            return 0x0
        else:
            raise Exception("Unexpected address")

    def write_mmio_reg(self, pa, size, value, offset=0, bar_size=None):
        if pa + offset < self.SPIBAR_ADDR or pa > self.SPIBAR_END:
